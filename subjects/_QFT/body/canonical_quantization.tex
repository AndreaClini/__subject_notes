...
\begin{mytheorem}[Wave functional \& fluctuations of the conjugate field $\pi$.\todotag{Do it.}]
%
The above arguments are immediately applied to the conjugate field $\pi$.
Sticking to the treatment \eqref{eq:relations_fields_creation_operators_harmonic_oscillator_var} above with PBc, the field is decomposed as
\begin{align}
    \hat{\pi}(x,t_0) &= \int \frac{d^3\bar{k}}{(2\pi)^3}\, e^{i\bar{k}\cdot\bar{x}} \hat{\pi}_k(t_0)
    = : \int \frac{d^3\bar{k}}{(2\pi)^3} \frac{1}{\sqrt{2\omega_k}} \left( -i \omega_k a_k e^{i \bar{k}\cdot\bar{x}} + i \omega_k a_k^{\dagger} e^{- i \bar{k}\cdot\bar{x}} \right)\,,
\end{align}
...
\end{align}
%
As above recast the Hilbert space $\mathcal{M}_k\otimes \mathcal{M}_{-k} \equiv \mathcal{M}_+^{(k)} \otimes \mathcal{M}_-^{(k)}$ for the two oscillators with modes $k, -k$ into that for rotated oscillators $(Q_{\pm}, P_{\pm})$.
In the previous notation, a spectral basis for $\hat{\pi}_k$ is given by tensor states
\begin{align}
    |g_k\rangle := V^{\frac{1}{2}}\omega_k^{\frac{1}{2}}\, |\eta_+\rangle \otimes |\xi_-\rangle,\qquad
