...
\end{cases}
\end{align}}
On the other hand, if $\{i_1\dots i_k\} \cap \{j_{k+1}\dots j_n\} = \emptyset$ then $\epsilon_{\gamma_1\dots \gamma_k j_{k+1}\dots j_n}\neq0$ only if $\{\gamma_1\dots \gamma_k\} = \{i_1\dots i_k\}$.
...
\begin{align}
    d_\nabla T &= \Omega_{\, j} \wedge \phi^j\qquad \text{i.e.}\quad d_\nabla T:=d_\nabla(f_i\otimes T^i) = f_i\otimes\big( \Omega^i_{\, j} \wedge \phi^j\big),
    \\
    d \Omega^i_{\, j} &= 0\qquad \text{i.e.}\quad d_\nabla \Omega_{\, j}:=d_\nabla\Big(f_i \otimes \Omega^i_{\, j}\Big) = f_i \otimes \Big(\omega_k^i\wedge \Omega^k_j + \underbrace{d \Omega_{\, j}^i}_{=0}\Big) = f_i \otimes\omega_k^i\wedge \Omega^k_j\,.
\end{align}
...
\begin{align}
    &d_\nabla T :=d_\nabla\big(e_a\otimes T^a\big) = \Omega_{\, b} \wedge \theta^b\qquad \text{i.e.}\quad \omega_b^a\wedge T^b +d T^a = \Omega^a_{\, b} \wedge \theta^b,
    \\
    &d \Omega^a_{\, b} = 0\qquad \text{i.e.}\quad d_\nabla \Omega_{\, b} :=d_\nabla (e_a \otimes \Omega^a_{\, b})= e_a \otimes \Big(\omega_c^a\wedge \Omega^c_b + \underbrace{d \Omega_{\, b}^a}_{=0}\Big) = e_a \otimes\omega_c^a\wedge \Omega^c_b\,.
\end{align}
Beware the 2nd identity says the exterior derivative of the $\R$-valued 2-forms $\Omega^i_{\, j}$ vanish, \emph{not} that the exterior covariant derivative of the $(1,0)$-tensor valued 2-form $\Omega_{\, j}$ vanishes!
The \textbf{Bianchi identities} essentially express \textbf{symmetry contraints} that follow \textbf{from the very definition} of torsion and curvature.
...
\end{align}
To express the Einstein equations in this formalism, it wil be convenient to define a (0,2) tensor valued 2-form\todotag{Check def \& name}
\begin{align}
    H_{\alpha\beta} \theta^\alpha\otimes \theta^\beta,\quad H_{\alpha\beta}:= \star\Big(\theta^\alpha \wedge \theta^\beta\Big) = \tfrac{1}{2} \underbrace{\sqrt{|g|}}_{=1}\,\epsilon_{\alpha\beta\gamma\delta} \,\, \theta^\gamma \wedge \theta^\delta,
\end{align}
and a (0,3)-tensor valued 1-form 
\begin{align}
    H_{\alpha\beta\gamma} \theta^\alpha\otimes \theta^\beta \otimes \theta^\gamma,\quad H_{\alpha\beta\gamma} := \star\Big(\theta^\alpha \wedge \theta^\beta \wedge \theta^\gamma\Big) = \underbrace{\sqrt{|g|}}_{=1}\, \epsilon_{\alpha\beta\gamma\delta} \,\, \theta^\delta.
\end{align}
Note they are antisymmetric in all the indices, regardless of whch are considered to be part of the form or part of the tensor.

We now want to express the Eintein equations in this formalisms...
