...
Assume the Stone--von Neumann theorem applies and decompose the quantum field in terms of two different sets of mode functions $\{u_n, u_n^*\}_n$ and $\{\tilde{u}_m, \tilde{u}_m^*\}_m$ related by a Bogoliubov transformation \eqref{eq:transformation_bogoliubov_modes}-\eqref{eq:inverse_transformation_bogoliubov_modes}, that is
\begin{align}
    \tilde{u}_m = \sum_i \alpha_{mi} u_i + \beta_{mi} u_i^*\,,\quad u_j= \sum_m \alpha_{mj}^* \tilde{u}_m - \beta_{mj} \tilde{u}_m^*, \quad \alpha_{mi}=\langle u_i, \tilde{u}_m\rangle, \quad \beta_{mi} = -\langle u_i^*, \tilde{u}_m\rangle.
\end{align}
Equating the two expressions and using these relations we get
{\small
\begin{align}
    \begin{array}{c}
        \displaystyle    \hat{\phi}=\sum_j u_ja_j+u_j^*a_j^\dagger= \sum_m \Big(\sum_j \alpha_{mj}^* a_j - \beta_{mj}^* a_j^\dagger\Big) \tilde{u}_m + \Big(\sum_j \alpha_{mj} a_j^\dagger - \beta_{mj} a_j\Big) \tilde{u}_m^*\\[3pt]
        \displaystyle    \qquad=\sum_m \tilde{u}_m \tilde{a}_m + \tilde{u}_m^* \tilde{a}_m^\dagger= \sum_j \Big(\sum_m \alpha_{mj} \tilde{a}_m + \beta_{mj}^* \tilde{a}_m^\dagger\Big) u_j + \Big(\sum_m \alpha_{mj}^* \tilde{a}_m^\dagger + \beta_{mj} \tilde{a}_m\Big) u_j^*
    \end{array}\Rightarrow
    \begin{array}{c}
        \displaystyle
        \tilde{a}_m = \sum_j \alpha_{mj}^* a_j - \beta_{mj}^* a_j^\dagger,\\[3pt]
        \displaystyle
        a_j = \sum_m \alpha_{mj} \tilde{a}_m + \beta_{mj}^* \tilde{a}_m^\dagger.
    \end{array}
\end{align}}
More concisely, in matrix form, demanding both are simplectic bases enforces\todotag{redo it}
{\small
\begin{align}\label{eq:bogol_transform_mode_function_matrix_form_quantum}
    &\begin{pmatrix}\tilde{u}_m\\ \tilde{u}_m^*\end{pmatrix}
    = \begin{pmatrix}\alpha & \beta\\\beta^* & \alpha^* \end{pmatrix}
    \begin{pmatrix}u_j\\ u_j^*\end{pmatrix},\quad
    \begin{pmatrix}u_j\\u_j^*\end{pmatrix}
    = \begin{pmatrix}\alpha^\dagger & -\beta^T\\-\beta^\dagger & \alpha^T \end{pmatrix}
    \begin{pmatrix}\tilde{u}_m\\ \tilde{u}_m^*\end{pmatrix},
    \quad\text{with}\quad \begin{array}{c} \alpha\alpha^\dagger - \beta \beta^\dagger = \mathbb{I},\, \alpha\beta^T - \beta\alpha^T = 0 \\[2pt]
    \alpha^\dagger \alpha - \beta^T \beta^* = \mathbb{I},\, \alpha^\dagger \beta - \beta^T \alpha^* = 0
    \end{array}
\end{align}
Equating the two expressions for $\hat{\phi}$ we get
\begin{align}
...
We now relate the Fock spaces defined by ladder operators of the two theories, since under the Stone--von Neumann assumption these are unitarily equivalent via a transformation mapping the quantum DoFs of the theories into each other.
In particular, the vacuum of either theory must be expressible as a suitable superposition of multi-particle states of the other $|\tilde{0}\rangle =f\big(\{{a}^\dagger_j\}_n\big)|0\rangle$ for a suitable function $f$ of creation operators.\footnote{Note $f$ will indeed be independent of annihilation operators ${a}_j$ since they annihilate the vacuum $|0\rangle$ and we may commute them to the right upon changing the form of the $f$.}
...
\end{align}}
Since this must hold for arbitrary $m$ we get a set of differential equations for $f$
\begin{align}
    ...
    \,\,\Rightarrow\,\, f\big(a)\, \propto\,  \exp\Big(\tfrac{1}{2} a^\dagger_i \big[(\alpha^*)^{-1}\beta^*\big]_{ij} a^\dagger_j\Big).
\end{align}
Beware the matrix $(\alpha^*)^{-1}\beta^*=\big[(\alpha^*)^{-1}\beta^*\big]^T =\beta^\dagger (\alpha^\dagger)^{-1}$ is indeed symmetric since \eqref{eq:bogol_transform_mode_function_matrix_form_quantum} enforces $\alpha\beta^T-\beta\alpha^T=0$.

We next normalize the vacuum just like for squeezed states.
First note the hermitian conjugate matrix $\big[(\alpha^*)^{-1}\beta^*\big]^\dagger \equiv \big[\beta^\dagger (\alpha^\dagger)^{-1}\big]^\dagger = (\alpha)^{-1}\beta$ is of course again symmetric, namely $\big[(\alpha)^{-1}\beta\big]=\big[(\alpha)^{-1}\beta\big]^T= \beta^T (\alpha^T)^{-1}$.
Next note that
\begin{align}
    \exp\Big(-\tfrac{1}{2} a_p\big[\alpha^{-1}\beta\big]_{pq}a_q\Big)|\tilde{0}\rangle
    = \Big(1+ aa+\dots\Big) |0\rangle = |0\rangle.
\end{align}
Therefore we add this operator in between $f(a^\dagger)$ and $|0\rangle$ without changing the state.
Crucially the commutator of the two operators in the exponentials is a c-number thanks to \eqref{eq:bogol_transform_mode_function_matrix_form_quantum}, namely using the symmetry of the matrix $M^*_{ij}=(\alpha^*)^{-1}\beta^*$ we have 
{\small\begin{align}
    \Big[\tfrac12 a_p\big(\alpha^{-1}\beta\big)_{pq}a_q\,,\,\tfrac12 \,a^\dagger_i \big((\alpha^*)^{-1}\beta^*\big)_{ij} a^\dagger_j\Big]
    &= \tfrac12  a_p a^\dagger_i (M M^*)_{pi}+ \tfrac12 a_i^\dagger a_p (MM^*)_{pi}
    &= ...
\end{align}}
Next use the Baker--Campbell--Hausdorff formula to get
{\small
\begin{align}
   \exp\Big(\tfrac{1}{2} a^\dagger_i \big[(\alpha^*)^{-1}\beta^*\big]_{ij} a^\dagger_j\Big)
   \exp\Big(-\tfrac{1}{2} a_p\big[\alpha^{-1}\beta\big]_{pq}a_q\Big)
    = 
    \exp\Big(\tfrac{1}{2} a^\dagger_i \big[(\alpha^*)^{-1}\beta^*\big]_{ij} a^\dagger_j-\tfrac{1}{2} a_p\big[\alpha^{-1}\beta\big]_{pq}a_q\Big) + \exp\Big(\textcolor{red}{??}\Big).
\end{align}}
Normalizing the vacuum states we find\todotag{fnish}
{\small
\begin{align}
    ....
\end{align}}
More generally, using the Baker--Campbell--Hausdorff formula we can express any state of the theory as a suitable superposition of states of the other theory, namely
{\small
\begin{align}
    ...
\end{align}}
The same holds swapping the two theories with the replacements $\alpha \leftrightarrow \alpha^\dagger$ and $\beta \leftrightarrow -\beta^T$.
%
\end{mytheorem}
