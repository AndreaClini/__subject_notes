...
\begin{mytheorem}[Observable DoFs of the Klein-Gordon field in flat spacetime\todotag{Do it}]
...
%
In ordinary QM, we can expand the state $|\Psi\rangle$ in terms of the common eigenstates of suitable sets of commuting self-adjoint operators.
...
\begin{mytheorem}[Position \& momentum basis in QFT i.e. eigenstates of field operators $\hat{\phi},\,\hat{\pi}$]
%
...
\end{align}
That is, the state $|f\rangle$ has eigenvalue collections $\{f(x)\}_{x}$ for the commuting operators $\{\hat{\phi}(x)\}_x$, resp. $|g\rangle$ has $\{g(x)\}_x$ for $\{\hat{\pi}(x)\}_x$, for suitable functions $f,g:\mathbb{R}^3 \to \mathbb{R}$.
%
...
That is the function space the functions $f$ and $g$, i.e. the possible collections $\{f(x)\}_x$ or $\{g(x)\}_x$ of $\hat{\phi}$ or $\hat{\pi}$ eigenvalues, belong to.
...
That is, the expressions $|\{f(x)\}_x\rangle\equiv |\{f_k\}_k\rangle$ denote the \emph{same} state $|f\rangle$ i.e. the same field configuration, simply explicitated in terms of position space values or Fourier modes.
...
Indeed, for fixed $k$ the operator $\hat{\phi}_k(t)$ is a linear combination of the $\{\hat{\phi}(t,x)\}_x$ operators and returns the corresponding linear combination $f_k$ of eigenvalues when acting on the state $|f\rangle$.
%
In formulas
\begin{align}
...
\end{align}
The two expressions should coincide since the Foruier transform is expected be a measure-preserving bijection between the two expression of the function space $W_{\phi}$.
%
The above expressions are made rigorous by introducing suitable infrared regularization, making the Foruier modes countable discrete, and ultraviolet cutoff, discarding Foruier mode above some scale $\Lambda$ and making the function space finite-dimensional.
%
