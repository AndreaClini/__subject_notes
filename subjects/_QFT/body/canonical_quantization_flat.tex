...
\begin{wrapfigure}[3]{r}{0.1\textwidth} % 14 = lines to wrap; adjust as needed
  \centering
  %\vspace{-0.5\baselineskip}            % optional: nudge up
...
%
Consider Minkowsky space with inertial coordinates $x^{\mu}=(t,\mathbf{x})$, and a scalar quantum field $\hat{\phi}(t_0,\mathbf{x})$ on it with free Hamiltonian $H_{\phi}$.
The freely time-evolved field solves the Heisenberg equation $ih\partial \phi(t,\mathbf{x}) = [\phi(t,\mathbf{x}), H_{\phi}]$ wrt the inertial time $t$ (and states similarly solve the Schrödinger equation wrt time $t$), and its mode decomposition wrt such inertial coordinates is thus given by \eqref{eq:time_evolution_free_field}.

The detector is a localized quantum system with wordline $x^\mu(\tau)$ parametrized by its proper time $\tau$.
For simplicity we consider the detector in 1st quantization and the field in 2nd quantization, with total Hibert space $\mathcal{H}_{\phi} \otimes \mathcal{H}_{\mathrm{det}}$.
The detector solves the Schrödinger and Heisenberg equations wrt its own proper time $\tau$. 
...
\end{align}
Note we use two different times for the evolution operators, as it should be, since the parameter for the generator of time translations for the detector (resp. field) is $\tau$ (resp. $t=x^0$), but we ultimately have to evolve up to the same time $\tau$ or $x^0(\tau)$.
