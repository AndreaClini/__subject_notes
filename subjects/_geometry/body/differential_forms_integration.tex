...
%================================================
%========================================================
\chapter{Differential forms \& integration} 
%=========================================================
%===================================================================


\begin{mytheorem}[Antisymmetrization and wedge product of differential forms]
The wedge product of dufferential forms is defined as
{\small\begin{align}
\begin{aligned}
    \alpha\wedge\beta &= A\Big(\alpha\otimes\beta\Big)
    =
    \tfrac{1}{\alpha!\beta!}\sum_{\sigma\in \mathcal{S}_{\alpha+\beta}}(-1)^{\sigma}(\alpha\otimes\beta)\circ \sigma
    =
    \tfrac{1}{\binom{\alpha+\beta}{\beta}}\sum_{\rho\in\mathcal{R}^{\alpha+\beta}_\alpha}(-1)(\alpha\otimes\beta)\circ\rho\,,
\end{aligned}
\end{align}}
where $\mathcal{R}^{\alpha+\beta}_{\beta}$ denotes the \emph{ordered} splittings of indices $\{i_1,\dots,i_{\alpha+\beta}\}$ into two subsets of $\alpha$ and $\beta$ indices each, which is a subset of the set of permutations $\mathcal{S}_{\alpha+\beta}$.

This definition makes the wedge product distributive and associative.
In particular it makes the set of differential forms a \textbf{graded-commutative algebra}
\begin{align}
    \Lambda(M)=\bigotimes_{k=0}^n\Lambda^k(M), \qquad \alpha\wedge\beta=(-1)^{\alpha\beta}\beta\wedge\alpha\,.
\end{align}
\end{mytheorem}
...
We will instead use greek letters $\sigma_1, \dots, \sigma_k \subseteq \{1, \dots, n\}$ to denote unordered indices.
A $k$-form $\omega \in \Lambda^k (M)$ can be equivalently written  as
{\small\begin{align}\label{eq:indices_bookkeeping_differential_forms}
\begin{aligned}
...
%-----------------------------------------------------
%========================================================
\section{Derivatives on differential forms}
%=========================================================
%-----------------------------------------------------
...
The Lie derivative is a well-behaved derivative of degree 0 on differential forms, defined as the restriction of the usual Lie derivative on tensors.
...
The inner derivative is the unique \emph{anti}derivative of degree $-1$ on differential forms such that
{\small\begin{align}
\begin{aligned}
    i_Y f = 0, \qquad    i_Y df = df(Y), \qquad \forall f\in \Lambda^0(M)\,.
\end{aligned}
\end{align}}
It is extended to all differential forms by the Leibniz rule in a basis
{\small\begin{align}\begin{aligned}
    i_Y(f dx^{i_1} \wedge \cdots \wedge dx^{i_k}) = & (i_Y f) dx^{i_1} \wedge \cdots \wedge dx^{i_k} 
    + f (i_Y dx^{i_1}) \wedge dx^{i_2} \wedge \cdots \wedge dx^{i_k} + \dots
    \\
    &+(-1)^{j-1} f \,\,dx^{i_1} \wedge \cdots \wedge (i_Y dx^{i_j}) \wedge \cdots \wedge dx^{i_k} +\dots
\end{aligned}
\end{align}}
Equivalently, the inner derivative can be defined explictly as the contraction along a vector field $X$
\begin{equation}
    i_X: \Lambda^k (M) \to \Lambda^{k-1} (M)\quad  \mid\quad  i_X \omega := \omega (X, \dots)\,.
\end{equation}
In a basis of $\Lambda^{k-1}(M)$, that is summing over \emph{ordered} indices $i_1< \dots <i_{k-1}$, but over \emph{any} value $\sigma\in \{1\dots n\}$, we have
...
\end{aligned}
\end{align}}
The inner derivative enjoys
\begin{itemize}
    \item $i_X i_Y + i_Y i_X = 0$ for any vector fields $X,Y$;
    \item in particular $i_X i_X = 0$ for any vector field $X$;
\end{itemize}
%
\end{mytheorem}
...
The exterior derivative is an \emph{anti}derivative of degree 1 on differential forms, defined as the unique map
\begin{equation}
...
\begin{mytheorem}[Cartan formula \& commutators of derivatives]
This fundamental result links the Lie derivative $\mathcal{L}_X$, the exterior derivative $d$ and the inner derivative $i_X$, namely, for any differential form $\omega$, 
\begin{equation}
    \mathcal{L}_X \omega = i_X d \omega + d (i_X \omega).
\end{equation}
Notably that the right-hand side is defined only for differential forms, while the left-hand side makes sense for any tensor.
Since differential forms include functions (0-forms) and differentials (1-forms), we can use Cartan formula to \emph{define} the Lie derivative and then extend it to all tensors using linearity and the Leibniz rule.

Moreover, we have the following commutators of derivatives on differential forms, easily proved using Cartan formula and the Jacobi identity,
\begin{itemize}
    \item $[L_X, d]=0$;
    \item $[L_X, i_Y] = i_{[X,Y]}$;
    \item $[L_X, L_Y] = L_{[X,Y]}$.
...
\begin{mytheorem}[Curl, pseudo-vectors and Hodge duals]
Consider a vector field $V=(V_x,V_y,V_z)$ on a 3-dimensional manifold $M^{(3)}$.
Its curl is naively defined as the pseudo-vector field
\begin{align}
    \nabla \times V= (\partial_y V_z - \partial_z V_y, \partial_z V_x - \partial_x V_z, \partial_x V_y - \partial_y V_x).
\end{align}
However, the natural way is to define the curl as the 2-form obtained by taking the exterior derivative of the 1-form associated to $V$ via the metric, i.e. $V^\flat = g(V,\cdot)= V_x dx + V_y dy + V_z dz$, that is
{\small\begin{align}
    \nabla\times V:=dV^\flat = (\partial_y V_z - \partial_z V_y) dy\wedge dz + (\partial_z V_x - \partial_x V_z) dz\wedge dx + (\partial_x V_y - \partial_y V_x) dx\wedge dy.
\end{align}}
This definition also makes it obvious why we get a pseudo-vector: the curl is naturally a 2-form, and we get a 1-form/vector only upon taking its Hodge dual in a 3D manifold.
\end{mytheorem}
...
\end{equation}
Indeed, since $\phi^* (\omega) \in \Lambda^k (U)$ is a $k$-form on $\mathbb{R}^k$, it can be written as $f(x) dx^1 \wedge \cdots \wedge dx^k$ for some function $f: U \to \mathbb{R}$ and then integrated as usual.

Note that the \textbf{orientation of $S$ is necessary} to have a well-defined integral, since changing the orientation changes the sign of the integral, while \textbf{orientability of $M$ is not necessary}.
\end{mytheorem}
...
Indeed, if we iterate Stokes theorem, we get
{\small\begin{align}
\begin{aligned}
    \int_M \underbrace{d^2\!\omega}_{=0}=\int_{\partial M} d\omega = \int_{\underbrace{\partial^2 M}_{=0}} \omega = 0\,.
\end{aligned}
\end{align}}
...
Given a 3D \emph{Riemannian} manifold $(M^{(3)},g)$ and a vector field $V$, define the 1-from $V^\flat=g(V,\cdot)= V_\mu dx^{\mu}$, that is lower an index with the metric $V_\mu=g_{\mu \nu} V^{\nu}$.
Define the curl as the 2-from obained by taking the exterior derivative
\begin{equation}
    \nabla\times V := d (V^\flat) = d \big(V_{\mu} dx^{\mu}\big).
\end{equation}
For any oriented surface $S\subset M$ with boundary $\partial S$, the general Stokes theorem then gives
\begin{equation}
    \int_S \nabla\times V =\int_S d (V^\flat) = \int_{\partial S} V^\flat\,.
\end{equation}
The definition of curl and the integral relation reduce to the familiar notion of curl and Stokes' theorem in $\mathbb{R}^3$ with the flat Euclidean metric.
\end{mytheorem}


\begin{mytheorem}[Complex integration \& Gauss--Green theorem]
Consider the complex plane as the oriented Riemannian manifold $(\mathbb{R}^2,g)$ with the standard Euclidean metric and orientation $dx\wedge dy$.
Let $D\subset\mathbb{C}$ be a compact domain with positively oriented boundary and let $f=u+iv$ be holomorphic on a neighborhood of $D$.
The real and imaginary parts of the complex contour integral are ordinary integrals of real 1-forms:
\begin{align}
    \int_{\partial D} f(z)\,dz = \int_{\partial D} (u+iv)(dx+idy)
    = \int_{\partial D} (u\,dx-v\,dy)
    + i\int_{\partial D} (v\,dx+u\,dy).
\end{align}
Applying the Gauss--Green theorem, i.e.\ Stokes theorem in two dimensions, gives
\begin{align}
    \int_{\partial D} f(z)\,dz
    &= \int_D d(f\,dz) 
    = \int_D
    \left[-(\partial_y u+\partial_x v)
    +i(\partial_x u-\partial_y v)\right]dx\wedge dy
    =0,
\end{align}
where the last equality follows from the Cauchy--Riemann equations
$\partial_xu=\partial_yv$ and $\partial_yu=-\partial_xv$.
Thus Cauchy's integral theorem is precisely the Gauss--Green theorem applied to the complex 1-form $f(z)\,dz$.
\end{mytheorem}
...
\end{equation}
Note that this definition is independent of the notion of metric, we only need a volume form $\Omega$.
...
\begin{mytheorem}[Metric volume form convention]
Given a pseudoriemannian manifold \((M^{(n)}, g)\), the metric volume form in $n$ dimension is given by 
\begin{align}\begin{aligned}
    \Omega_g:&=\sqrt{g}\,\,dx^1\wedge dx^2\wedge...\wedge dx^n \\
    &= \sqrt{g} \epsilon_{\mu_1\mu_2...\mu_n}dx^{\mu_1}\otimes dx^{\mu_2}\otimes...\otimes dx^{\mu_n}
    \\
    &= \tfrac{1}{(n+1)!}\sqrt{g} \epsilon_{\mu_1\mu_2...\mu_n}dx^{\mu_1}\wedge dx^{\mu_2}\wedge...\wedge dx^{\mu_n},
\end{aligned}
\end{align}
where repeated \emph{greek} indices are summed over, we denote {\small$\sqrt{g}:=\sqrt{|\det g|}$}, and we denote the Levi-Civita totally antisymmetric \emph{symbols}, not tensors, by
\begin{align}
    \epsilon_{12...n}:=1, \qquad \epsilon^{12...n}:=1\,.
\end{align}
The corresponding tensorial quantities/coefficients are defined as
\begin{align}\begin{aligned}
    &\varepsilon_{\mu_1\mu_2...\mu_n}:=\sqrt{g}\,\epsilon_{\mu_1\mu_2...\mu_n},
        \\
    &\varepsilon^{\mu_1\mu_2...\mu_n}:= g^{\mu_1\nu_1}g^{\mu_2\nu_2}...g^{\mu_n\nu_n}\varepsilon_{\nu_1\nu_2...\nu_n}
    {\overset{!}{=}} \det(g^{\alpha\beta}) \sqrt{g}\,\epsilon_{\mu_1\mu_2...\mu_n}
    =\tfrac{\operatorname{sign(\det g)}}{\sqrt{g}}\,\epsilon^{\mu_1\mu_2...\mu_n}.
\end{aligned}
\end{align}
\end{mytheorem}
...
%
The Hodge dual maps a $k$-form $\theta$ to an $(n-k)$-form by transforming $\theta$ into a antisymmetric $(k,0)$-tensor with the metric, then feeding it to the \emph{first} $k$ slots of the volume form, and finally dividing by a symmetry factor $k!$ to avoid overcounting.
Equivalently, it contract the $k$-form with the first $k$ indices of the volume form using the metric and divide by $k!$.
Note we adopt the \textbf{first index convention} for the volume form, that is we feed the first $k$ slots of $\Omega_g$ with $\theta^{\#}$ and leave the last $n-k$ slots free.

In formulas, denoting \(\theta^{\#}\) the tensor of rank $(k,0)$ obtained by raising all indices of $\theta$ with the metric, the Hodge dual is defined as
\begin{align}
    \star: \Lambda^k (M) &\to \Lambda^{n-k} (M) \quad \mid \quad \theta \mapsto \tfrac{1}{k!}\Omega_g(\theta^{\#}, \cdots)\,.
\end{align}
The symmetry factor $1/k!$ is inserted to avoid overcounting, since both $\theta$ and $\Omega$ are totally antisymmetric in all indices, so that  $\star\star = (-1)^{k(n-k)+s}$ where $s$ is the signature of the metric.
...
\begin{itemize}
    ...
    \begin{align}
        ...
        =\tfrac{\alpha!}{(\alpha+\beta)!}\,\,\iota_{\beta^\sharp}(\star\alpha)
        =(-1)^{\alpha\beta}\,\,\iota_{\alpha^\sharp}(\star\beta).
    \end{align}
    The inner derivative $i_{\beta^{\#}}$ of a $j$-form with respect to the  $(\ell,0)$-tensor $\beta^\sharp$ means contracting the first $\ell$ slots of the form with the tensor, and gives zero if $\ell>j$. That is when $\alpha+\beta>n$.
    %
\end{itemize}
\end{mytheorem}
\begin{proof}
...
Similarly we prove the inner product property \todoinline{finish and check}
We have
{\small\begin{align}
    \star\theta &= \tfrac{1}{k!}\sqrt{g}\, \epsilon_{\beta_1\dots\beta_k\mu_{k+1}\dots\mu_n} g^{\nu_1\beta_1}\dots g^{\nu_k\beta_k} \theta_{\nu_1\dots\nu_k} dx^{\mu_{k+1}}\otimes\dots\otimes dx^{\mu_n}
    \\
    &= \tfrac{1}{k!}\tfrac{1}{(n-k)!} \epsilon_{\beta_1\dots\beta_k\mu_{k+1}\dots\mu_n} g^{\nu_1\beta_1}\dots g^{\nu_k\beta_k} \theta_{\nu_1\dots\nu_k} dx^{\mu_{k+1}}\wedge\dots\wedge dx^{\mu_n}
\end{align}}
Then 
{\small\begin{align}
    \eta\wedge\star\theta
    &= \Big(\tfrac{1}{k!}\eta_{\alpha_1\dots\alpha_k} dx^{\alpha_1}\dots\wedge dx^{\alpha_k}\Big)\wedge 
    \Big(\tfrac{1}{k!}\tfrac{1}{(n-k)!}\sqrt{g} \epsilon_{\beta_1\dots\beta_k\mu_{k+1}\dots\mu_n} g^{\nu_1\beta_1}\dots g^{\nu_k\beta_k} \theta_{\nu_1\dots\nu_k} dx^{\mu_{k+1}}\wedge\dots\wedge dx^{\mu_n}\Big)
    \\
    &=
    \tfrac{1}{(k!)^2}\tfrac{1}{(n-k)!}
    \eta_{\alpha_1\dots\alpha_k}
    \epsilon_{\beta_1\dots\beta_k\mu_{k+1}\dots\mu_n} g^{\nu_1\beta_1}\dots g^{\nu_k\beta_k} \theta_{\nu_1\dots\nu_k} \,\,
    dx^{\alpha_1}\dots\wedge dx^{\alpha_k}\wedge
    dx^{\mu_{k+1}}\wedge\dots\wedge dx^{\mu_n}\,.
\end{align}}
The symbol $\epsilon_{\beta_1\dots\beta_k\mu_{k+1}\dots\mu_n}$ and the wedge product in the second line vanish by antisymmetry unless the set of indices obey respectively
{\small\begin{align}
    \{\beta_1\dots\beta_k\}\cap\{\mu_{k+1}\dots\mu_n\} = \emptyset, \qquad
    \{\alpha_1\dots\alpha_k\}\cap\{\mu_{k+1}\dots\mu_n\} = \emptyset.
\end{align}}
This forces the two sets to be equal: $\{\alpha_1\dots\alpha_k\} = \{\beta_1\dots\beta_k\}$.
Then, using
{\small
\begin{align}
    dx^{\alpha_1}\wedge\cdots\wedge dx^{\alpha_k}
    \wedge dx^{\mu_{k+1}}\wedge\cdots\wedge dx^{\mu_n}
    &=
    \epsilon^{\alpha_1\dots\alpha_k\mu_{k+1}\dots\mu_n}
    \,dx^1\wedge\cdots\wedge dx^n
\end{align}}
and the contraction identity
{\small
\begin{align}
    \epsilon_{\beta_1\dots\beta_k\mu_{k+1}\dots\mu_n}
    \epsilon^{\alpha_1\dots\alpha_k\mu_{k+1}\dots\mu_n}
    &=
    (n-k)!\,
    \delta^{\alpha_1\dots\alpha_k}_{\beta_1\dots\beta_k},
\end{align}}
where
{\small\begin{align}
    \delta^{\alpha_1\dots\alpha_k}_{\beta_1\dots\beta_k}
    :=
    \sum_{\sigma\in\mathcal S_k}
    \operatorname{sgn}(\sigma)\,
    \delta^{\alpha_1}_{\beta_{\sigma(1)}}\cdots
    \delta^{\alpha_k}_{\beta_{\sigma(k)}}
\end{align}}
is the generalized Kronecker symbol, we obtain
{\small
\begin{align}
    \eta\wedge\star\theta
    &=
    ...
    dx^1\wedge\cdots\wedge dx^n .
\end{align}}
Since \(\eta\) is totally antisymmetric $\eta_{\alpha_1\dots\alpha_k}
\delta^{\alpha_1\dots\alpha_k}_{\beta_1\dots\beta_k}=
k!\,\eta_{\beta_1\dots\beta_k}$.
{\small
\begin{align}
    \eta\wedge\star\theta
    &=
    \frac{1}{k!}\,
    \eta_{\beta_1\dots\beta_k}\,
    g^{\nu_1\beta_1}\cdots g^{\nu_k\beta_k}
    \theta_{\nu_1\dots\nu_k}\,
    ...
    &=
    \frac{1}{k!}\,
    \eta_{\beta_1\dots\beta_k}\theta^{\beta_1\dots\beta_k}
    \,\Omega_g
    =
    \langle\eta,\theta\rangle_g\,\Omega_g.
\end{align}}
Finally, the symmetry of the inverse metric gives
{\small
\begin{align}
    \eta_{\beta_1\dots\beta_k}\theta^{\beta_1\dots\beta_k}
    =
    \theta_{\beta_1\dots\beta_k}\eta^{\beta_1\dots\beta_k},
\end{align}}
and hence
\[
    \eta\wedge\star\theta
...
%
The codifferential \(\delta\) is defined as the adjoint of the exterior derivative \(d\) wrt the Hodge scalar product\footnote{Beware that $\delta$ is sometimes defined as \emph{minus} the adjoint of $d$ to make the D'Alambertian $\Box =\delta d+d\delta$ positive definite rather than negative in the Riemannian case. It is just a convention.}
\begin{align}\label{eq:definition_codifferential}
...
\begin{align}
    ...
    \\
    &=
    \underbrace{(-1)^{|\beta|+1+(|\alpha|-1)(n-|\alpha|+1)+s}}_{=(-1)^{n(|\alpha|-1)+s+1}} \int_M  \beta \wedge (\star\star d\star \alpha)
    \equiv
...
\end{align}
The codifferential is a linear map of degree \((0,-1)\), but is not an (anti)-derivative since it \textbf{fails the Leibniz rule} 
\begin{equation}
    \delta: \Lambda^\bullet (M) \to \Lambda^{\bullet-1} (M)\,, \quad \qquad \delta(\alpha \wedge \beta) \neq \delta \alpha \wedge \beta + (-1)^{|\alpha|} \alpha \wedge \delta \beta.
\end{equation}
Moreover, while the exterior derivative is completely independent of it, the \textbf{codifferential does encode information about the metric} since it's defined via the Hodge dual \eqref{eq:definition_codifferential}.
\smallskip
...
\begin{wrapfigure}[9]{r}{0.34\textwidth}
  \centering
  \vspace{-\intextsep}
  \includegraphics[width=\linewidth]{images/seismic_waves.pdf}
...
\begin{samepage}
\begin{center}
  \includegraphics[width=0.4\textwidth]{images/hodge_spectrum_symmetries.pdf}
\end{center}
\end{samepage}
...
%-----------------------------------------------------
%========================================================
\section{Closed vs exact forms \& de Rham cohomology}
%=========================================================
%-----------------------------------------------------
...
\begin{mytheorem}[Cohomology theories, naturality \& $K$-theory \questioninline{finish}]
Let $\mathbf{Man}$ denote the category whose objects are smooth manifolds and whose morphisms are smooth maps, and let $\mathbf{GrAb}$ denote the category of graded abelian groups.
A cohomology theory is a contravariant functor
\begin{equation}
    H^\bullet:\mathbf{Man}^{\mathrm{op}}\longrightarrow\mathbf{GrAb},
    \qquad
    M\longmapsto H^\bullet(M),
    \qquad
    \big(f:M\to N\big)\longmapsto
    \big(f^*:H^\bullet(N)\to H^\bullet(M)\big)\,,
\end{equation}
where functoriality means
\begin{equation}
    \operatorname{id}_M^*=\operatorname{id}_{H^\bullet(M)},
    \qquad
    (g\circ f)^*=f^*\circ g^*.
\end{equation}
In particular, a diffeomorphism induces an isomorphism of cohomology groups.
Hence if $H^\bullet(M)\not\cong H^\bullet(N)$, then $M$ and $N$ cannot be diffeomorphic, although the converse need not hold.

A \textbf{natural transformation} $\eta:H^\bullet\Rightarrow K^\bullet$ between two cohomology theories is a family of homomorphisms
$\eta_M:H^\bullet(M)\to K^\bullet(M)$ compatible with every smooth map $f:M\to N$, namely
\begin{equation}
\begin{tikzcd}[
    row sep=large,
    column sep=large,
...
\end{tikzcd}
\end{equation}
De Rham cohomology is the basic example, with the pullback of differential forms inducing the maps $f^*$.

Topological $K$-theory is another generalized cohomology theory.
The isomorphism classes of complex vector bundles over $M$ form a commutative monoid under the Whitney sum $\oplus$.
Its group completion defines
\begin{equation}
    K^0(M)
    :=\operatorname{Gr}\!\left(\operatorname{Vect}_{\mathbb C}(M),\oplus\right),
\end{equation}
whose elements are formal differences $[E]-[F]$, subject for example to
\begin{equation}
    [E\oplus G]-[F\oplus G]=[E]-[F].
\end{equation}
Pullback of vector bundles makes $K^0$ contravariantly functorial:
\begin{equation}
    f^*\big([E]-[F]\big)=[f^*E]-[f^*F].
\end{equation}
On a contractible manifold every fixed-rank vector bundle is trivial; consequently
$K^0(M)\cong\mathbb{Z}$ records only the virtual rank, while the reduced group
$\widetilde K^0(M)$ vanishes.
\end{mytheorem}
