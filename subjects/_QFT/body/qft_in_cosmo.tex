...
%     inner sep=1.5pt
%   },
%   arr/.style={->, thick, draw=orange!85!black},
%   equiv/.style={<->, thick, draw=orange!85!black}
% ]
% \matrix (m) [matrix of nodes, row sep=16mm, column sep=36mm, nodes={block}] {
...
% };
% % Top row
% \draw[equiv] (srH) -- node[lab, above] {Legendre transform\\equivalence} (srL);
% % Middle row
% \draw[arr]  (srL) -- node[lab, right] {allow curvature\\\& make covariant} (grLc);
% \draw[equiv] (grHc) -- node[lab, above] {Legendre transform\\equivalence} (grLc);
% % Bottom row
% \draw[arr] (grHc) -- node[lab, left] {canonical\\quantization} (grHq);
% \draw[arr] (grLc) -- node[lab, right] {path\\integral} (grLq);
% \draw[<->, thick] (grHq) -- node[
%   font=\scriptsize, align=center,
%   fill=white, fill opacity=0.9, text opacity=1,
%   inner sep=1.5pt, below
% ]{same Dyson--Schwinger eqs\\ for correlators} (grLq);
% \end{tikzpicture}
%
...
If the two foliations are equivalent i.e. fixed-time leaves are mapped into each other by the change of coordinates, \todoinline{Not sure what if $\tilde{t}=\tilde{t}(t,\mathbf{x})$ i.e. leaves don't map into each other?}
\begin{align}
