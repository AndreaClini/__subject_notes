...
\begin{mytheorem}[A first (failed) attempt: relativistic quantum mechanics.\todotag{Finish. See Matheus 1.3}]
%
A first attempt to address the limitations of Quantum Mechanics was aimed to combine the principles of quantum mechanics with special relativity, namely
\begin{center}
\begin{tikzpicture}[
  font=\small,
  >=Stealth,
...
\draw[arr] (qm) -- (rqm);
\draw[arr] (sr) -- (rqm);
\node[font=\scriptsize, text=red, below=2mm of rqm] {Does it work?};
\end{tikzpicture}
\end{center}
...
While this approach successfully incorporated relativistic effects and predicted phenomena such as antimatter, it still faced significant challenges, including the interpretation of negative energy solutions and the inability to account for particle creation and annihilation processes.

For example, both the Schr\"odinger and Klein-Gordon equations imply a continuity equation $\partial_t\rho=-\nabla\cdot \mathbf{j}$, identifying a probability density $\rho$ and current $\mathbf{j}$, 
\begin{align}
...
In the Klein-Gordon case, recalling solutions take the form $\phi=N e^{-iEt+i\mathbf{k}\cdot \mathbf{x}}$ with $E^2 = |\mathbf{k}|^2 + m^2$, we find $\rho = 2E|N|^2$, which is not positive definite since $E$ can be negative.

Crucially, the problem of negative energies is not due to the second-order equation, inevitably involving square roots.
Indeed Dirac came up with his equation, first-order in time and space derivatives to avoid this problem,
\begin{align}
...
In fact, the two solutions are positive $u(k)e^{-iEt+i\mathbf{k}\cdot \mathbf{x}}$ and negative-energy spinors $v(k)e^{iEt+i\mathbf{k}\cdot \mathbf{x}}$ with $E^2 = |\mathbf{k}|^2 + m^2$, which he interpreted as antiparticles.

Regardless of negative energies, the interpretation of the wave function as a probability amplitude for a single particle is still problematic, since it cannot account for processes where particles are created or destroyed, which are common in high-energy physics.
...
Moreover, any the above interpretation of the wave function as a probability amplitude for finding a single particle at a given position and time is also problematic since it violates causality.
Indeed, for an hamiltonian $H = \sqrt{p^2 + m^2}$, the propagator $\langle \mathbf{x} | e^{-iHt} | \mathbf{y} \rangle$ is non-zero for spacelike separations $|\mathbf{x}-\mathbf{y}| > t$, thus allowing for superluminal propagation of signals.
Indeed we find nonzero amplitude 
...
The above issues highlighted the need for a more comprehensive framework, ultimately leading to the development of Quantum Field Theory, which treats fields as the fundamental entities and allows for the creation and annihilation of particles. 
...
\begin{itemize}
    ...
    \item the problem of causality is resolved since there is no point in asking if the field is at a position $\mathbf{x}$ at a given time $t$, since the field is `everywhere' at all times, and thus the question is simply ill-posed.
    In fact, even for fields we would find that the analogus fomula to \eqref{eq:nonvanishing_superluminal_propagation_apmplitude} for the propagator of the field would be non-zero for spacelike separations, albeit exponentially suppressed.
    The proper question is instead whether the state of the field at a given spacetime point can influence the state of the field at another spacetime point, i.e. a question about causal correltors, which will be shown to vanish for spacelike separations.
\end{itemize}
\end{mytheorem}
...
\begin{mytheorem}[How to make the Schr\"odinger equation relativistic? Klein-Gordon is \emph{always} the answer. \questiontag{Polish}]
%
...
\begin{itemize}
    ...
    More physically, Dirac proposed energy i.e. the Hamiltonian could be expressed linearly in momentum and mass
    \begin{align}
        H= a_i p_i + b m .
    \end{align}
