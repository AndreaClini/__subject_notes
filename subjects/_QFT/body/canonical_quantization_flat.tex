...
\begin{mytheorem}[Solving Heisenberg equation: time evolution of fields\questiontag{Check}]
%
...
In general the probability densities for field configurations in a state $|\Psi\rangle$ evolve non-trivially in time.
However, even though it is not immediately obvious from the Heisenberg evolution above, if the state $|\Psi\rangle$ is an eigenstate of the Hamiltonian such as the tensors states $|\{n_k\}_k\rangle$, the rime evolution of the state $|\Psi\rangle$ in the Schrödinger picture is a simple phase, and thus the probability densities \eqref{eq:probability_density_field_configuration} remains constant in time since the state.
\end{mytheorem}
