...
{\color{red}
More generally, we can plug in the \emph{arbitrary} expression for the torsion $T_{ij}^k$ and formula \eqref{eq:koszul_formula_cohordinates} for the Christoffel symbols to express the Riemann tensor purely in terms of metric, structure constants and torsion.
%
If we start from a coordinate system $x^\mu$ and we pass to a orthonormal tetrad $\{e_a = B_a^\mu \partial_\mu,\, \theta^a = (B^{-1})^a_\mu dx^\mu\}_a$, we can express everything in terms of the torsion, and the matrix $B$ and its derivatives.
}\todotag{Finish}
\end{mytheorem}
