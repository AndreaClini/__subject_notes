...
\end{align}}
In this case, inverting the relation by inserting a completeness in the $\{\tilde{u}_m, \tilde{u}_m^*\}_m$ basis and recalling the property $\langle x,y\rangle^* = \langle x^*,y^*\rangle$ of the symplectic hermitian product, we find 
\begin{align}\label{eq:inverse_transformation_bogoliubov_modes}
    u_i= \sum_m \alpha_{mi}^* \tilde{u}_m - \beta_{mi} \tilde{u}_m^*.
\end{align}
...
{\small
\begin{align}
    \tilde{u}&=\alpha u + \beta u^* \,\,\Rightarrow\,\, \hat{\phi} = \tilde{a}^T \tilde{u} + c.c. = \tilde{a}^T (\alpha u + \beta u^*) + c.c. = \big(\alpha^T\tilde{a}+\beta^\dagger \tilde{a}^\dagger\big)^T u + c.c. \,\,\Rightarrow\,\,  a = \alpha^T \tilde{a} + \beta^\dagger \tilde{a}^\dagger\\[3pt]
    u&= \alpha^\dagger \tilde{u}-\beta^T\tilde{u}^*\,\,\Rightarrow\,\, \hat{\phi} = a^T u + c.c. = a^T (\alpha^\dagger \tilde{u}-\beta^T\tilde{u}^*) + c.c. = \big(\alpha^* a - \beta^* a^\dagger\big)^T \tilde{u} + c.c. \,\,\Rightarrow\,\,  \tilde{a} = \alpha^* a - \beta^* a^\dagger.
\end{align}}
Or again\todotag{redo it}
{\small
\begin{align}
    \begin{pmatrix}
        u_j\\
    ...
    \begin{pmatrix}
        \tilde{u}_m\\
        \tilde{u}_m^*
    \end{pmatrix}\,\,\Rightarrow\,\, \hat{\phi} = \begin{pmatrix}
        a_j & a_j^\dagger
    \end{pmatrix} \begin{pmatrix}
        u_j\\
        u_j^*
...
We now relate Fock states of the two theories defined by ladder operators.
The vacuum of the first theory will be expressible as a suitable superposition of multi-particle states of the second theory $|0\rangle =f\big(\{\tilde{a}^\dagger_n\}_n\big)|\tilde{0}\rangle$ for a suitable function $f$ of creation operators.\footnote{Note $f$ will indeed be independent of annihilation operators $\tilde{a}$ since they annihilate the vacuum $|\tilde{0}\rangle$ and we may commute them to the right upon changing the form of the $f$.}
...
\begin{align}
     0=a_j|0\rangle = (\sum_m\alpha_{mj} \tilde{a}_m + \beta_{mj}^* \tilde{a}_m^\dagger) |0\rangle
     = (\sum_m\alpha_{mj} \tilde{a}_m + \beta_{mj}^* \tilde{a}_m^\dagger) f\big(\{\tilde{a}^\dagger_n\}_n\big)|\tilde{0}\rangle = \sum_m \Big(\alpha_{mj} \frac{\partial f}{\partial \tilde{a}_m^\dagger} + \beta_{mj}^* \tilde{a}_m^\dagger f\Big)|\tilde{0}\rangle.
\end{align}
...
\begin{align}
    ...
    \,\,\Rightarrow\,\, f\big(\tilde{a})\propto e^{-\frac{1}{2} \tilde{a}^\dagger (\alpha^T)^{-1}\beta^\dagger \tilde{a}^\dagger}.
\end{align}
where we used that the above matrix is already symmetric since we have \todotag{redo}
\begin{align}
    \alpha\beta^T-\beta\alpha^T=0\,\Rightarrow\, (\beta^T)^{-1}\alpha^{-1} = (\alpha^T)^{-1}\beta^{-1}, \quad \alpha\alpha^\dagger - \beta \beta^\dagger = \mathbb{I} \,\,\Rightarrow\,\, \alpha \alpha^\dagger = \mathbb{I}+\beta\beta^dagger\\
    (\beta^T)^{-1}\alpha^\dagger = {\color{blue}(\beta^T)^{-1}\alpha^{-1}}(\alpha \alpha^\dagger) = {\color{blue}(\alpha^T)^{-1}\beta^{-1}}\big(\mathbb{I}+\beta\beta^\dagger) = (\alpha^T)^{-1}\beta^{-1} + (\alpha^T)^{-1}\beta^\dagger 
\end{align}
Normalizing the vacuum states we find\todotag{fix}
{\small
\begin{align}
