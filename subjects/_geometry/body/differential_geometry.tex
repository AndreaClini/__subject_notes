...
\begin{mytheorem}[Inducing connections on arbitrary fiber bundles from the tangent bundle connection \answerinline{OK}]
...
The connection form on the tangent bundle $\omega_{TM}$ must then take values in the representation of the Lie algebra $\mathfrak{g}$ on $T^1_0M$ induced by the action of $G \curvearrowright T^1_0M$, with concrete matrix elements defined in the chosen tetrad $\{e_i\}_i$ as
\begin{align}
...
Indeed, regardless of the specific representation, the connection 1-form on the tangent bundle can be seen as taking values in the \emph{abstract} Lie group $\mathfrak{g}$.
\begin{align}
...
\begin{align}
    \nabla_\cdot f_b &=: \omega_{(F)}(\cdot) f_b =: f_a \otimes \omega^a_{\, b}(\cdot)\,.
\end{align}
...
\end{equation}
We again remark that, as abstract elements of $\mathfrak{g}$, the connection 1-forms on $E_F\to M$ and on $TM\to M$ coincide i.e. $[\omega_{(F)}(\cdot)]\equiv [ \omega_{TM}(\cdot)] \equiv \omega(\cdot)\in \mathfrak{g}$.
\end{mytheorem}
...
\begin{mytheorem}[Curvature 2-form \todotag{Check \& finish}]
...
\begin{mytheorem}[Torsion 2-form in a tetrad frame on the tangent bundle\todotag{Check and finish}]
%
Beware torsion makes sense only for the tangent bundle since only there we have a natural notion of commutator of vector fields...
\end{mytheorem}


\begin{mytheorem}[Riemann curvature tensor in a tetrad frame on the tangent bundle\todotag{Check and finish}]
...
\begin{mytheorem}[Cartan \& Bianchi equations on the tangent bundle \& recovering coordinate formalism\todotag{Check and finish}]
...
\begin{enumerate}
    ...
    \item connection 1-form on arbitrary fiber bundle  MISSING (swap with above point when done)
    \item metric compatibility in arbitrary fiber bundle MISSING
    \item example: connection on spinors MISSING
    \item curvature 2-form on arbitrary fiber bundle MISSING
    \item Cartan structure equations MISSING
    \item Bianchi identities MISSING
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Christoffel symbols in a tetrad frame of the tangent bundle MISSING
    \item torsion 2-form in a tetrad frame on the tangent bundle MISSING
    \item Riemann curvature tensor in a tetrad frame on the tangent bundle MISSING
    \item Cartan \& Bianchi equations on the tangent bundle \& recovering coordinate formalism MISSING
    \item Einstein tensor in a tetrad frame of the tangent bundle MISSING
    \item EM tensor varying wrt tetrad fields MISSING
\end{enumerate}