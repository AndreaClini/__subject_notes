...
In adapted coordinates $x^\mu=(t,\mathbf{x})$ i.e. $\Sigma_t=\{x^0=t\}$, the outward normal and the metric induced on spatial leaves $\Sigma$ obey
{\small
...
\end{align}}
Using \eqref{eq:metric_induced_spatial_leaves_folitation} the conjugate field may also be written in terms of the unit normal and induced metric on spatial leaves as
\begin{align}
...
\end{align}}
where note the inverse of the metric \eqref{eq:inverse_metric_induced_leaves_foliation} induced on spatial slices $g_{|\Sigma}^{ij}$ is negative definite in our mostly minus convention.
...
The pair $(\mathcal{S}_\R, \omega)$ is thus the classical phase space of the theory.

Namely, for $\phi_1, \phi_2 \in \mathcal{S}_\R$, we have the equivalent definitions
\begin{align}
...
\noindent
where $n^\mu$ is the unit outward normal to the spatial slice $\Sigma^{(n)}$ and it is always intended that the $n$-form is pulled back to $\Sigma^{(n)}$ before integration (except for the last case, where it is already done explicitly). 
...
\noindent
The symplectic form is then independent of the choice of Cauchy surface $\Sigma_t$ thanks to current conservation and Gauss theorem
\begin{align}
