...
For any $f\in \mathcal{S}_\C$, any $y=(y^0,\mathbf{y})\in \mathcal{M}$ and any time $t\in\R$ for the leaf $\Sigma_t$ over which to evaluate the symplectic form 
{\small
\begin{align}
...
\end{align}}
Since spacetime is globally hyperbolic, given any time $t\in\R$ and any function $b(\mathbf{y})\in C^\infty_0(\Sigma_t)$ there exists a (unique) solution $f\in \mathcal{S}_\C$ of the Klein--Gordon equation with initial conditions at this time $t$ given by
\begin{align}
...
Fixing also $y^0=t$ and taking the solution of this Cauchy problem, the above condition reads
{\small
\begin{align}
...
\end{align}
As usual it is sufficient to impose the CCR at some fixed time, since time evolution is unitary $U(t,t_0) = \mathcal{T} e^{-\frac{i}{\hbar}\int_{t_0}^t H(s) ds}\in U(\mathcal{F})$ and thus preserves the CCR at all times.
...
We mimick the flat-spacetime strategy and expand the field operator in terms of mode functions i.e. a Bogoliubov basis of solutions to the Klein--Gordon equation.
Inserting the expansion \eqref{eq:expansion_mode_function_phi_classical}-\eqref{eq:expansion_mode_function_pi_classical} into the \emph{classical} Hamiltonian, we recognize a sum (over the mode function indices) of independent harmonic oscillators and proceed to canonical quantization by promoting the coefficients $a_n$ to operators satisfyng the CCR
...
We verify the self-adjointness conditions, the EoM and the CCR are indeed satified with this ansatz.
