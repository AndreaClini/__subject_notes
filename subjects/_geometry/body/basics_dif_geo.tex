...
Given the local triviality request on a frame bundle, i.e.
\begin{align}
    E_F\to M_{\, \big|_{U}}\sim U\times F \to U,
\end{align}
one might wonder about the point of studying them at all\dots
...
\begin{mytheorem}[Whitney smoothness theorem]
If a manifold is topological and admits a $C^{1}$ structure, then it admits a unique $C^{\infty}$ structure.
That is, if a manifold is $C^{1}$-differentiable, then it is automatically smooth i.e. $C^{\infty}$-differentiable.
...
\begin{mytheorem}[Whitney embedding theorem]
