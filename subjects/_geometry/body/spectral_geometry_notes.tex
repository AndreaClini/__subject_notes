...
\begin{itemize}
    ...
    \begin{align}
        \alpha = d\beta\,\Rightarrow\, \delta \alpha \neq 0, \quad \eta = \delta \theta\,\Rightarrow\, d\eta \neq 0.
    \end{align}
\end{itemize} 
...
\begin{tikzpicture}[remember picture,overlay,>=Stealth,shorten >=2pt,shorten <=2pt]
  ...
  \draw[violet,<->] (delp1.south west) to[bend right=19] node[near end,right=2pt] {$\star$} ($(dnp.north east)+(-2.5em,0)$);
\draw[blue,<->]   ($(dp.south east)+(-1.9em,0)$)    to[bend left=19] node[near end,right=2pt] {$\star$} ($(delnp.north west)+(0,0)$);
\end{tikzpicture}
%
