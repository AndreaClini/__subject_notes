...
\end{align}}
The \textbf{scattering operator} is the unitary operator relating the Fock spaces and ladder operators as\todotag{No, it seems it is $\text{arctgh}(Z)$ instead of $Z$}
...
\begin{mytheorem}[Alegbra of Bogoliubov transfromation]
%
After having diagonalized the Bogoliubov transformations we are left with exponentials of the following operators $A=\tfrac12 b^2$, $C=\tfrac12 (b^\dagger)^2$ and $M=\tfrac12 (b^\dagger b+b b^\dagger)=b^\dagger b +\tfrac12$.
This operators satisfy the algebra
\begin{align}
    &[A,C] = M, \quad [M,A] = -2A, \quad [M,C] = 2C.
\end{align} 
The unitary scattering matrix exponential $\exp(\zeta^* C - \zeta A)$ can then be factorized using the BCH formula since the above is essentially the $SU(1,1)$ algebra.\todotag{Finish with chatpgt} 

\end{mytheorem}

\begin{mytheorem}[Takagi identity\todotag{Check \& polish}]
%
If $Z$ is a complex symmetric matrix, there exists a unitary matrix $V\in\mathcal{U}(\mathcal{F})$ and a diagonal matrix $\Xi=\text{diag}(\xi_i)$ such that $Z=V\Xi V^T$ and the real non-negative entries $\xi_i\in\R^+_0$ are precisely the singular values of $Z$ in the SVD decomposition sense, that is $Z= Q \Xi R^\dagger$ for some $Q,R\in \mathcal{U}(\mathcal{F})$.
\end{mytheorem}
\begin{proof}  
