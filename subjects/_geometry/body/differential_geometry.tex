...
%===============================================================================
\subsection{Connection forms \& Cartan-Bianchi structure equations}
%================================================================================


\begin{mytheorem}[Structure constants of a tetrad frame]
Given a frame $\{\theta^i\}_i$ for the cotangent bundle, we define its structure constants $C^i_{\, jk}$ as the coefficients in the expansion
\begin{equation}
...
Given an affine connection $\nabla$ on a fibre bundle $E_F \to M$ with fiber $F$ and a local frame $\{e_i\}_i$, we define the $F$-valued connection 1-forms $\omega_j$ and $\R$-valued connection 1-forms $\omega^i_{\,j}$ as the set of 1-forms satisfying
\begin{equation}
...
\begin{align}
    ...
    \\
    d \Omega^i_{\, j} &= 0\qquad \text{i.e.}\quad d_\nabla \Omega_{\, j} = de_i \wedge \Omega^i_{\, j} \graycancel{= e_i \otimes \underbrace{d \Omega_{\, j}^i}_{=0}}\,.
\end{align}
...
%===============================================================================
\subsection{Curvature tensors in the tetrad formalism}\todotag{To be written}
%================================================================================


\begin{mytheorem}[Christoffel symbols in a tetrad frame\todotag{Check and finish}]
Given a tetrad frame $\{\theta^i,\, e_i\}_i$ on a manifold \(M\) with affine connection \(\nabla\), we define the Christoffel symbols \(\Gamma^i_{\, jk}\) as the coefficients in the expansion 
\begin{equation}
    \nabla_{e_j} e_k =: \Gamma^i_{\, jk} e_i.
\end{equation}
In particular, the connection 1-forms can be expressed in terms of the Christoffel symbols as
\begin{equation}
    \omega^i_{\, j} = \Gamma^i_{\, jk} \theta^k.
\end{equation}
In fact, in case of a metric connection, we may also express in terms of the metric and the structure constants \(C^i_{\, jk}\) of the tetrad frame as
\begin{equation}
    \Gamma_{ijk} = \tfrac{1}{2} \left( C_{ijk} - C_{jki} + C_{kij} \right) + \frac{1}{2} \left( \partial_j g_{ik} + \partial_k g_{ij} - \partial_i g_{jk} \right),
\end{equation}
where indices are lowered and raised with the metric \(g\) and its inverse.
\end{mytheorem}


\begin{mytheorem}
[Riemann curvature tensor in a tetrad frame\todotag{Check and finish}]
