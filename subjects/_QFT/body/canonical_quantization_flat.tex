...
\end{align}
The operators $e^{\alpha a^\dagger - \alpha^* a}$ are unitary since $(\alpha a^\dagger - \alpha^* a)^\dagger = -(\alpha a^\dagger - \alpha^* a)$, and the normalized coherent state is then
\begin{align}
...
This identification is indeed justified only on states with a definite number of particles, like Fock states $|n\rangle$ or superpositions thereof.
...
%
Let us consider a 1D harmonic oscillator driven by time-dependent forces $J(t)$ and $K(t)$ acting on position and momentum respectively
\begin{align}
    H = \frac{1}{2m} p^2 + \frac{m\omega^2}{2} q^2 - J(t) q - K(t) p.
\end{align}
We define ladder operators to diagonalize the free hamiltonian and get
\begin{align}
    a &= \frac{1}{\sqrt{2}}\left(\sqrt{\frac{m\omega}{\hbar}}\, q + i \frac{1}{\sqrt{m \omega \hbar}}\, p\right), \quad [a, a^\dagger] = 1,\\
    H &= \hbar \omega \left(a^\dagger a + \tfrac{1}{2}\right) + \sqrt{\frac{\hbar}{2 m \omega}} J (a + a^\dagger) - i \sqrt{\frac{m \omega \hbar}{2}} K (a - a^\dagger).
\end{align}
If $J,\,K$ are independent of the canonical variables, the equations of motion for $a$ read
\begin{align}%\label{eq:1d_oscillator_ladder_operator_time_evolution}
    &i\hbar\,\partial_t a= [a, H] = \hbar \omega a + \sqrt{\frac{\hbar}{2 m \omega}} J + i \sqrt{\frac{m \omega \hbar}{2}} K\\
    &\Longrightarrow \quad a(t) = a(t_0) e^{-i\omega (t-t_0)} - \frac{i}{\hbar} \int_{t_0}^t dt' \left(\sqrt{\tfrac{\hbar}{2 m \omega}} J(t') + i \sqrt{\tfrac{m \omega \hbar}{2}} K(t')\right) e^{-i\omega (t-t')}.
\end{align}
