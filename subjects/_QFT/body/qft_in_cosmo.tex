...
\begin{align}
    \pi_\chi = \frac{\delta L}{\delta \dot{\chi}} = \dot{\chi} - \mathcal{H} \chi\,, \quad \mathcal{H}_\chi = \pi_\chi \dot{\chi} - L = \frac{1}{2} \pi_\chi^2 + \frac{1}{2} (\nabla \chi)^2 + \frac{1}{2}\Big(m^2 a^2 - \frac{\ddot{a}}{a}\Big) \chi^2 + a^4 V(\chi/a).
\end{align}
The Hamilton equations give back the Euler--Lagrange equations
\begin{align}
    &\frac{\partial \chi}{\partial t} = \frac{\delta H_\chi}{\delta \pi_\chi} = \pi_\chi = \dot{\chi} - \mathcal{H} \chi, \quad \text{with}\quad \partial_t \pi_\chi = \ddot{\chi} - \mathcal{H} \dot{\chi} - \dot{\mathcal{H}} \chi, \\
    &\frac{\partial \pi_\chi}{\partial t} = -\frac{\delta H_\chi}{\delta \chi} = \nabla^2 \chi - \Big(m^2 a^2 - \tfrac{\ddot{a}}{a}\Big) \chi - a^4 \partial_\chi V \quad \Rightarrow \quad \ddot{\chi} + \Big(m^2 a^2 - \tfrac{\ddot{a}}{a}\Big) \chi + a^4 \,\partial_\chi\! V = 0.
\end{align}

Note the conjugate fields and hamiltonian densities are different in the two cases, but of course they ultimately yield the same equations of motion.
Namely we have
\begin{align}
    &\pi_\chi = \frac{\delta L}{\delta \dot{\chi}} = \frac{\delta L}{\delta \dot{\phi}} \frac{\delta \dot{\phi}}{\delta \dot{\chi}} = a^2 \dot{\phi} \frac{1}{a} = a \pi_\phi,\\
    &\mathcal{H}_\chi = \pi_\chi \dot{\chi} - \sqrt{g} \mathcal{L} = a \pi_\phi (a \dot{\phi} + \mathcal{H} a \phi) - a^4 \Big( \frac{1}{2a^2}\partial_\mu\phi \eta^{\mu\nu}\partial_\nu\phi - \tfrac12 m^2 \phi^2-V(\phi) \Big) = a^2 \mathcal{H}_\phi + a^3 \mathcal{H} (\pi_\phi \phi).
\end{align}
Since we used the same coordinates, both Hamiltonians must generate the same time evolution wrt the parameter $x^0=\tau$ and it might seem puzzling we have two different hamiltonians.
The puzzle is solved by recalling that, both in the classical and quantum case, the time evolution of a field/operator is given by the Poisson bracket/commutator with the hamiltonian plus any explicit time-dependence of the operator itself
\begin{align}
    \frac{d\hat{O}}{dt} = \frac{\partial \hat{O}}{\partial t} + \{\hat{O}, \hat{H}\}.
\end{align}
For example, we can see the time evolution of the field $\chi=a\phi$ are generated solely by $\mathcal{H}_\chi$ or by $\mathcal{H}_\phi$ with additional explicit time-dependence
\begin{align}
  \chi(t) &= e^{\tfrac{i}{h}H_\chi t} \chi(0) e^{-\tfrac{i}{h}H_\chi t} =a(t)\phi(t)= a(t) e^{\tfrac{i}{h}H_\phi t} \phi(0) e^{-\tfrac{i}{h}H_\phi t}.
\end{align}
In turn we have
\begin{align}
\frac{d\chi}{dt} &= \frac{\partial \chi}{\partial t} + \{\chi, H_\phi\} = \mathcal{H} a \phi + a \dot{\phi} = \pi_\chi,\\
\frac{d\chi}{dt} &= \frac{\partial \chi}{\partial t} + \{\chi, H_\chi\} = 0 + \pi_\chi =\pi_\chi.
\end{align}
The same holds for $\pi_\chi$, $\phi$ and $\pi_\phi$, viewing their time dependence as due to either hamiltonian plus the needed explicit time-dependence.

What is the physically meaningful hamiltonian then?
We should rather look at the 00 component of the stress-energy tensor which will identify $H_\phi$ ats the physical hamiltonian, and $H_\chi$ as a convenient auxiliary hamiltonian that generates the same time evolution but is not directly related to the energy of the system.

Also note that in a runaway scenario the equation for $\chi$ would read something like
\begin{align}
  \ddot{\chi}=\frac{\ddot{a}}{a}\chi \quad\Rightarrow\quad \chi(t)\propto a(t) \quad\text{and}\quad \phi(t) = \frac{\chi(t)}{a(t)} = \text{constant}\Rightarrow \text{freezing of oscillations superhorizon}
\end{align}
However note this is the equation for the full field, not the fluctuations. We shuld rather look at $\delta\chi$.
