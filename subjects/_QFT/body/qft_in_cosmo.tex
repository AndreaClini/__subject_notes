...
Indeed, the CCR are de-facto the conserved Noether charged associated to the global unitary symmetry of the theory.
...
Do different Bogoliubov bases also yield equivalent quantizations?\questiontag{Probably should move to general intro}

If the number of modes if finite, the Stone--von Neumann theorem ensures they are indeed equivalent, in the sense that there exists a unitary transformation $U$ relating the two Fock spaces $\mathcal{F}$ and $\tilde{\mathcal{F}}$ and mapping the ladder operators of one basis to the other, that is
\begin{align}
    U \hat{a}_n U^\dagger = \tilde{\hat{a}}_n = \sum_i \alpha_{ni} \hat{a}_i + \beta_{ni} \hat{a}_i^\dagger.
\end{align}
In such case we can pretend different ladder operators act on the same Fock space $\mathcal{F}\simeq \tilde{\mathcal{F}}$ and are related by the same Bogoliubov transformations as the mode functions in $\mathcal{S}_\C$.

The unitary transformation $U$ might not exist if the Bogoliubov transformation is `too large', that is if the Bogoliubov coefficients $\alpha_{ni}, \beta_{ni}$ do not satisfy the Shale--Stinespring condition $\sum_{n,i} |\beta_{ni}|^2 < \infty$.
In this case the two quantizations are unitarily inequivalent, and the two Fock spaces $\mathcal{F}$ and $\tilde{\mathcal{F}}$ are different Hilbert spaces.
This is a peculiarity of infinite-dimensional systems, and in particular of quantum field theories, since for finite-dimensional systems all Bogoliubov transformations are indeed always unitarily implementable and thus all quantizations are equivalent.
...
Crucially the commutator of the two operators in the exponentials is a c-number thanks to \eqref{eq:bogol_transform_mode_function_matrix_form_quantum}, namely\todotag{Do it}
{\small\begin{align}
    \Big[a_p\big(\alpha^{-1}\beta\big)_{pq}a_q\,,\,\,a^\dagger_i \big((\alpha^*)^{-1}\beta^*\big)_{ij} a^\dagger_j\Big]
    &= \big(\alpha^{-1}\beta\big)_{pq}\big((\alpha^*)^{-1}\beta^*\big)_{ij} \Big(a_pa_j^\dagger \delta_{qi}+a_pa_i^\dagger \delta_{qj}+ a^\dagger_i a_q \delta_{pj}+a^\dagger_j a_q \delta_{pi}\Big)\\
    &= ...
\end{align}}
