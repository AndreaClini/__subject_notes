...
{\small\begin{align}
    ...
    \begin{pmatrix}\tilde{a}_m\\ \tilde{a}_m^\dagger \end{pmatrix}.
\end{align}}
Under the Stone--von Neumann assumptions, we can relate the Fock spaces of the two theories via unitary transformation mapping the corresponding quantum DoFs into each other. We consider the transformation mapping $\mathcal{F}\mapsto\tilde{F}$ and $a\mapsto \tilde{a}$.
Virtually identical formulas holds swapping the roles of the theories with the replacements $\alpha \leftrightarrow \alpha^\dagger$ and $\beta \leftrightarrow -\beta^T$.
...
\end{align}}
Since this holds for arbitrary $m$ and the matrix $\alpha$ is invertible, we readily solve this set of differential equations for $f$ to get
{\small\begin{align}
...
We next normalize the vacuum $|\tilde{0}\rangle \propto f(a^\dagger)|0\rangle$ similarly to squeezed states.
...
\end{align}}
\todotag{Fix from here to the end}{\color{red}
This formula is easily proved when the operators are diagonal (cf. below).
The full \textbf{scattering operator} is the unitary operator relating the Fock spaces and ladder operators as\todotag{No,  it is $\text{arctgh}(Z)$ instead of $Z$ see below}
\begin{align}\label{eq:scattering_operator_bogoliubov_curved}
    \mathcal{S}:=&\exp\left(\tfrac12 a_i^\dagger (\alpha^{-1}\beta)^*_{ij}a_j^\dagger - \tfrac12 a_i(\alpha^{-1}\beta)_{ij}a_j\right)\equiv \exp\left(\tfrac12\tilde{a}_i^\dagger((\alpha^\dagger)^{-1}\beta^T)^*_{ij}\tilde{a}_j^\dagger - \tfrac12\tilde{a}_i((\alpha^\dagger)^{-1}\beta^T)_{ij}\tilde{a}_j\right),\\[3pt]
    &\tilde{a}_\ell = \mathcal{S} a_\ell \mathcal{S}^\dagger, \quad |\{\tilde{n}_i\}\rangle = \mathcal{S} |\{n_i\}_i\rangle.
\end{align}
The second expression for the scattering operators immediately follows from swapping the roles of the two theories with the replacements $\alpha \leftrightarrow \alpha^\dagger$ and $\beta \leftrightarrow -\beta^T$.

The above identities are most easily proved by unitarily rotating ladder operators in both theories so that the matrices $\alpha$, $\beta$ are diagonal and the complex symmetric matrix $Z:=\alpha^{-1}\beta$ is diagonal with strictly positive entries.
Indeed there exist $U,V\in \mathcal{U}(\mathcal{F})$ and $\Xi=\text{diag}(\xi_i)$ with $\xi_i>0$ real such that
{\small\begin{align}
    Z:= \alpha^{-1}\beta = V \Xi V^T, \quad \alpha = U \left(\mathbb{I}-\Xi^2\right)^{-\tfrac12} V^\dagger,\quad  \beta= U \Xi \left(\mathbb{I}-\Xi^2\right)^{-\tfrac12} V^T, \\[3pt]
    \mathcal{S}:=\exp\left(\tfrac12a_i^\dagger(V\Xi V^T)_{ij}^* a_j^\dagger - \tfrac12a_i(V\Xi V^T)_{ij}a_j\right)\equiv \exp\left(\tfrac12\tilde{a}_i^\dagger((\alpha^\dagger)^{-1}\beta^T)^*_{ij}\tilde{a}_j^\dagger - \tfrac12\tilde{a}_i((\alpha^\dagger)^{-1}\beta^T)_{ij}\tilde{a}_j\right)
\end{align}}
Defining unitarily rotated ladder operators $b= V^T a$ and $\tilde{b}:= U^T\tilde{a}$, still obeying the CCR, we thus find\todotag{No, it seems it is $\text{arctgh}(\Xi)$ instead of $\Xi$}
{\small\begin{align}
    \mathcal{S}:=\exp\left(\tfrac12a^\dagger_i(V\Xi V^T)^*_{ij} a_j^\dagger - \tfrac12a_i(V\Xi V^T)_{ij}a_j\right)=\exp\left(\tfrac12 b_i^\dagger \xi_i b_i^\dagger - \tfrac12 b_i \xi_i b_i\right)\\
    =\exp\left(\tfrac12\tilde{a}^\dagger_i(U\Xi U^T)^*_{ij} \tilde{a}_j^\dagger - \tfrac12\tilde{a}_i(U\Xi U^T)_{ij}\tilde{a}_j\right)=\exp\left(\tfrac12 \tilde{b}_i^\dagger \xi_i  \tilde{b}_i^\dagger - \tfrac12  \tilde{b}_i \xi_i  \tilde{b}_i\right)
\end{align}}
In this rotated bases the Bogoliubov coefficients read $\alpha'_{ij}=(1-\xi^2)^{-\tfrac12}\delta_{ij}$ and $\beta_{ij}=\xi(1-\xi^2)^{-\tfrac12}\delta_{ij}$ with $(1-\xi^2)^{-\tfrac12}=\cosh(y),\, \xi(1-\xi^2)^{-\tfrac12}=\sinh(y)$ for $y=\tfrac12 \log\left(\tfrac{1+\xi}{1-\xi}\right)$.
We verify the above relations by considering the antihermitian operator $G= -\tfrac12 b_i^\dagger \xi_i b_i^\dagger + \tfrac12 b_i \xi_i b_i = -\tfrac12 \tilde{b}_i^\dagger \xi_i  \tilde{b}_i^\dagger + \tfrac12  \tilde{b}_i \xi_i  \tilde{b}_i$ so that $\mathcal{S}= e^{-G}$ and solving the differential equation
{\small\begin{align}
    \begin{cases}
    \frac{d}{d\lambda} e^{-\lambda G} b_\ell e^{\lambda G} = e^{-\lambda G} [b_\ell,G] e^{\lambda G} = -\xi_\ell e^{-\lambda G} b_\ell^\dagger e^{\lambda G}, \quad (e^{-\lambda G} b_\ell e^{\lambda G})_{|\lambda=0}= b_\ell,\\[3pt]
    \frac{d}{d\lambda} e^{-\lambda G} b^\dagger_\ell e^{\lambda G} = e^{-\lambda G} [b^\dagger_\ell,G] e^{\lambda G} = -\xi_\ell e^{-\lambda G} b_\ell e^{\lambda G}, \quad (e^{-\lambda G} b^\dagger_\ell e^{\lambda G})_{|\lambda=0}= b^\dagger_\ell,
    \end{cases}\quad
    \text{i.e.} \quad \frac{d^2}{d\lambda^2}e^{-\lambda G} b_\ell e^{\lambda G} = \xi^2 e^{-\lambda G} b_\ell e^{\lambda G}
\end{align}}
Enforcing the initial conditions at $\lambda=0$ in the general solution $e^{-\lambda G} b_\ell e^{\lambda G} = C_1 \cosh(\xi\lambda)+C_2\sinh(\xi\lambda)$ and evaluating at $\lambda=1$ we get 
\begin{align}
    \mathcal{S} b_\ell \mathcal{S}^\dagger = e^{-\lambda G} b_\ell e^{\lambda G}\mid_{\lambda=1} = b_\ell \cosh(\xi) - b_\ell^\dagger \sinh(\xi) = b_\ell (1-\xi^2)^{-\tfrac12} - b_\ell^\dagger \xi (1-\xi^2)^{-\tfrac12} 
\end{align}}
\textcolor{red}{If instead the scattering operator had $\xi \mapsto \text{arctgh}(\xi)$ then it would work. Note the action on the vacuum would be the same as with chatgpt.}
\end{mytheorem}
...
\begin{mytheorem}[Bloch-Messiah decomposition of Bogoliubov coefficients]
%
...
\end{align}
By the Takagi factorization there exists a unitary $V\in\mathcal{U}(\mathcal{F})$ and a diagonal $\Xi=\text{diag}(\xi_i)$ with $\xi_i> 0$ real, such that
\begin{align}
    Z = V \Xi V^T.
\end{align}
...
\begin{align}
    ...
    \alpha (I - Z Z^\dagger) \alpha^\dagger = \alpha V(I - \Xi^2) V^\dagger \alpha^\dagger.
\end{align}
This implies that $\alpha V (I - \Xi^2)^{1/2}=: U \in\mathcal{U}(\mathcal{F})$ is a unitary matrix.
Hence we find
\begin{align}
    \alpha = U \underbrace{(I - \Xi^2)^{-1/2}}_{=:\,D_\alpha} V^\dagger, \quad \beta = \alpha Z = U \underbrace{\Xi (I - \Xi^2)^{-1/2}}_{=:\,D_\beta} V^T,
\end{align}
where the \emph{diagonal real positive} matrices equivalent to $\alpha$ and $\beta$ still obey the two Bogoliubov conditions since they are diagonal and satisfy 
\begin{align}
    \Big((I - \Xi^2)^{-1/2}\Big)^2 - \Big(\Xi(I - \Xi^2)^{-1/2}\Big)^2 = I.
\end{align}
