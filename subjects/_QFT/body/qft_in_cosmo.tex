...
\end{align}
\textcolor{red}{If instead the scattering operator had $\xi \mapsto \text{arctgh}(xi)$ then it would work. Note the action n the vacuum would be the same se chatgpt.}
\end{mytheorem}
...
\begin{mytheorem}[Takagi identity\todotag{Finish \& polish}]
%
If $Z$ is a complex symmetric matrix, there exists a unitary matrix $V\in\mathcal{U}(\mathcal{F})$ and a diagonal matrix $\Xi=\diag(\xi_i)$ such that $Z=V\Xi V^T$ and the real non-negative entries $\xi\in\R^+_0$ are precisely the singular values of $Z$ in the SVD decomposition sense, that is $Z= Q \Xi R^\dagger$ for some $Q,R\in \mathcal{U}(\mathcal{F})$.
\end{mytheorem}
\begin{proof}  
...
\noindent
\textbf{Step 1: existence of a Takagi vector} i.e. a nonzero $u_1\in\mathbb{C}^n$ s.t. $Au_1 =\lambda u_1^*$ and $\lambda \in\mathbb{R}^+$.
...
\end{align}
The first line implies $|z|^2=1$  and combining the last two we get
\begin{align}
    \partial_x f_\lambda -i \partial_yf_\lambda = (z^T A z)^* Az-\lambda z^*=0 \quad \Rightarrow\quad Az = \frac{\lambda}{(z^T A z)^*}\, z^*.
\end{align}
Note that multiplying on the left by $z^T$ gives $\lambda>0$ strictly, unless $A$ is identically zero since $z$ is supposed to be a maximum of $f_\lambda$,
\begin{align}
    |z^TAz|^2=(z^T A z)^* (z^TAz) = \lambda z^Tz^*= \lambda |z|^2 \quad \Rightarrow\quad \lambda\geq0.
\end{align}
Finally, up to rotating by a phase $z\mapsto e^{-i\varphi/2}z$ we can assume $(z^T A z)^*\in\mathbb{R}^+$.
Indeed if $(z^T A z)^*= e^{i\varphi}|z^T A z|$ we simply define $z:= e^{i\varphi/2}w$ and the above identity becomes
\begin{align}
    \cancel{e^{i\varphi}}Aw = \frac{\lambda}{\cancel{e^{-i2\varphi}}(w^T A w)^*}\, \cancel{e^{-i\varphi}}w^* \quad \Rightarrow\quad Aw= \underbrace{\frac{\lambda}{(w^T A w)^*}}_{\in\mathbb{R}^+}\, w^*.
\end{align}

\noindent
\textbf{Step 2: invariance of the orthogonal complement.}
Let $u_1$ be a Takagi vector for $A$ with eigenvalue $\lambda\in\mathbb{R}^+$ and consider the orthogonal linear subspace
\begin{align}
    W := u_1^\perp = \{ z \in \mathbb{C}^n : z^\dagger u_1^*= 0 \}.
\end{align}
Define the \emph{antilinear} map
\begin{align}
    z\mapsto T(z)=A z^*.
\end{align}
We claim that $W$ is invariant under $T$. Indeed if $z \in W$, then using $A^T = A$ and $A$
\begin{align}
    \Big((Az^*)^\dagger u_1\Big)^*=
    (Az^*)^T u_1^* = z^\dagger A^T u_1^* = z^\dagger A u_1^* = \lambda z^\dagger u_1=0
\end{align}
Since $A u_1 = \lambda u_1$, it follows that
\begin{align}
    u_1^* T(z) = \lambda z^* u_1 = 0.
\end{align}
Thus $T(z) \in W$ whenever $z \in W$. 

\paragraph{Step 3: induction on the restriction.}
Choose an orthonormal basis of $W$, say $v_2,\dots,v_n$. Relative to this basis, the restricted antilinear map $T|_W$ is represented by some matrix $B \in M_{n-1}(\mathbb{C})$.
We show that $B$ is again (complex) symmetric.
Indeed since the basis $v_2,\dots,v_n$ is orthonormal and $A=A^T$ we have, for $i,j \ge 2$,
\begin{align}
    B_{ij} = v_i^\dagger A v_j^* = v_i^{*(m)} A_{mn}v_j^{*(m)} = v_i^{*(m)} A_{nm}v_j^{*(m)} =  v_j^\dagger A v_i^* = B_{ji}\quad \text{i.e.}\quad B=B^T.
\end{align}
Because $A$ is invertible and $W$ is $T$-invariant, the restriction $T|_W$ is injective and thus invertible since $W$ is finite-dimensional. The matrix $B$ is then complex symmetric invertible and thus respect the inductive hypothesis.
...
\begin{align}
    A u_j = \lambda_j u^*_j,\qquad j = 2,\dots,n.
\end{align}
...
\end{align}
be the unitary matrix whose columns are the orthonormal Takagi vectors $u_j$, and let
\begin{align}
    \Lambda := \operatorname{diag}(\lamba_1,\dots,\lambda_n).
\end{align}
The relations $A u^*_j = \lambda_j u_j$ are equivalent to
\begin{align}
    A U^* = U \Lambda.
\end{align}
Since $U$ is unitary we thus multiplying on the right by $U^T$ we get
\begin{align}
    A = U \Lambda U^T.
\end{align}
This proves the Takagi factorization.
