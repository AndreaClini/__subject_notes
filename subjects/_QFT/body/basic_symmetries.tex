...
\begin{mytheorem}[Noether's theorem \todoinline{finish}]
...
Let us first discuss the classical version.
Consider a classical field theory described by a Lagrangian density
{\small\begin{align}
    \mathcal{L} = \mathcal{L}(\phi^a_b, \partial_\mu \phi^a_b,x^\mu),
\end{align}}
where $\phi^a_b$ is a field with arbitrary spin and internal indices labelled by $a$ and $b$, and $x^\mu$ are the spacetime coordinates.

Assume we have a transformation parametrized by a continuous parameter $\epsilon$ that leaves the action invariant:
{\small\begin{align}
    \phi^a_b(x) \to \phi^a_b(x) + \epsilon \delta \phi^a_b(x), \quad x^\mu \to x^\mu + \epsilon \delta x^\mu.
\end{align}}
