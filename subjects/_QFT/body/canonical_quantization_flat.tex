...
\begin{mytheorem}[Rotated harmonic oscillator\todotag{Finish}]
%
...
\begin{mytheorem}[Complex \& rotated harmonic oscillators\todotag{Finish}]
%
...
\end{equation}
%
It is customary to rewrite it as a single complex harmonic oscillator by defining the complex canonical variables in 4 possible ways, where the order of $\pm$ and $\mp$ signs correspond to not/taking the conjugate in the CCR, as
\begin{align}
...
\end{align}
Then identify corresponding real variables, where again the order of $\pm$ and $\mp$ signs correspond to not/taking the conjugate in the CCR, either expressing complex position (resp. momentum) as combination of real positions (resp. momenta) as
\begin{align}
...
\end{align}
or making both complex variables $q_z,p_z$ a mixture of real positions and momenta as
\begin{align}
    &q_x=\frac{q_z+q_z^\dagger}{\sqrt{2}},\quad p_x =\frac{p_z+p_z^\dagger}{\sqrt{2}},\quad
    q_y=\frac{\mp i}{\sqrt{2}}\frac{(p_z-p_z^\dagger)}{m\omega},\quad p_y =\frac{-im\omega}{\sqrt{2}} (q_z - q_z^\dagger).
\end{align}
%
In the \emph{complex} position representations, recalling that in \emph{any} case
\begin{align}
    \partial_z^ = \frac{1}{\sqrt{2}}\left(\partial_x - i \partial_y\right),\quad
    \partial_{z^*} = \frac{1}{\sqrt{2}}\left(\partial_x + i \partial_y\right),
\end{align}
the complex canonical variables read in the pure position/momentum case
\begin{align}
    q_z &= z = \frac{1}{\sqrt{2}}(x + i y), \quad p_z = \frac{1}{\sqrt{2}}(p_x \mp i p_y) = \frac{-i\hbar}{\sqrt{2}}\left(\partial_x \mp i \partial_y\right) = -i\hbar \frac{\partial}{\partial z^{(*)}}\,,
\end{align}
and in the mixed case
\begin{align}
    q_z &= z = \frac{1}{\sqrt{2}}\left(q_x + \tfrac{i}{m\omega} p_y\right) = \frac{1}{\sqrt{2}}\left(x + \tfrac{\hbar}{m\omega} \partial_y\right), \\
    p_z & = \frac{1}{\sqrt{2}}\left(p_x \pm i m \omega q_y\right) = \frac{1}{\sqrt{2}}\left(-i\hbar \partial_x \pm i \,m \omega y\right),
\end{align}
again with the order of $\pm$ and $\mp$ signs corresponding to not/taking the conjugate in the CCR.
%
\end{mytheorem}
...
Note we are assured they exist since $a$ is a \emph{normal} operator...
\end{mytheorem}
