...
\begin{mytheorem}[Energy-momentum of a scalar field: quintessence \& k-essence\questiontag{Finish}]
%
The most general action for a real scalar field is given by the $k$-essence lagrangians for arbitrary, yet physically meaningful, functions $P(\phi,X)$ of the scalar field $\phi$ and the kinetic term $X:=-g^{\mu\nu} \partial_\mu \phi \partial_\nu \phi$
\begin{align}
...
\end{align}
For the matching to work i.e. in order to identify the 4-velocity $u_\mu \,\propto \,\partial_\mu \phi$, it is \textbf{crucial that $\partial_\mu \phi$ is timelike}, that is time variations of the field should dominate over spatial ones $\dot{\phi}^2 > (\nabla \phi)^2$.

For example $P(X,\phi)=\tfrac{1}{2}X-V(\phi)$ recovers the simpler \emph{quintessence} action
{\small
\begin{align}
    S_{\text{5-ess}}=\int\!\! d^nx \sqrt{|g|} \left( -\frac{1}{2} g^{\mu\nu} \partial_\mu \phi \partial_\nu \phi - V(\phi) \right),\quad
    \rho = \tfrac{1}{2} X + V(\phi),\quad p = \tfrac{1}{2} X - V(\phi), \quad w:= \frac{p}{\rho} = \frac{\tfrac{1}{2}X - V(\phi)}{\tfrac{1}{2}X + V(\phi)}
\end{align}
}
with the famous equation of state parameter $w$ yielding $w\approx -1$ when potential energy dominates over the kinetic one $V(\phi) \gg X$.
%
\end{mytheorem}
