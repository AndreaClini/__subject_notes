...
\end{equation}
%
Consider the position space represenation of each Foruier mode $k$, so that the corresponding operators act as multplication and differentiation wrt a variable $\xi_k$, that is \emph{truly adimensional} $[\xi_k]=E^0$ variables,
\begin{align}
    \tilde{q}_k &= V^{-1/2}\xi_k\,\,,\quad
    \tilde{p}_k = -i V^{-1/2} \partial_{\xi_k}\,. \quad
\end{align}
%
...
\begin{align}
    \langle q_{k} | n_k \rangle =:
     ...
     \\
     &= \frac{1}{\sqrt{2^{n_k}n_k!}} H_{n_k}(V^{1/2}\omega_k^{1/2}q_k) \cdot \left( \frac{1}{\pi} \right)^{1/4} e^{-\frac{1}{2}V \omega_k q_k^2}
\end{align}
We could equivalently compute the wave function in terms of the momentum represenation $p_k$.
We find \todotag{Insert}
\begin{align}
    boh
\end{align}
