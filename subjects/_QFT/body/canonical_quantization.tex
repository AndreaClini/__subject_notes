...
\begin{mytheorem}[Infrared regularization and DoFs of the Klein-Gordon field]
%
To fix our conventions, we consider infrared regularization by placing the theory in a finite cubic box of volume $V=L^3$ with periodic boundary conditions.
Fourier modes become countable discrete $\mathbf{k} = \frac{2\pi}{L} \mathbf{n}$ with $\mathbf{n}\in\mathbb{Z}^3$, and we have usual correspondence
...
\end{align}
%
With the above conventions, all the previous formulas are unchanged upon the replacements \eqref{eq:IR_reg_replacements}.
The nonzero commutation relations become
\begin{align}
    [\phi_k(t), \pi_{k'}(t)] = i\hbar\, V \delta_{k,k'}, \quad
    [a_{k}, a_{k'}^{\dagger}] = V \delta_{k,k'}.
\end{align}
The energy dimensions of creation/annihilation operators $\{a_k,a_k^\dagger\}_k$, dimensional $\{q_k, p_k\}_k$ and adimensional $\{\tilde{q}_k, \tilde{p}_k\}_k$ harmonic oscillators variables, and their mutual relations \eqref{eq:relations_fields_creation_operators_harmonic_oscillator_var} remain unchanged.

%
Beware that in the literature it is common to rescale operators by $\sqrt{V}$ after IR regularization as
\begin{align}
    a_k &\mapsto \sqrt{V} a_k, \quad a_k^{\dagger} \mapsto \sqrt{V} a_k^{\dagger}, \quad
    q_k \mapsto \sqrt{V} q_k, \quad p_k \mapsto \sqrt{V} p_k, \quad
    \tilde{q}_k \mapsto \sqrt{V} \tilde{q}_k, \quad \tilde{p}_k \mapsto \sqrt{V} \tilde{p}_k,
\end{align}
...
\end{equation}
%
Consider the position space represenation of each Foruier mode $k$, so that the corresponding operators act as multplication and differentiation wrt a truly \emph{adimensional} variable $\xi_k$,
\begin{align}
    \tilde{q}_k &= V^{-1/2}\xi_k\,\cdot\,,\quad
    \tilde{p}_k = -i V^{-1/2} \frac{d}{d\xi_k}\,\cdot, \quad
\end{align}
%
In this representation, the wave function of the $n_k$th excited state $|n_k\rangle$ of the oscillator with mode $\vec{k}$ is\todotag{wrong from here}
{\color{red}
\begin{align}
    \langle q_{k} | n_k \rangle =:
     \psi_{n_k}(q_{k}) &=  = \left( \frac{m \omega_k}{\pi \hbar} \right)^{1/4} \frac{1}{\sqrt{2^{n_k} n_k!}} H_{n_k}\left( \sqrt{\frac{m \omega_k}{\hbar}} \phi_{k} \right) e^{-\frac{m \omega_k}{2\hbar} \phi_{k}^2}.
\end{align}
