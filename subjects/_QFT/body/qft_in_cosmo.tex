...
\begin{mytheorem}[Takagi identity\todotag{Check \& polish}]
%
If $Z$ is a complex symmetric matrix, there exists a unitary matrix $V\in\mathcal{U}(\mathcal{F})$ and a diagonal matrix $\Xi=\text{diag}(\xi_i)$ such that $Z=V\Xi V^T$. 
...
\noindent
\textbf{Step 1: existence of a Takagi vector}: a nonzero $u_1\in\mathbb{C}^n$ s.t. $Au_1^* =\lambda_1 u_1$ and $\lambda_1 \in\mathbb{R}^+$.
...
Namely, inserting a lagrange multiplier
{\small\begin{align}
    f_\lambda(z) &= \tfrac14|z^T A z|^2 +\tfrac12 \lambda(|z|^2-1)\\
    &= \tfrac14 \Big((x+iy)^TA(x+iy)\Big)\Big((x-iy)^TA^*(x-iy)\Big) +\tfrac12 \lambda(x^2+y^2-1).
\end{align}}
and imposing the differential vanish we get
\begin{align}
    \partial_\lambda f_\lambda = |z|^2-1=0,\\
    \partial_xf_\lambda = \tfrac12 (z^T A z)^* Az +\tfrac12 (z^T A z) A^*z^*-\lambda x=0,\\
    \partial_yf_\lambda = i\tfrac12 (z^T A z)^* Az -i\tfrac12 (z^T A z) A^*z^*-\lambda y=0.\\
\end{align}
...
\end{align}
Note that multiplying on the left by $z^T$ gives $\lambda>0$ strictly, indeed $\lambda=-0$ only is $A\equiv 0$ identically since $z$ is supposed to be a maximum of $f_\lambda$,
\begin{align}
...
Let $u_1$ be a Takagi vector for $A$ with eigenvalue $\lambda_1\in\mathbb{R}^+$ and consider the orthogonal linear subspace
\begin{align}
...
\end{align}
Because $A$ is invertible and $W$ is $T$-invariant and finite dimensional, the restriction $T|_W$ is injective and thus invertible. The matrix $B$ is then complex symmetric invertible and thus respect the inductive hypothesis.
...
\begin{mytheorem}[Bloch-Messiah decomposition of Bogoliubov coefficients\todotag{Finish \& polish}]
%
...
\end{align}
Assume $\alpha$ is invertible and define the symmetric ratio of Bogoliubov coefficient $Z := \alpha^{-1} \beta$.
Note $Z$ is indeed (complex) symmetric since
\begin{align}
...
\begin{align}
    Z = V \Xi V^T.
\end{align}
From the first Bogoliubov relation we have 
\begin{align}
...
\end{align}
For the creation operators, one has
\begin{align}
