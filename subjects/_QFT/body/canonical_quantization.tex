...
%
The field $\phi(t,\mathbf{x}) := \rho^{1/2} q_n(t)= m^{1/2}a^{-d/2} q_n(t)$ has mass dimension $[\phi] = \frac{d-1}{2}$ in spacetime dimension d+1.
Also note that we absorbed a factor $m^{1/2}$ in the rescaling of the field.
Therefore, when defining \emph{adimensional} variables for the densities of harmonic oscillators constituing the field down below, the rescaling will be
\begin{equation}
...
\begin{align}
    a^\dagger &= \tfrac{1}{\sqrt{2}}\left( \tilde{p} - i \tilde{q} \right) = \tfrac{1}{\sqrt{2\hbar}} \left(\sqrt{m\omega} \,q - \tfrac{i}{\sqrt{m\omega}} p \right), \qquad
