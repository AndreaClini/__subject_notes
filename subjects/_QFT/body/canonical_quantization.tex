...
%
\textbf{Beware that in the literature it is common to rescale operators by $\sqrt{V}$ after IR regularization} as
\begin{align}
...
Impose some IR regularization, so that Fourier modes are countable discrete and Fourier trasnforms change as discussed in \eqref{eq:IR_reg_replacements} above.
%
...
\end{equation}
%
Consider the space representation of each Fourier mode $k$, i.e. using a basis of eigenvalues of the $q_k$ operator\footnote{or equivalently of the truly adimensional operator $\hat{\xi}_k:=V^{-1/2}\tilde{q}_k=V^{-1/2}\omega_k^{1/2}q_k$.}, so that the corresponding operators act as multplication and differentiation wrt a variable $\xi_k\in\R$, that is \emph{truly adimensional} $[\xi_k]=E^0$,
\begin{align}
    q_k=\omega_k^{-1/2}\,\tilde{q}_k &=\omega_k^{-1/2}\, V^{1/2}\xi_k\,\,,\quad
    p_k=\omega_k^{1/2}\,\tilde{p}_k = -i \omega_k^{1/2}\, V^{1/2} \partial_{\xi_k}\,. \quad
\end{align}
%
In this representation, the wave function of the $n_k$th excited state $|n_k\rangle$ of the oscillator with mode $\vec{k}$ is\todotag{parts wrong from here}
{\color{blue}
\begin{align}
    \langle q_{k} | n_k \rangle =:
     \psi_{n_k}(q_{k}) 
     &= \frac{1}{\sqrt{n_k!}}\Big[\tfrac{1}{\sqrt{2}}\big(\xi_k-\partial_{\xi_k}\big)\Big]^{n_k} \left( \frac{1}{\pi} \right)^{1/4} e^{-\frac{1}{2}\xi_k^2} 
     \\
     &=:  \frac{1}{\sqrt{2^{n_k}n_k!}} H_{n_k}(\xi_k) \cdot \left( \frac{1}{\pi} \right)^{1/4} e^{-\frac{1}{2}\xi_k^2}
     \\
     &= \frac{1}{\sqrt{2^{n_k}n_k!}} H_{n_k}(V^{-1/2}\omega_k^{1/2}q_k) \cdot \left( \frac{1}{\pi} \right)^{1/4} e^{-\frac{1}{2}V^{-1} \omega_k q_k^2}
\end{align}
We could equivalently compute the wave function the momentum represenation of the Fourier mode $k$, i.e. using a basis of eigenvalues of the $p_k$ operator\footnote{or equivalently of the truly adimensional operator $\hat{\eta}_k:=V^{-1/2}\tilde{p}_k=V^{-1/2}\omega_k^{-1/2}p_k$.}, so that the corresponding operators act as multplication and differentiation wrt a variable $\eta_k\in\R$, that is \emph{truly adimensional} $[\eta_k]=E^0$ variables,
\begin{align}
    q_k=\omega_k^{-1/2}\,\tilde{q}_k &= i \omega_k^{-1/2}\, V^{-1/2}\partial_{\eta_k}\,\,,\quad
    p_k=\omega_k^{1/2}\,\tilde{p}_k = \omega_k^{1/2}\, V^{-1/2} \eta_k\,. \quad
\end{align}
In this representation, the wave function of the $n_k$th excited state $|n_k\rangle$ of the oscillator with mode $\vec{k}$ is
\begin{align}
    \langle p_{k} | n_k \rangle =:
     \tilde{\psi}_{n_k}(p_{k}) 
     &= \frac{1}{\sqrt{n_k!}}\Big[\tfrac{(-i)}{\sqrt{2}}\big(\eta_k-\partial_{\eta_k}\big)\Big]^{n_k} \left( \frac{1}{\pi} \right)^{1/4} e^{-\frac{1}{2}\eta_k^2} 
     \\
     &=: \frac{(-i)^{n_k}}{\sqrt{2^{n_k}n_k!}} H_{n_k}(\eta_k) \cdot \left( \frac{1}{\pi} \right)^{1/4} e^{-\frac{1}{2}\eta_k^2}
     \\
     &= \frac{1}{\sqrt{2^{n_k}n_k!}} H_{n_k}(V^{-1/2}\omega_k^{-1/2}p_k) \cdot \left( \frac{1}{\pi} \right)^{1/4} e^{-\frac{1}{2}V^{-1} \omega_k^{-1} p_k^2}
\end{align}

Equivalently, the eigenvectors of the $p_k$ operator have overlap with those of the $q_k$ operator given by 
\begin{align}
    \langle q_k | p_k \rangle &= \frac{1}{2\pi}e^{i \xi_k\eta_k}= \frac{1}{2\pi}e^{i V q_k p_k}.
\end{align}
%
In turn he wave function of the $n_k$th excited state $|n_k\rangle$ of the oscillator with mode $\vec{k}$ in the $p_k$ representation is obtained inserting a resolution of the identity, using $\xi_k\mapsto i \partial_{\eta_k}$ and $\partial_{\xi_k} \mapsto i \eta_k$ under Fourier transform, we find
\begin{align}
    \langle p_{k} | n_k \rangle = 
     &= \int d q_k \, \langle p_k | q_k \rangle \langle q_k | n_k \rangle
     \\
     &= \int_\R d\xi_k\, \frac{1}{2\pi}e^{-i \xi_k\eta_k} \underbrace{\frac{1}{\sqrt{n_k!}}\Big[\tfrac{1}{\sqrt{2}}\big(\xi_k-\partial_{\xi_k}\big)\Big]^{n_k} \left( \frac{1}{\pi} \right)^{1/4} e^{-\frac{1}{2}\xi_k^2}}_{= \frac{1}{\sqrt{2^{n_k}n_k!}} H_{n_k}(\xi_k) \cdot \left( \frac{1}{\pi} \right)^{1/4} e^{-\frac{1}{2}\xi_k^2}}
     \\
     &= \int_\R d\xi_k\, \frac{1}{2\pi}e^{-i \xi_k\eta_k} \frac{1}{\sqrt{n_k!}}\Big[(-i)\tfrac{1}{\sqrt{2}}\big(\eta_k-\partial_{\eta_k}\big)\Big]^{n_k} \left( \frac{1}{\pi} \right)^{1/4} e^{-\frac{1}{2}\xi_k^2}
     \\
     &= \frac{(-i)^{n_k}}{\sqrt{n_k!}}\Big[\tfrac{1}{\sqrt{2}}\big(\eta_k-\partial_{\eta_k}\big)\Big]^{n_k} \left( \frac{1}{\pi} \right)^{1/4} \int_\R d\xi_k\, \frac{1}{2\pi}e^{-i \xi_k\eta_k}  e^{-\frac{1}{2}\xi_k^2}
     \\
     \\
     &= \frac{(-i)^{n_k}}{\sqrt{n_k!}}\Big[\tfrac{1}{\sqrt{2}}\big(\eta_k-\partial_{\eta_k}\big)\Big]^{n_k} \left( \frac{1}{\pi} \right)^{1/4}e^{-\frac{1}{2}\eta_k^2}
     \\
     &=: \frac{(-i)^{n_k}}{\sqrt{2^{n_k}n_k!}} H_{n_k}(\eta_k) \cdot \left( \frac{1}{\pi} \right)^{1/4} e^{-\frac{1}{2}\eta_k^2}
     \\
     &= \frac{(-i)^{n_k}}{\sqrt{2^{n_k}n_k!}} H_{n_k}(V^{-1/2}\omega_k^{-1/2}p_k) \cdot \left( \frac{1}{\pi} \right)^{1/4} e^{-\frac{1}{2}V^{-1} \omega_k^{-1} p_k^2}
\end{align}
}
