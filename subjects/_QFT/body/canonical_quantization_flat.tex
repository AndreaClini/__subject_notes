...
%
Consider a 1D harmonic oscillator driven by time-dependent forces $J(t)$ and $K(t)$ acting on position and momentum respectively
\begin{align}
    H = \frac{1}{2m} p^2 + \frac{m\omega^2}{2} q^2 - J(t) q - K(t) p.
\end{align}
Define ladder operators to diagonalize the free hamiltonian and get
\begin{align}
    a &= \frac{1}{\sqrt{2}}\left(\sqrt{\frac{m\omega}{\hbar}}\, q + i \frac{1}{\sqrt{m \omega \hbar}}\, p\right), \quad [a, a^\dagger] = 1,\\
    H &= \hbar \omega \left(a^\dagger a + \tfrac{1}{2}\right) + \sqrt{\frac{\hbar}{2 m \omega}} J (a + a^\dagger) - i \sqrt{\frac{m \omega \hbar}{2}} K (a - a^\dagger).
\end{align}
In the Heisenberg picture, if $J,\,K$ commute with $a,\,a^\dagger$ e.g. if they are independent of the canonical variables, the equation of motion for $a$ reads
\begin{align}%\label{eq:1d_oscillator_ladder_operator_time_evolution}
    &i\hbar\,\partial_t a= [a, H] = \hbar \omega a + \sqrt{\frac{\hbar}{2 m \omega}} J + i \sqrt{\frac{m \omega \hbar}{2}} K\\
    &\Longrightarrow \quad a(t) = a(t_0) e^{-i\omega (t-t_0)} - \frac{i}{\hbar} \int_{t_0}^t dt' \left(\sqrt{\tfrac{\hbar}{2 m \omega}} J(t') + i \sqrt{\tfrac{m \omega \hbar}{2}} K(t')\right) e^{-i\omega (t-t')}.
\end{align}
...
where we define
\begin{align}
    a_{\mathrm{out}} &= a_{\mathrm{in}} + J_0\,,\quad J_0 := - \frac{i}{\hbar} \int_{t_i}^{t_f} dt' \left(\sqrt{\tfrac{\hbar}{2 m \omega}} J(t') + i \sqrt{\tfrac{m \omega \hbar}{2}} K(t')\right) e^{i\omega t'}.
\end{align}
In particular we note the Heisenberg hamiltonian is indipendent of time outside the interval $[t_i, t_f]$
\begin{align}
...
\begin{itemize}
    ...
    \item \textbf{Resonance.} What is the best choice of $J(t)$ to excite the oscillator.
...
\textbf{Motion.} How expectations of observables like $\langle q(t)\rangle$ and $\langle p(t)^2\rangle$ change due to the driving force.
...
\textbf{Energy.} How energy is pumped into the oscillator by the driving force, i.e. how $\langle H(t)\rangle$ changes with time. \todotag{Finish}
