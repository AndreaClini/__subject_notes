...
\begin{mytheorem}[Coherent \& squeezed states\todotag{Polish}]
%
...
A concrete example is how the electron field excites the electromagnetic field to produce photons e.g. when electrons in an antenna oscillate due to an applied voltage and excite the EM field to produce radio waves.
...
A primary example if inflation, that modifies frequencies of oscillators even up to turning some of them upside-down leading to run-away solutions.
%
\end{mytheorem}



\begin{mytheorem}[Driving forces \& first examples of Bogoliubov transformations\todotag{Finish}]
%
Consider a 1D harmonic oscillator driven by time-dependent forces $J(t)$ and $K(t)$ acting on position and momentum respectively, with Schrödinger Hamiltonian
\begin{align}
    H = \frac{1}{2m} p^2 + \frac{m\omega^2}{2} q^2 + J(t) q - K(t) p.
\end{align}
...
Note the Schrödinger Hamiltonian is time-dependent due to the driving forces $J(t), K(t)$ and does not commute at different times.
Indeed, assuming for simplicity $K(t)=0$ and $J(t)$ commute with itself at any two times and with ladder operators, we have
\begin{align}
    [H(t), H(s)] = \hbar \omega \sqrt{\frac{\hbar}{2 m \omega}} (J(t) - J(s)) [a + a^\dagger, a^\dagger a] = \hbar \omega \sqrt{\frac{\hbar}{2 m \omega}} (J(t) - J(s)) (a - a^\dagger) \neq 0.
\end{align}
The Heisenberg Hamiltonian will thus be different from the Schrödinger one
\begin{align}
    H_H(t,t_0) &= U^\dagger(t,t_0) H_S(t) U(t,t_0) 
    = \mathcal{T} \exp\left(\tfrac{1}{\hbar} \int_{t_0}^t dt' H_S(t')\right) H_S(t) \mathcal{T} \exp\left(-\tfrac{1}{\hbar} \int_{t_0}^t dt' H_S(t')\right) 
    \neq H_S(t).
\end{align}
The equation of motion for $a$ in the Heisenberg picture reads
\begin{align}%\label{eq:1d_oscillator_ladder_operator_time_evolution}
    &i\hbar\,\partial_t a= [a, H] = \hbar \omega a + \sqrt{\frac{\hbar}{2 m \omega}} J \quad
    \Rightarrow \quad a(t) = a(t_0) e^{-i\omega (t-t_0)} - \frac{i}{\hbar} \int_{t_0}^t dt' \sqrt{\tfrac{\hbar}{2 m \omega}} J(t')  e^{-i\omega (t-t')}.
\end{align}
...
\end{align}
In particular the Heisenberg hamiltonian result independent of time outside the interval $[t_i, t_f]$, albeit different from the Schrödinger one at all times $t\geq t_i$,
\begin{align}
    H = \hbar \omega \left(a_{\mathrm{in}}^\dagger a_{\mathrm{in}} + \tfrac{1}{2}\right), \quad t < t_i, \qquad H = \hbar \omega \left(a_{\mathrm{out}}^\dagger a_{\mathrm{out}} + \tfrac{1}{2}\right), \quad t > t_f.
\end{align}
...
We want to understand how the driving force affects several aspects of the dynamics of the system. \todotag{Finish see Kempf L6}
\begin{itemize}
    \item \textbf{Motion.} How expectations of observables like $\langle q(t)\rangle$ and $\langle p(t)^2\rangle$ change due to the driving force.
    \item \textbf{Energy.} How energy is pumped into the oscillator by the driving force, i.e. how $\langle H(t)\rangle$ changes with time.
    \item \textbf{Resonance.} What is the best choice of $J$ to excite the oscillator? The driving $J(t)$ should oscillate at the natural frequency $\omega$ of the oscillator, as shown by the two previous points, and as expected from the \emph{classical} case.
    \item \textbf{Energy eigenstates \& Bogoliubov transformation.} How the in- and out-Fock states are related, and how the number of quanta (particles) is changed by the driving force.
\end{itemize}
For simplicity consider only a position driving force $J(t)$, setting $K(t)=0$.
------------------------------------------------------------------------------------------


\textbf{Motion.} 
Using the time evolution above and the expression of $q$ and $p$ in terms of ladder operators, we find
\begin{align}
...
\end{align}
We see that indeed the average position is not zero anymore. 
...
Indeed, the bigger the interval $t_f-t_i$ the better the `wannabe' Fourier transform above matches $\tilde{J}(\omega)$.
Similarly we can compute expectation values of other observables.


\textbf{Energy.}
For simplicity consider only late times $t>t_f$, the energy expectation value reads
\begin{align}
...
\end{align}
This is confirmed acting explicitly with $a_{\mathrm{out}}$ on the rhs, using $[a_{\mathrm{out}},J_0]=0$,
\begin{align}
...
\end{align}
More generally the in-vacuum have overlap with out-Fock states given by
{\small
\begin{align}
    {}_{\mathrm{out}}\langle n |0\rangle_{\mathrm{in}} &= e^{-\frac{1}{2} |J_0|^2} {}_{\mathrm{out}}\langle n | \sum_{m=0}^\infty \frac{(J_0 a_{\mathrm{out}}^\dagger)^m}{m!} |0\rangle_{\mathrm{out}}
    = {}_{\mathrm{out}}\langle 0 | \frac{d^n}{d a_{\mathrm{out}}^n} \left(e^{-\frac{1}{2} |J_0|^2} e^{J_0 a_{\mathrm{out}}^\dagger} |0\rangle_{\mathrm{out}}\right)
    = e^{-\frac{1}{2} |J_0|^2} \frac{J_0^n}{\sqrt{n!}}
    \\[6pt]
    &\quad\Rightarrow\quad |0\rangle_{\mathrm{in}} = \sum_{n=0}^\infty e^{-\frac{1}{2} |J_0|^2} \frac{J_0^n}{\sqrt{n!}} |n\rangle_{\mathrm{out}}.
\end{align}
}
General in states have overlap with out-states given by\todotag{CHECK}
{\small
\begin{align}
    {}_{\mathrm{out}}\langle n | m \rangle_{\mathrm{in}} &= {}_{\mathrm{out}}\langle 0 | \frac{(a_{\mathrm{out}}\big)^n}{\sqrt{n!}}\frac{(a_{\mathrm{out}}^\dagger-J_0^\dagger\big)^m}{\sqrt{m!}} e^{-\tfrac{1}{2} |J_0|^2} e^{iJ_0a_{\mathrm{out}}^\dagger}|0\rangle_{\mathrm{out}}
    \\
    &= e^{-\tfrac{1}{2} |J_0|^2} \frac{1}{\sqrt{n! m!}} {}_{\mathrm{out}}\langle 0 |\frac{d^n}{d a_{\mathrm{out}}^n} \left(a_{\mathrm{out}}^\dagger - J_0^\dagger\right)^m e^{J_0 a_{\mathrm{out}}^\dagger}|0\rangle_{\mathrm{out}}
    \\
    &= e^{-\tfrac{1}{2} |J_0|^2} \frac{1}{\sqrt{n! m!}} \sum_{\ell=0}^{n\wedge m}\binom{n}{\ell}{}_{\mathrm{out}}\langle 0 |\frac{d^\ell}{d (a_{\mathrm{out}}^\dagger)^\ell}\left(a_{\mathrm{out}}^\dagger - J_0^\dagger\right)^{m} \frac{d^{n-\ell}}{d (a_{\mathrm{out}}^\dagger)^{n-\ell}} e^{J_0 a_{\mathrm{out}}^\dagger}|0\rangle_{\mathrm{out}}
    \\
    &= e^{-\tfrac{1}{2} |J_0|^2} \frac{1}{\sqrt{n! m!}} \sum_{\ell=0}^{n\wedge m}\binom{n}{\ell}{}_{\mathrm{out}}\langle 0 |\frac{m!}{(m-\ell)!}\left(a_{\mathrm{out}}^\dagger - J_0^\dagger\right)^{m-\ell} J_0^{n-\ell} e^{J_0 a_{\mathrm{out}}^\dagger}|0\rangle_{\mathrm{out}}
    \\
    &= e^{-\tfrac{1}{2} |J_0|^2} \frac{1}{\sqrt{n! m!}} \sum_{\ell=0}^{n\wedge m}\binom{n}{\ell}{}_{\mathrm{out}}\langle 0 |\frac{m!}{(m-\ell)!}(-1)^{m-\ell}(J_0^\dagger)^{m-\ell} J_0^{n-\ell} e^{J_0 a_{\mathrm{out}}^\dagger}|0\rangle_{\mathrm{out}}
    \\
    &= e^{-\tfrac{1}{2} |J_0|^2} \frac{1}{\sqrt{n! m!}} \sum_{\ell=0}^{n\wedge m}\binom{n}{\ell}\frac{m!}{(m-\ell)!}(-1)^{m-\ell}(J_0^\dagger)^{m+n-2\ell}
    \\
    &=  \frac{e^{-\tfrac{1}{2} |J_0|^2}}{\sqrt{n!\, m!}} \sum_{\ell=0}^{n\wedge m} \binom{n}{\ell}\binom{m}{\ell}\ell!(-1)^{m-\ell}\,J_0^{m+n-2\ell}
\end{align}
}



\begin{mytheorem}[Making semiclassical treatments rigorous: electron in classical EM field\todotag{Fix \& finish (Kempf L6 fine)}]
%
The above analysis of exciting harmonic oscillators allows to understand the rigorous treatment of a quantized electron in a classical electromagnetic field and other semiclassical treatments usually presented in basic QM courses.
...
These quantum states are the closest to classical states, minimizing the uncertainty relation and having dynamics closely following classical trajectories.
If each mode of the EM field is in such a state, we can already understand why the above semiclassical treatment works so well.

More rigorously, consider a full Hamiltonian for the EM field interacting with an electron
\begin{align}
    H = H_{\mathrm{EM}}\otimes 1 + 1 \otimes H_{\mathrm{e}^-} + H_{\mathrm{int}},
\end{align}
where $H_{\mathrm{EM}} \sim \sum_{\mathbf{k},\lambda} \hbar \omega_k (a_{\mathbf{k},\lambda}^\dagger a_{\mathbf{k},\lambda} + \tfrac{1}{2})$ is the Hamiltonian of the free quantized EM field, $H_{\mathrm{e}^-}\sim p^2/2m$ is the Hamiltonian of the free electron and $H_{\mathrm{int}}=\vec{p}\otimes B$ describes a possible interaction.
The time evolution of a given state will be 
\begin{align}
...
However, if the EM field is in a coherent state $|\psi_{\mathrm{EM}}(t_0)\rangle = |\{\alpha_{\mathbf{k},\lambda}\}\rangle$, then we can approximate the interaction Hamiltonian as
\begin{align}
    H_{\mathrm{int}} &= \vec{p} \otimes B \approx \vec{p} \otimes \langle \{\alpha_{\mathbf{k},\lambda}\}| B | \{\alpha_{\mathbf{k},\lambda}\}\rangle = \vec{p} \otimes B^{cl}(t,\mathbf{x}) = B^{cl}(t,\mathbf{x})\cdot\,\, \vec{p}\otimes 1,
\end{align}
where $B^{cl}(t,\mathbf{x})$ is a classical EM field given by the expectation value of the field operator $B$ on the coherent state.
Beware that $|\psi_{\mathrm{EM}}(t_0)\rangle$ cannot be an \emph{exact} eigenstate of $B$, since otherwise the uncertainty in the conjugate variable would be infinite, but since it saturates the uncertainty relation the fluctuations around the expectation value are minimal, making this a good approximation.
The time evolution is then non-entangling and factorizes as
\begin{align}
    |\Psi(t)\rangle &\approx e^{\tfrac{-i(t-t_0)}{\hbar} \left(H_{\mathrm{EM}}\otimes 1 + 1 \otimes H_{\mathrm{e}^-} + B^{cl}(t,\mathbf{x})\cdot\,\, \vec{p}\otimes 1\right)}\,\,
...
\end{align}
After tracing out the EM field degrees of freedom, the electron is thus left in a pure state evolving under the free Hamiltonian augmented with the interaction with a classical EM field.
%
\end{mytheorem}
...
\end{aligned}
\end{align}
The ladder operators are defined wrt the rotated oscillators $\{q_k, p_k\}$ as
...
\end{align}
After some algebra, \emph{before} normal ordering, the Hamiltonian reads
...
\begin{itemize}
    ...
    The most immediate examples are coherent (squeezed) states, superpositions of different number eigenstates without a well-defined number of particles in any mode, that return c-numbers when acted upon by ladder operators.
    %
