...
%--------------------------------------------------------
%=========================================================
\section[Classical \& Quantum Harmonic Oscillators]
{Classical \& Quantum Harmonic Oscillators
\questioninline{Maybe move to separate chapter `QM basics'}}
\label{sec:harmonic_oscillators}
%=========================================================
%------------------------------------------------------------
...
\begin{mytheorem}[Review of classical \& quantum harmonic oscillator]
%
The \emph{classical} harmonic oscillator with mass $m$ and frequency $\omega$ is described by the Hamiltonian
{\small\begin{align}\label{eq:classical_harmonic_oscillator_hamiltonian}
\begin{aligned}
...
%
We recale the conjugate variables so as to make kinetic and potential energy terms equal, that is imposing
{\small\begin{align}
    \tfrac{1}{2m}(\hbar/\ell)^2\overset{!}{=} \tfrac{1}{2} m\omega^2\ell^2\implies \ell=\sqrt{\tfrac{\hbar}{m \omega}}\,.
\end{align}}
gives
{\small\begin{align}\label{eq:adimensional_harmonic_oscillator_variables}
\begin{aligned}
...
{\small\begin{align}\label{eq:classical_harmonic_oscillator_factorization}
\begin{aligned}
...
%
Following Heisenberg canonical quantization procedure, promote the classical theory to a quantum one following:
\begin{itemize}
    ...
    %
    \item All the equations are left formally unchanged, but now refer to evolving operators.
\end{itemize}
%
The Hamiltonian operator is given by
{\small\begin{align}\label{eq:quantum_harmonic_oscillator_hamiltonian}
\begin{aligned}
...
{\small\begin{align}\label{eq:harmonic_oscillator_canonical_commutators}
\begin{aligned}
    [q,p]=i\hbar \delta_{ij} ,\quad [q,q]=[p,p]=0,
    \qquad
    [\tilde{q}, \tilde{p}] = i \delta_{ij},\quad [\tilde{q}, \tilde{q}] = [\tilde{p}, \tilde{p}] = 0.
\end{aligned}\end{align}}
We cannot use the previous factorization since $\tilde{p}$ and $\tilde{q}$ do not commute anymore.
We have instead
{\small\begin{align}\label{eq:harmonic_oscillator_ladder_hamiltonian}
\begin{aligned}
    \hat{H} &=
    \hbar \omega \bigg[
...
{\small\begin{align}\label{eq:ordinary_QM_harmonic_oscillator}
\begin{aligned}
...
{\small\begin{align}\label{eq:quantum_harmonic_oscillator_spectrum}
\begin{aligned}
    E_n = \hbar \omega \left( n + \tfrac{1}{2} \right), \quad n = 0, 1, 2, \ldots
\end{aligned}\end{align}}
%
In the familiar position representation  we have
{\small\begin{align}\label{eq:harmonic_oscillator_position_representation}
\begin{aligned}
...
We remark that we could have equivalently started the quantization procedure directly from the second expression of the \emph{classical} Hamiltonian, thereby obtaining
{\small\begin{align}\label{eq:normal_ordered_harmonic_oscillator}
\begin{aligned}
...
\end{wrapfigure}
It is fundamental to understand how the \emph{fixed} frequency $\omega$ of the oscillator determines the likelyhood of finding the system at a given elongation from equilibrium i.e. the amplitude of oscillations.
...
{\small\begin{align}\label{eq:harmonic_oscillator_wavefunctions}
\begin{aligned}
...
\end{aligned}\end{align}}
Higher wave functions are more likely to be found at larger elongations, but the Gaussian envelope always dominates at large $|q|$.
...
This is already familiar from Electrodynamics: it is very unlikely to find high-frequency electromagnetic waves with large amplitudes, whereas low-frequency waves (e.g. radio waves) are easily found with large amplitudes.
\end{mytheorem}



\begin{mytheorem}[Identifying ladder operators]
%
...
The definition of ladder operators can be obtained \textbf{by direct inspection of the Hamiltonian} after rescaling the canonical variables to adimensional ones as done above.
However, this procedure somewhat hides the arbitrariness of their definition, which is source of profound physical consequences for QFT in curved spacetime or accelerated frames.
...
{\small\begin{align}\label{eq:ladder_operator_heuristics_1D}
...
Namely, using $[q,p]=i\hbar$, we choose
\begin{align}
    [a, a^\dagger] &= [\alpha q + i \beta p, \alpha q - i \beta p] = 2 \alpha \beta \hbar \quad \Rightarrow\quad \beta = \frac{1}{2\alpha \hbar}\,.
\end{align}
The original Schrodringer Hamiltonian can then be expressed as\footnote{We specify Schrodringer Hamiltonian since time-dependence of $J(t),K(t)$ makes the Heinseberg $H_H(t):=U(t,t_0)^\dagger H_S(t)U(t,t_0)$ and Schrodringer Hamiltonians different in general since $[H_S(t),H_S(s)]\neq0$, so that $H_S(t')$ do not commute with $U(t,s)$. }
{\small\begin{align}
    H_S(t)&= \frac{1}{2m}p^2+\frac{m\omega^2}{2}q^2 +Jq + Kp\\
...
\begin{align}\label{eq:choosing_alpha_ladder_op}
    \alpha = \tfrac{1}{\sqrt{2}}\sqrt{\tfrac{m\omega}{\hbar}} \quad \Longrightarrow \quad \beta = \tfrac{1}{\sqrt{2}}\tfrac{1}{\sqrt{m \omega \hbar}}\,.
\end{align}}
Note that this is precisely the adimensional rescaling $\ell=\sqrt{\tfrac{\hbar}{m\omega}}$ used in \eqref{eq:adimensional_harmonic_oscillator_variables} to make kinetic and potential energy terms equal.
The final expressions are
{\small
\begin{align}\label{eq:oscillator_hamiltonian_written_ladder_operators}
    a &= \frac{1}{\sqrt{2}}\Big(\sqrt{\tfrac{m\omega}{\hbar}}\, q + i \tfrac{1}{\sqrt{m \omega \hbar}}\, p\Big), \quad [a, a^\dagger] = 1,\\[2.5pt]
    H_S(t) &= \hbar \omega \left(a^\dagger a + \tfrac{1}{2}\right) + \sqrt{\frac{\hbar}{2 m \omega}} J (a + a^\dagger) - i \sqrt{\frac{m \omega \hbar}{2}} K (a - a^\dagger).
\end{align}}
...
On the other hand, if $J$ or $K$ do depend on $q,\,p$, the above expression suggests it might be more \textbf{convenient to define ladder operators in some other way!}

Denote the time-evolution operator $U(t,s)=\mathcal{T}\exp\left(-\tfrac{i}{\hbar}\int_s^t\!\!dt'H_S(t')\,\right)$.
If $J(t),K(t)$ are independent of the canonical variables, the Heisenberg equation of motion for $a(t)=U(t,t_0)^\dagger aU(t,t_0)$ reads
\begin{align}\label{eq:1d_oscillator_ladder_operator_time_evolution}
    &i\hbar\,\partial_t a(t)= U(t,t_0)^\dagger [a, H_s(t)] U(t,t_0)= \hbar \omega a(t) + \sqrt{\frac{\hbar}{2 m \omega}} J(t) + i \sqrt{\frac{m \omega \hbar}{2}} K(t)\\
...
\begin{mytheorem}[Rotated harmonic oscillators]
%
Consider an isotropic\footnote{\label{fn:isotropic_oscillator}for simplicity, otherwise diagonalize the frequency matrix} harmonic oscillator in real dimension $n$
\begin{align}
    H = \frac{1}{2m} \sum_{j=1}^n (p^j)^2 + \frac{m\omega^2}{2} \sum_{j=1}^n (q^j)^2, \quad \{q, p^T\} = \delta_{ij}.
\end{align}
We can rotate coordinates with an orthogonal matrix $R \in O(n)$ as   
\begin{align}
     \tilde{q}^i = R^i_{j} q^j, \quad \tilde{p}^i = R^i_{j} p^j, \quad 
     \{\tilde{q}, \tilde{p}^T\} = R\{q,p^T\} R^T = R\,R^T = \delta_{ij}\,.
\end{align}
Passing to the quantum case, everythin is unchanged upond replacing $\{\,,\,\}\mapsto \frac{1}{i\hbar} [\,,\,]$.
...
Correspondingly the ladder operators just change via the same rotation matrix $R$ as
\begin{align}
...
\begin{mytheorem}[Complex \& rotated harmonic oscillators\noteinline{Check}]
%
Consider a 2D real isotropic\footnotemark[\getrefnumber{fn:isotropic_oscillator}] harmonic oscillator with Hamiltonian
\begin{equation}
    H = \tfrac{1}{2m}(p_x^2 + p_y^2) + \tfrac{m\omega^2}{2} (q_x^2 + q_y^2), \quad \{q_x, p_x\} = \{q_y, p_y\} = 1, \,\,\, \{q_x, p_y\} = \{q_y, p_x\} = 0.
\end{equation}
%
We can rewrite it as a single complex harmonic oscillator by defining the complex canonical variables in 4 possible ways\footnote{\label{fn:order_sign_CCR_conjugate} The order of $\pm$ and $\mp$ signs correspond to not taking (resp. taking) the conjugate in the CCR. For example in the first line choosing $p_z = \frac{1}{\sqrt{2}}(p_x - i p_y)$ corresponds to $\{q_z, p_z\} = 1$, while $p_z = \frac{1}{\sqrt{2}}(p_x + i p_y)$ corresponds to $\{q_z, p_z^\dagger\} = 1$.}
\begin{align}
...
\end{align}
In practice one often starts from a complex harmonic oscillator obeying the CCR with/out conjugation, and then identify corresponding real variables\footnotemark[\getrefnumber{fn:order_sign_CCR_conjugate}] either expressing the complex position (resp. momentum) as combination of real positions (resp. momenta) as in \eqref{eq:complex_oscillator_pure_decomposition}
\begin{align}
...
\end{align}
or making both complex variables $q_z,p_z$ a mixture of real positions and momenta as in \eqref{eq:complex_oscillator_mixed_decomposition}, 
\begin{align}
    &q_x=\frac{q_z+q_z^\dagger}{\sqrt{2}},\quad p_x =\frac{p_z+p_z^\dagger}{\sqrt{2}},\quad
...
Recall that in \emph{any} case derivatives obey
\begin{align}
    \partial_z = \frac{1}{\sqrt{2}}\left(\partial_x - i \partial_y\right),\quad
    \partial_{z^*} = \frac{1}{\sqrt{2}}\left(\partial_x + i \partial_y\right),
\end{align}
In the pure position/momentum case \eqref{eq:complex_oscillator_pure_decomposition}, we use the position representation for both the $x$ and $y$ variables to get\footnotemark[\getrefnumber{fn:order_sign_CCR_conjugate}]
\begin{align}
...
\end{align}
In the mixed case \eqref{eq:complex_oscillator_position_rep_mixed}, it is instead convenient to use the position representation for $x$ and the momentum representation for $y$, yielding\footnotemark[\getrefnumber{fn:order_sign_CCR_conjugate}]
...
\end{align}
Crucially, note that as an operator $\big(i\partial_z)^\dagger = i \partial_{z^*}$ since the adjoint flips the sign of derivatives $\partial_{x,y}^\dagger = -\partial_{x,y}$ wrt real variables, so that indeed the CCR are obeyeed in the forms prescribed\footnotemark[\getrefnumber{fn:order_sign_CCR_conjugate}].
...
\end{align}
However, in the former case the Hamiltonian is invariant only provided the orthogonal matrix $U$ is also \emph{real}, so that $U^\dagger U=U^TU=1$, since
\begin{align}
    ...
    &= \tfrac{1}{m} p_z^{l\,\dagger} \big(U^{\dagger} U\big)^l_k p_z^k + m \omega^2 q_z^{l\,\dagger} \big(U^{\dagger} U\big)^l_k q_z^k
...
\begin{align}
    q_\pm &= \frac{1}{\sqrt{2}}(q_x \pm q_y), \quad p_\pm = \frac{1}{\sqrt{2}}(p_x \pm p_y),\quad a_\pm = \frac{1}{\sqrt{2}}(a_x \pm a_y).
\end{align}
...
\begin{mytheorem}[Ladder operator toolkit\noteinline{Add stuff if needed}]
%
Any ladder operators $a$ and $a^\dagger$ obeying the condition $[a, a^\dagger] = 1$ enjoy the following useful properties.
\begin{itemize}
    ...
    \end{align}
    From the formula it follows immediately that $\xi \mapsto H_n(\xi)$ has parity $(-1)^n$.
    %
    ...
    \begin{align}
        a \, f(a^\dagger) |0\rangle = \tfrac{d f(a^\dagger)}{d a^\dagger} |0\rangle.
    \end{align}
    %
    ...
    \begin{align}
        ...
        \\[5pt]
        &\Rightarrow \quad e^A e^B = e^{A+B} e^{+\frac{1}{2}[A,B]} \qquad \text{if }\quad  [A,[A,B]] = [B,[A,B]] = 0\,.
    \end{align}
    since the commutator of any ladder operators $[\alpha a, \beta a^\dagger] = \alpha \beta\in\C$ is a c-number.
\end{itemize}
    %
\end{mytheorem}
...
\end{align}
Note it's not assured\footnote{A normal operator $A$ is always diagonalizable since $A+A^\dagger$ and $i(A-A^\dagger)$ are hermitian and commute, so they can be diagonalized simultaneously.} they exist since $a$ is not a \emph{normal} operator!
...
\begin{align}\label{eq:disentangle_scattering_operator_coherent_states}
...
Coherent states are not orthogonal, but the overlap of two states expnentially decays with the distance $|\alpha-\beta|$ in the complex plane of eigenvalues of $a$ up to a phase $i\text{Im}(\alpha^*\beta)$
\begin{align}
    \langle \alpha|\beta\rangle = e^{-\frac{1}{2}(|\alpha|^2 + |\beta|^2)} e^{\alpha^*\beta} = e^{-\frac{1}{2}|\alpha-\beta|^2 +\tfrac12(\alpha^*\beta - \alpha\beta^*)}.
\end{align}
Coherent states are overcomplete: they are not orthogonal but yield a resolution of the identity in the form
\begin{align}
    \int_\C \frac{d^2\alpha}{\pi} |\alpha\rangle \langle \alpha| = \mathbb{I}_\mathcal{H}.
\end{align}
The identity is most easily checked by inserting two resolutions of the identity in terms of Fock states $|n\rangle$ to the left and to the right, and using the integral identity $\int_\C\frac{d^2z}{\pi} e^{-|z|^2}\frac{(z^*)^mz^n}{\sqrt{m!\,n!}}=\delta_{m,n}$.
...
\begin{wrapfigure}[6]{r}{0.45\textwidth} % 16 = lines to wrap; adjust as needed
    \centering
    \vspace{-0.9mm}            % optional: nudge up
    \raisebox{1.5mm}{\includegraphics[width=0.4\textwidth]{images/exciting_harmonic_oscillator.png}}
    %\includegraphics[width=0.4\textwidth]{images/exciting_harmonic_oscillator.png}
    %\caption{}
    \label{fig:exciting_1d_harmonic_oscillator}
\end{wrapfigure}
...
\begin{align}\label{eq:a_in_a_out_1d_oscillator_driven}
    a(t,t_0) &= \begin{cases}
        a(t_0) e^{-i\omega (t-t_0)}=: a_{\mathrm{in}}e^{-i\omega (t-t_0)}, & t < t_i\,\\[6pt]
...
\begin{align}
    H_H(t,t_0)= \hbar \omega \left(a_{\mathrm{in}}^\dagger a_{\mathrm{in}} + \tfrac{1}{2}\right)\equiv H_S(t)\,\,\quad t < t_i, \qquad H_H(t,t_0) = \hbar \omega \left(a_{\mathrm{out}}^\dagger a_{\mathrm{out}} + \tfrac{1}{2}\right)\neq H_S(t)\,\,\,\quad t > t_f.
\end{align}
...
\noindent
\textbf{Eigenstates \& Bogoliubov transformations.}
...
\begin{align}\label{eq:disentangle_scattering_operator_driven_harmonic_oscillator_1d}
...
\end{align}
More generally the \textbf{scattering operator} is the unitary operator relating the Fock spaces and ladder operators as
\begin{align}\label{eq:scattering_operator_harmonic_oscillator_1d}
    \mathcal{S}:=\exp\left(J_0^* a_{\mathrm{out}}-J_0 a_{\mathrm{out}}^\dagger \right)\equiv \exp\left(J_0^* a_{\mathrm{in}}-J_0 a_{\mathrm{in}}^\dagger \right),\quad
    a_{\mathrm{out}} &= \mathcal{S} a_{\mathrm{in}} \mathcal{S}^\dagger, \quad |n\rangle_{\mathrm{out}} = \mathcal{S} |n\rangle_{\mathrm{in}}.
\end{align}
The identities are easily verified recalling $a_{\mathrm{out}}=a_{\mathrm{in}}+J_0$, defining $G=J_0^* a_{\mathrm{out}}-J_0 a_{\mathrm{out}}^\dagger\equiv J_0^* a_{\mathrm{in}}-J_0 a_{\mathrm{in}}^\dagger$ so that $\mathcal{S}=e^G$ and then solving the differential equation
{\small\begin{align}
    \frac{d}{d\lambda} e^{-\lambda G} a_{\mathrm{out}} e^{\lambda G} = e^{-\lambda G} [a_{\mathrm{out}},G] e^{\lambda G} = -J_0\,\, \Rightarrow \,\, e^{-\lambda G} a_{\mathrm{out}} e^{\lambda G} = a_{\mathrm{out}} - \lambda J_0 \,\, \overset{\lambda=1}{\Rightarrow} \,\,e^{-G} a_{\mathrm{out}} e^{G} = a_{\mathrm{out}} + J_0 =: a_{\mathrm{in}}.
\end{align}}

We can then compute overlaps with in- and out-states using the scattering operator or even by hand.
Fro example, the in-vacuum have overlaps and transition amplitudes to out-states given by
{\small
\begin{align}
...
%=========================================================
\subsection[Path integral formulation of the harmonic oscillator]{Path integral formulation of the harmonic oscillator\todoinline{FINISH!}}
%=========================================================
%
We now consider the path integral formulation for the driven harmonic oscillator, in increasing level of accuracy:
\begin{itemize}
    \item with wishy washy boundary conditions and canonical variables $q,p$, using naive asusmptions on the effect of sources and introducing somewhat ad hoc the Feynman $i\epsilon$ prescription to make the path integral well-defined, which will lead to the correct result but without a clear understanding of the underlying physics and mathematical subtleties,
    \item in the harmonic phase space with coherent state variables and well-defined boundary conditions, which will clarify the Feynman prescription is simply demanding $\alpha(-T)=0$ and $\alpha^*(T)=0$ that is we start from vacuum and we end up in vacuum
    \item in the Wick rotated Euclidean time and then sending $\beta\to\infty$ to project onto the vacuum, which will give a rigorous derivation of the Feynman prescription 
\end{itemize}
This will give invaluable insight into the boundary conditions.
It will also showcase the role of sources in the generating functional before turning to QFT.
...
%=========================================================
\subsection{Fermionic harmonic oscillator\todotag{FINISH!}}
%=========================================================
%
We now quickly repeat the above analysis for a fermionic harmonic oscillator, defined by the Hamiltonian
\begin{align}
    H = \hbar \omega \left(c^\dagger c - \tfrac{1}{2}\right), \quad \{c, c^\dagger\} = 1, \quad c^2 = (c^\dagger)^2 = 0.
\end{align}
We repeat all the step including the path integral formulation.
...
\begin{mytheorem}[Towards the Klein-Gordon Lagrangian]
%
...
\end{align}
The density of oscillators $\rho = \mu / a^d$ and the speed of sound $c_s = a \omega_n$, where $a$ is the lattice spacing, are kept constant as we take the continuum limit $a \to 0$.
...
\end{align}
where we defined the field as $\phi(t,\mathbf{x}) := \rho^{1/2} q_n(t)= \mu^{1/2}a^{-d/2} q_n(t)$ and the potential density as $\mathcal{V}(\phi) := \lim_{a\to 0} a^{-d} V(\rho^{-1/2} \phi)$.
Choosing a quadratic potential $V(q_n) = \tfrac{1}{2} m_\phi \mu q_n^2$ yields the Klein-Gordon lagrangian density for a free scalar field of mass $m_\phi$.


Note that we absorbed a factor $\mu^{1/2}$ in the definition of the field.
...
Below we will identify harmonic oscillator variables constituting the field as $\phi_k\sim q^\phi_k+\tfrac{1}{\omega_k} p^\phi_k$, so that $[q^\phi_k] = E^{-1/2} E^{-d/2}$ and $[p^\phi_k] = E^{1/2} E^{-d/2}$.
Since the oscillators' mass $\mu$ has already been absorbed, \emph{adimensional} density\footnote{In the sense they are 'square-root densities' in $k$-space i.e. $[\tilde{q}_k]=[\tilde{p}_k]=E^{-d/2}$} variables will then be obtained rescaling as
\begin{equation}
    \tilde{q}^\phi_k = \sqrt{\frac{\omega_k}{\hbar}} q^\phi_k, \quad \tilde{p}^\phi_k = \frac{1}{\hbar}\sqrt{\frac{\hbar}{\omega_k}} p^\phi_k,\quad
    \text{instead of the usual}\quad \tilde{q}^\phi_k = \sqrt{\frac{m\omega_k}{\hbar}} q^\phi_k,\quad \tilde{p}^\phi_k = \frac{1}{\hbar}\sqrt{\frac{\hbar}{m\omega_k}} p^\phi_k.
\end{equation}
%
\end{mytheorem}
...
On the other hand, a wave with fixed frequency oscillates while maintaining its shape (amplitude and frequency) over time, freely travelling across the lake surface without deforming.
...
This point-like detector interacts with the field, extracting information on its field amplitude at the given spacetime position.
...
Indeed, in this idealized case of perfectly frictionless water, oscillations with a given frequency will not excite oscillations with other frequencies, and the same holds for the KG field.

Finally note that in the absence of \emph{external} excitations, say `no rocks thrown into the lake', the surface of the lake--i.e. the classical field--will stay flat (or retain its initial excitation, if any).
In the quantum case, on the other hand, there will be fluctuations on top of these classical profiles due to zero-point energy of each Fourier mode, even in the absence of external perturbations.
%
\end{mytheorem}
...
It is the Euler-Lagrange equation of the (non-unique) Lagrangian density and action
\begin{equation}
...
In particular, the fundamental fields obey the following equations of motion, which are equivalent to and combine to give the Klein-Gordon equation above,
\begin{align}
...
\end{itemize}
%
The equal-time Poisson backe of the field become the equal-time canonical commutation relations
{\small\begin{align}
...
\end{align}}
%
The evolution of any operator $\mathcal{O}[\hat{\phi},\hat{\pi},t]$ expressed as a function of the fundamental fields is
...
{\small\begin{align}\label{eq:heisenberg_equation_KG_field}
...
\end{equation}
In the original formulation $|\Psi\rangle$ is a ket in a single-particle Hilbert space, which is inevitably bound to fail when dealing with systems with variable particle number.
%
However, since the Heisenberg and Schrödinger formulations of Quantum Mechanics are completely equivalent, we can still use the latter in QFT and identify a wave function obeying a suitable Schrödinger equation.
The space $\mathcal{F}=\bigoplus_{n=0}^{\infty} \mathcal{H}_1^{\otimes n}$ of states in QFT is now a Fock space, i.e. a direct sum of $n$-particle Hilbert spaces.
...
\begin{equation}\label{eq:hamiltonian_fourier_space}
...
However something seems slightly off track, and requires some attention.
\begin{itemize}
    \item The field $\phi_k$ is no more self-adjoint, since $\phi_k^{\dagger} = \phi_{-k}$, whereas we would rather our DoFs be observables.
    In fact the $\phi_k$ are \emph{normal} operators, hence they actually are observables with complex eigenvalues, even though this is not very intuitive. 
    \item The $\delta$ functions in the CCR is $\delta^{(3)}(\mathbf{k} + \mathbf{k}')$ instead of the expected $\delta^{(3)}(\mathbf{k} - \mathbf{k}')$, suggesting that the conjugate momentum of the mode $\phi_k$ is actually $\pi_{-k}$ instead of $\pi_k$.
...
The third one is expected, since we are considering infinite volume and thus a continuum of modes $k$.
...
The first two issues have more physical relevance and are still related.
We can fix them by explicitly rewriting $\phi_k$ as linear combination of \emph{physical} self-adjoint harmonic oscillators $\{q_k,p_k\}_k$ to highlight the true DoFs of the theory, rather than \emph{complex} oscillators $\{\phi_k,\pi_k\}_k$.
...
{\small\begin{align}\label{eq:IR_reg_replacements}
...
\begin{align}
    ...
    \\
    H&= \frac{1}{V} \sum_k \hbar \omega_k \tfrac12\left( a_k^{\dagger} a_k + a_k a_k^\dagger \right).
\end{align}
%
...
\end{align}
The \textbf{energy dimensions of operators/variables and their mutual relations \eqref{eq:relations_fields_creation_operators_harmonic_oscillator_var}  stay unchanged}, including:
\begin{itemize}
    \item fields $\phi_k,\,  \pi_k$,
    \item ladder operators $a_k,\, a_k^\dagger$,
    \item dimensional  $q_k,\,p_k$, and adimensional oscillators $\tilde{q}_k,\,\tilde{p}_k$.
\end{itemize}
...
\end{align}}
In particular ladder operators $a_k$ and $a_k^{\dagger}$ would become dimensionless and fields, Hamiltonian and commutation relations would change as
{\small
\begin{align}
    ...
    \\
    H&= \sum_k \tfrac{1}{2}\,\pi_k^\dagger\pi_k + \tfrac{1}{2}\omega_k^2 \,\phi_k^\dagger \phi_k =\sum_k \hbar \omega_k \left( a_k^{\dagger} a_k + a_k a_k^\dagger \right),\qquad [\phi_k,\pi_k^\dagger]=i\hbar\delta_{k,k'}\,\quad[a_{k}, a_{k'}^{\dagger}] = \delta_{k,k'}.
\end{align}
}
%
...
\end{align}
Note the overall factor $\frac{1}{2}$ in \eqref{eq:hamiltonian_fourier_space} cancels since modes $\pm k$ contribute the same amount to the Hamiltonian upon commuting $f_kf_k^\dagger=f_k^\dagger f_k$ for $k\neq0$ and the shorthand $k\geq0$ means the integral/sum is only over half of momentum space, e.g. $k_1\geq0$, to avoid double counting.

These are exactly complex harmonic oscillators \eqref{eq:complex_harmonic_oscillator} with complex CCR $[q_z, p_z^*] = i\hbar$ since $[\phi_k, \pi_k^\dagger]=[\phi_k, \pi_{-k}] = i\hbar \delta_0$.
...
Namely, for each mode $k$ define, a truly complex operator tracking both $\phi_k$ and $\pi_k$ as a linear combination for suitable constants $\alpha_k, \beta_k\in\R$,
\begin{align}
    a_k=\alpha_k \phi_k + i \beta_k \pi_k, \quad \Rightarrow\quad [a_k,a_{k'}^\dagger]= 2\alpha_k\beta_k\hbar (2\pi)^3 \delta^{(3)}(k-k')\,.
\end{align}
Demanding ladder operators satisfy the easiest CCR $[a_k,a_{k'}^\dagger]=\hbar (2\pi)^3 \delta^{(3)}(k-k')$ fixes $\beta_k=\frac{1}{2\alpha_k}$.
Finally requiring the Hamiltonian be diagonal in terms of $a_k^\dagger a_k$ fixes $\alpha_k=\alpha_{-k}=\sqrt{\frac{\omega_k}{2}}$ and thus recovers formula  \eqref{eq:ladder_operators_field}.
...
\begin{align}\label{eq:decompsition_field_ladder_operators_fixed_times}
    ...
    = \int \frac{d^3{k}}{(2\pi)^3} \frac{1}{\sqrt{2\omega_k}} \left( a_k e^{i \mathbf{k}\cdot\mathbf{x}} + a_k^{\dagger} e^{- i \mathbf{k}\cdot\mathbf{x}} \right),
    \\
    \hat{\pi}(x,t_0) &= \int \frac{d^3{k}}{(2\pi)^3}\, e^{i{k}\cdot{x}} \hat{\pi}_k(t_0)
    = \int \frac{d^3{k}}{(2\pi)^3} \frac{1}{\sqrt{2\omega_k}} \left( -i \omega_k a_k e^{i \mathbf{k}\cdot\mathbf{x}} + i \omega_k a_k^{\dagger} e^{- i \mathbf{k}\cdot\mathbf{x}} \right).
\end{align}
...
For example, we could choose a different reference time $t_0$. However, for a free theory in flat spacetime, this simply corresponds to a phase shift in the definition of creation and annihilation operators, leaving the vacuum invariant.
On the other hand, there are truly inequivalent choices of creation and annihilation operators, for instance mixing $a_k$ and $a_{-k}^\dagger$ as dsicussed in Section \ref{sec:harmonic_oscillators}, and this in turn changes the vacuum of the theory, i.e. the state $|0\rangle$ annihilated by all $a_k$ operators.
This is not a problem in flat spacetime, where Poincaré invariance selects a preferred vacuum, which crucially also coincides with the ground state of the Hamiltonian, but it becomes a serious issue in curved spacetime, where no preferred vacuum exists in general. 
%
\end{mytheorem}
...
\end{align}
The probability in the abstract function space is explicitated as the probability density of finding position values between $f'(x)$ and $f'(x) + df'(x)$ or Fourier modes between $f'_k$ and $f'_k + d f'_k$,
\begin{align}
...
\end{align}
Obvious caveats apply on ill-defined squares of Dirac deltas, possibly also infinite-dimensional in the function space of field configurations, and infinite products.
...
Using these two bases for the Hilbert space of states is called the \emph{position} or \emph{momentum representation} respectively.
...
In ordinary QM, operators are labelled by $1\dots n$ and bra-kets by $x\in \mathbb{R}^n$, whereas in QFT operators are labelled by $\mathbf{x}\in \mathbb{R}^d$ and bra-kets by functions $f:\mathbb{R}^d \to \mathbb{R}$.

We remark that nobody really knows what is the \emph{space of field configurations}, i.e. the function spaces $W_\phi$ and $W_\pi$ the functions $f$ and $g$ belong to, i.e. the possible collections $\{f(x)\}_x$ or $\{g(x)\}_x$ of $\hat{\phi}$ or $\hat{\pi}$ eigenvalues.
...
The state $|f\rangle$ is also an eigenstate of the Fourier transformed field operators $\hat{\phi}_k(t)$ with its Fourier modes $f_k$ as eigenvalues.
...
The above is made rigorous by introducing suitable infrared regularization, making Fourier modes countable discrete, and ultraviolet cutoff, discarding Fourier modes above some scale $\Lambda$, thereby making the function space finite-dimensional.
%
...
\begin{align}
    ...
    \dim|\{f_x\}_x \rangle= \prod_x [f_x]^{-\nicefrac{1}{2}} = [f_x]^{-\frac{1}{2}\underset{x}{\sum}1},\, \dim|\{f_k\}_k \rangle= \prod_k [f_k]^{-1} = [f_k]^{-\underset{k}{\sum}1}.
\end{align}
}
...
\end{aligned}
\end{align}
We could equivalently perform adimensional rescaling using position values $f(x)$, but Fourier modes make it more rigorous upon IR regularization.
...
\end{align}
%
As $f$ varies in the (unknown, ill-defined) configuration space $W_{\phi}$, the states $|f\rangle$ form an \emph{improper} \emph{orthonormal} basis of the Fock space $\mathcal{F}\simeq L^2(W_{\phi})$.
...
The $\hat{\phi}$ and $\hat{\pi}$ \textbf{representations are simply related by a functional Fourier transform}, just like position and momentum representations in ordinary QM.
Namely, we pass from one basis to the other via a resolution of the identity
\begin{align}
    |g\rangle &= \int_{W_{\phi}}\!\!\! Df\,\, |f\rangle\, \langle f | g \rangle
    = \int_{W_{\phi}}\!\!\! Df\,\, |f\rangle\, \,\,\mathcal{N} e^{\frac{i}{\hbar} \int d^3 x\, f(x) g(x)}.
\end{align}
%
The above is confirmed by acting with $\hat{\pi}(t_0,x)=-i\hbar\frac{\delta\,\cdot}{\delta f(x)}$ on $|g\rangle$ in the $\hat{\phi}$representation, i.e. on $\langle f | g\rangle$, or with $\hat{\phi}(t_0,x)=i\hbar\frac{\delta\,\cdot}{\delta g(x)}$ on $|f\rangle$ in the $\hat{\pi}$ representation, and possibly integrating by parts in the functional integral.
%
...
\end{align}
%
Implementing the $\hat{\phi}$ and $\hat{\pi}$ operators in any of the two ways indeed reproduces the canonical commutation relations in both real and momentum space (easy check) and thus provides two \emph{unitaryly equivalent} representations of the operator algebra on the Fock space $\mathcal{F}\simeq L^2(W_{\phi})\simeq L^2(W_{\pi})$.
The unitary transformation between the two $L^2$ spaces is induced give by the funtional Fourier transform between the two spaces $W_{\phi}$ and $W_{\pi}$, as discussed above.
...
\begin{wrapfigure}[16]{r}{0.45\textwidth} % 16 = lines to wrap; adjust as needed
    \centering
    \vspace{-0.5\baselineskip}            % optional: nudge up
...
Before discussing the particle interpretation of field excitations, we must clarify the Lorenz invariance of the theory, since particles are ultimately defined as irreducible representations of the Poincaré group.


\begin{mytheorem}[Relativistically normalized states \& Lorentz covariance of the Klein-Gordon field]
States created by acting on the vacuum with ladder operators are not normalized in a Lorentz-invariant way.
Indeed, due to the commutation relation we have overlap
\begin{align}
    \langle 0 | a_k a_{k'}^{\dagger} |0\rangle = (2\pi)^3 \delta^{(3)}(\bar{k} - \bar{k}')\,.
\end{align}
The Lorentz-invariant measure is {\small$d^3\bar{k}/(2\pi)^3 2E_k$}, and the Lorentz-invariant Dirac delta is then {\small$2E_k (2\pi)^3 \delta^{(3)}(\bar{k} - \bar{k}')$}. 
We thus define the relativistically normalized states as
\begin{align}
    |\bar{k}\rangle := \sqrt{2E_k} a_k^{\dagger} |0\rangle, \qquad \langle \bar{k} | \bar{k}' \rangle = 2E_k (2\pi)^3 \delta^{(3)}(\bar{k} - \bar{k}')\,.
\end{align}
States with multiple quanta are simply rescaled correspondingly as 
\begin{align}
    |{n}_{k_1}.. {n}_{k_j}.. \rangle := \prod_{k} \tfrac{(2E_k)^{\frac{n_k}{2}}}{\sqrt{n_k!}} (a_k^{\dagger})^{n_k} |0\rangle, \quad \langle {m}_{k_1}.. {m}_{k_j}.. | {n}_{k_1}..{n}_{k_j}.. \rangle = \prod_k \delta_{m_k, n_k}\left( 2E_k(2\pi)^3\delta^{(3)}(0) \right)^{n_k}.
\end{align}
The unitary representation of the Poincaré group on the Fock space {\small$\mathcal{P}\curvearrowright\mathcal{F}$} must then map such relativistically normalized states into each other according to the given transformation.
Lorentz transformations $\Lambda: x^\mu \mapsto \Lambda^\mu_{\,\nu} x^\nu$ should modify momentum accordingly $k^\mu \mapsto \Lambda^\mu_{\,\nu} k^\nu$ so as to preserve inner products, while translations $T_c:x^\mu\mapsto x^\mu + c^\mu$ should leave states invariant up to a phase.
Since this must be a symmetry of the theory, if states transform as $|\chi\rangle \mapsto U(g) |\chi\rangle$, operators must transform as $O \mapsto U(g) O U(g)^{-1}$ so as to preserve expectation values $\langle \chi | O | \chi \rangle$.
For Lorentz transformations we thus immediately find the condition
\begin{align}
    U(\Lambda)\, \sqrt{2 E_k}a_k\, U(\Lambda)^\dagger = \sqrt{2 E_{\Lambda k}} \,\,a_{\Lambda k}.
\end{align}
The specific phase for translations is found by recalling that the space (resp. time) translation operator is generated by the momentum operator (resp. Hamiltonian), which are diagonal in the Fock basis,
\begin{align}
    U(T_c)= e^{\tfrac{i}{\hbar}P^\mu c_\mu}, \quad P^\mu= (H, \bar{P})=\sum_{{k}}k^\mu a_k^{\dagger} a_k, \quad k^\mu=(E_k, \bar{k}).
\end{align}
The phase is then simply $U(T_c) a_k U(T_c)^\dagger = e^{-i k^\mu c_\mu} a_k$.
At the level of states this corresponds to
\begin{align}
    U(\Lambda)|{n}_{k_1}\dots {n}_{k_j}\dots \rangle = |{n}_{\Lambda k_1}\dots {n}_{\Lambda k_j}\dots \rangle, \qquad
    U(T_c)|{n}_{k_1}\dots {n}_{k_j}\dots \rangle = e^{i c^\mu \sum_{k} n_k k_\mu} |{n}_{k_1}\dots {n}_{k_j}\dots \rangle.
\end{align}
Notably, this immediately allows to recast the field decomposition \eqref{eq:time_evolution_free_field} in a manifestly Lorentz-covariant way, namely
\begin{align}
    \hat{\phi}(x) = \int \frac{d^3\bar{k}}{(2\pi)^3} \frac{1}{\sqrt{2E_k}} \left( a_k e^{-ikx} + a_k^{\dagger} e^{ikx} \right)
    = \int \frac{d^3\bar{k}}{(2\pi)^32E_k}\left( \sqrt{2E_k} a_k e^{-ikx} + \sqrt{2E_k} a_k^\dagger e^{ikx} \right).
\end{align}
It is then immeadiate to verify that the field operator indeed transforms as a scalar under the action of the Poincar\'e group.
For translations we have
\begin{align}\label{eq:poincare_translation_transformation_operator}
    U(T_c) \hat{\phi}(x) U(T_c)^\dagger &= \int \frac{d^3\bar{k}}{(2\pi)^32E_k}\left( \sqrt{2E_k} a_k e^{-ik(x+c)} + \sqrt{2E_k} a_k^\dagger e^{ik(x+c)} \right) = \hat{\phi}(x+c).
\end{align}
For Lorentz transformations, using the invariance of the measure $\tfrac{d^3\bar{k}}{2E_k}=\tfrac{d^3\Lambda k}{2E_{\Lambda k}}$ and the inner product $k^\mu \eta_{\mu\nu} x^\nu =(\Lambda k)^\mu \eta_{\mu\nu} (\Lambda x)^\nu$, we have
{\small
\begin{align}
    U(\Lambda) \hat{\phi}(x) U(\Lambda)^\dagger &= \int \frac{d^3\bar{k}}{(2\pi)^32E_k}\left( \sqrt{2E_{\Lambda k}} a_{\Lambda k} e^{-i k\cdot x} + \sqrt{2E_{\Lambda k}} a_{\Lambda k}^\dagger e^{i k \cdot x} \right)
    \\
    &= \int \frac{d^3\Lambda k}{(2\pi)^32E_{\Lambda k}}\left( \sqrt{2E_{\Lambda k}} a_{\Lambda k} e^{-i \Lambda k \cdot\Lambda x} + \sqrt{2E_{\Lambda k}} a_{\Lambda k}^\dagger e^{i \Lambda k \cdot \Lambda x} \right)
    = \hat{\phi}(\Lambda x).
\end{align}}
Finally we stress that spin-fields will transform as in \eqref{eq:poincare_translation_transformation_operator} under translations, but will involve a matrix part for Lorentz transformations according to their representation of the Lorentz group.
For example (m,n)-tensor fields
{\small\begin{align}
    \hat{T}^{\mu_*}_{\nu_\bullet}(x) = \int \frac{d^3\bar{k}}{(2\pi)^3} \frac{1}{\sqrt{2E_k}} \Big( \epsilon^{\mu_*}_{\nu_\bullet}(k)\,b^{\mu_*}_{\nu_\bullet,k} e^{-ikx} + \epsilon^{\mu_*}_{\nu_\bullet}(k)^\dagger\,b^{\mu_*, \, \dagger}_{\nu_\bullet,k} e^{ikx} \Big)
    \mapsto U(\Lambda)\hat{T}(x)U(\Lambda)^\dagger = \Lambda^{\mu_*}_{\alpha_*} \Lambda_{\nu_\bullet}^{\beta_\bullet} \,\,\hat{T}^{\alpha_*}_{\beta_\bullet}(\Lambda x)\,,
\end{align}}
and spin-$\tfrac12$ fields
{\small\begin{align}
    \hat{\psi}(x) = \int \frac{d^3\bar{k}}{(2\pi)^3} \frac{1}{\sqrt{2E_k}}\sum_r \Big( u_r(k)c_r(k) e^{-ikx} + v_r(k)d_r^{\dagger}(k) e^{ikx} \Big)
    \mapsto U(\Lambda)\hat{\psi}(x)U(\Lambda)^\dagger = S(\Lambda)\,\,\hat{\psi}(\Lambda x)\,.
\end{align}}
%
\end{mytheorem}
...
%======================================================================
\subsection{Excitations \& coherent states}
%======================================================================
...
\begin{mytheorem}[Intuition for the Unruh effect: pulling a cork across the lake \& melting ice in vacuum\questiontag{Polish}]
%
Consider attaching a cork to a string and pulling it across the surface of a calm lake.
If we pull the cork gently and slowly--inertial uniform motion--it will follow the motion of the string without creating any ripples on the water surface.
However, if we pull the cork abruptly and quickly--noninertial accelerated motion-- it will create ripples that propagate away from it.
The faster we pull the cork, the more ripples we create.

\begin{wrapfigure}[16]{r}{0.3\textwidth} % 14 = lines to wrap; adjust as needed
  \centering
  \vspace{-0.5\baselineskip}            % optional: nudge up
  \includegraphics[width=0.3\textwidth,height=3.5cm]{images/unruh effect/cork_pull.png}\\[1pt]
  \includegraphics[width=0.3\textwidth,height=3.5cm]{images/unruh effect/cork_pull_spring.png}
  %\includegraphics[width=0.30\textwidth,height=5cm,keepaspectratio]{images/lake_fourier.png}
  %\caption{...}
\end{wrapfigure}
...
However, if we pull the cork abruptly and quickly, the ripples created on the water surface will interact with the internal DoFs of the cork and excite them to higher energy levels, also making the cork vibrate.
...
A particle in inertial motion through the vacuum (or a definite particle state) of a quantum field will not detect any particles (resp. detect the given number of particles), since the field is in its ground state and there are no excitations to interact with.
However, an accelerated particle will interact with the field in a non-trivial way, causing excitations and detecting particles even if the field is in its vacuum state (resp. a different number of parrticles then the inertial observer would detect).
%

\begin{wrapfigure}[7]{r}{0.4\textwidth} % 16 = lines to wrap; adjust as needed
    \centering
    %\vspace{-0.9mm}            % optional: nudge up
    \includegraphics[width=0.25\textwidth]{images/unruh effect/melting_ice.png}
    %\caption{}
    \label{fig:melting_ice_unruh_effect}
\end{wrapfigure}
An even more simple analogy for the Unruh effect is: a cube of ice following inertial motion through vacuum remains solid because there is no particle to interact with, while an accelerated cube of ice will detect particles and melt due to interactions with them (even if the inertial observer confirm there are no particles the cube could interact with). 
\end{mytheorem}


\begin{mytheorem}[What is a particle? Unruh - DeWitt detectors\questiontag{Polish}]
%
So far we have identified particles with quanta created/annihilated by ladder operators $a_k^{(\dagger)}$ associated to a given mode decomposition of the field.
However we would rather identify particles as entities \emph{localized in space} rather than propagating waves extended over all space.

\begin{wrapfigure}[4]{r}{0.1\textwidth} % 14 = lines to wrap; adjust as needed
  \centering           % optional: nudge up
  \includegraphics[width=0.1\textwidth]{images/unruh effect/atom.png}
  %\caption{...}
\end{wrapfigure}
Unruh and DeWitt proposed a simple \emph{operational definition of particles} based on the interaction between a quantum field and a localized quantum system acting as a particle detector, such as an atom, an oscillator like a diatomic molecule like $H_2$ or another small quantum system with internal DoFs.
A particle is then defined as the spacetime-localized excitation of the detector caused by its interaction with the quantum field.
According to this defintion, particles can be detected by at least 4 mechanisms:
\begin{itemize}
    \item the field is in an excited state wrt the detector's frame of reference;
    \item the detector is accelerated wrt the field's vacuum state;
    \item the detector is `switched on/off' non-adiabatically i.e. in a finite time; due to the energy-time uncertainty principle, this can excite the detector even if the field is in its vacuum state and the detector is inertial, since we have an uncertainty in the energy exchanged;
    \item the spacetime is curved, causing different observers to disagree on the particle content of a given state.
\end{itemize}
This is clarified in the analysis of the Unruh effect below.
\end{mytheorem}

\begin{mytheorem}[The Unruh effect\todotag{Expand final part \& polish}]
%
For simplicity consider the detector in 1st quantization and the field in 2nd quantization, with total Hibert space $\mathcal{H}_{\phi} \otimes \mathcal{H}_{\mathrm{det}}$.

Consider Minkowsky space with inertial coordinates $x^{\mu}=(t,\mathbf{x})$, and a scalar quantum field $\hat{\phi}(t_0,\mathbf{x})$ with free Hamiltonian $H_{\phi}$ in such coordinates.
The freely time-evolved field thus solves the Heisenberg equation $ih\partial \phi(t,\mathbf{x}) = [\phi(t,\mathbf{x}), H_{\phi}]$ wrt the inertial time $t$ (and states similarly solve the Schrödinger equation wrt time $t$), and its mode decomposition wrt such inertial coordinates is given by \eqref{eq:time_evolution_free_field}.

The detector is a localized quantum system with wordline $x^\mu(\tau)$ parametrized by its proper time $\tau$.
Let  $H_{\mathrm{det}}$ be the free Hamiltonian of the detector wrt its proper time $\tau$, so that its the Schrödinger and Heisenberg equations holds wrt $H_{\mathrm{det}}$ and its own proper time $\tau$. 

We use the proper time of the detector $\tau$ as time parameter, and for simplicity reference time $\tau_0$ such that $x^0(\tau_0)=t_0$.
This is allowed as long as we use parameter $\tau$ and $H_{\mathrm{det}}$ to evolve the detector operators and states, and parameter $t=t(\tau)$ and $H_{\phi}$ to evolve the field operators and states, and we ultimately evolve up to same corresponding times $\tau^*$ or $t^*=t(\tau^*)$.
Indeed the Hamiltonians $H_{\mathrm{det}}$ and $H_\phi$ are written wrt time $\tau$ and $t$ respectively and thus are generators of time translations wrt paramter $\tau$ and $t=x^0$ respectively.

To fix ideas, let the free Hamiltonian and the corresponding lagrangian of the detector written wrt its proper time $\tau$ be
\begin{align}
    H^\tau_{\mathrm{det}} = \tfrac{1}{2m} \pi_\tau^2- \tfrac12 m\omega^2 q^2 \quad \Rightarrow\quad \tfrac{dq}{d\tau}:= \tfrac{\partial H^\tau_{\mathrm{det}}}{\partial \pi_\tau} = \tfrac{\pi_\tau}{m}, \quad L^\tau_{\mathrm{det}} = \tfrac12 m \left(\tfrac{dq}{d\tau}\right)^2 + \tfrac12 m\omega^2 q^2.
\end{align}
The action must be invariant under coordinate reparametrizations, thus changing measure from proper time $\tau$ to inertial time $t$ in the action, we obtain the lagrangian wrt inertial time $t$ and then the corresponding conjugate variable $\pi_t$ and hamiltonian
{\small
\begin{align}
    S_{\mathrm{det}} = \int d\tau\, L^\tau_{\mathrm{det}} = \int dt\,\tfrac{d\tau}{dt} L^\tau_{\mathrm{det}}
    \quad\Rightarrow\quad
    & L^t_{\mathrm{det}} = \tfrac{d\tau}{dt} L^\tau_{\mathrm{det}}=
    \tfrac{d\tau}{dt} \left(\tfrac12 m \left(\tfrac{dq}{d\tau}\right)^2 + \tfrac12 m\omega^2 q^2\right)
    =  \tfrac12 m \left(\tfrac{dq}{dt}\right)^2 \tfrac{dt}{d\tau} + \tfrac12 m\omega^2 q^2 \tfrac{d\tau}{dt}\\
    &\Rightarrow\quad  \pi_t = \tfrac{dL^t_{\mathrm{det}}}{d(dq/dt)} = m \tfrac{dq}{dt} \tfrac{dt}{d\tau}\equiv \pi_\tau,\quad
    H^t_{\mathrm{det}} = \pi_t \tfrac{dq}{dt} - L^t_{\mathrm{det}} =... = \tfrac{d\tau}{dt} H^\tau_{\mathrm{det}}.
\end{align}
}
The conjugate field wrt the two time coordinates turns out to be the same, while the hamiltonians are related by a time-dilation factor, with $H^\tau_{\mathrm{det}}$ time independent and $H_{\mathrm{det}}^t$ in general time-dependent due to the non-trivial $\frac{d\tau}{dt}$ accounting for the detector's motion.
The free time evolution from time $\tau_0= \tau(t_0)$ to $\tau=\tau(t)$ is thus equivalently written wrt either time coordinate as
{\small
\begin{align}
    U_{\mathrm{det}}(\tau,\tau_0) &= \exp\left(-\frac{i}{\hbar} \int_{\tau_0}^{\tau} d\tau' H_{\mathrm{det}}^\tau\right) = \exp\left(-\frac{i}{\hbar} \int_{t(\tau_0)}^{t(\tau)} dt' \tfrac{d\tau}{dt}H_{\mathrm{det}}^\tau\right) = \exp\left(-\frac{i}{\hbar} \int_{t(\tau_0)}^{t(\tau)} dt' H_{\mathrm{det}}^t\right) = U_{\mathrm{det}}^t(t,t_0).
\end{align}
}
The same argument holds for the time evolution of the field wrt either time coordinate using the corresponding hamiltonian $H_\phi:=H_\phi^t$ or $H_\phi^\tau = \tfrac{dt}{d\tau} H_\phi^t$.

The interaction-picture Hamiltonian, \emph{already freely time-evolved} up to time $\tau$ is then
\begin{align}
    \hat{H}^I_{\mathrm{tot}}(\tau)= 1 \otimes \hat{H}_{\phi} + \hat{H}_{\mathrm{det}} \otimes 1 + \hat{H}_{\mathrm{int}}^I(\tau),\quad\text{with}\quad \hat{H}^I_{\mathrm{int}}(\tau) = \epsilon(\tau) \,\,\hat{Q}^I(\tau) \otimes \phi^I(x^0(\tau),\mathbf{x}(\tau))\,,
\end{align}
where $\hat{H}_{\phi}$ and $\hat{H}_{\mathrm{det}}$ are the free hamiltonians, time-independent and thus with trivial free time evolution, and each operator is time-evolved wrt the total free Hamiltonian\footnote{We suppress the reference time i.e. $\hat{\mathcal{O}}^I(\tau)\equiv \hat{\mathcal{O}}^I(\tau,\tau_0)$ for brevity.}
{\small
\begin{align}
    \mathcal{O}\mapsto \mathcal{O}^I(\tau)\!=\! \left(e^{\frac{i}{\hbar} (\tau-\tau_0)\hat{H}_{\mathrm{det}}}\otimes 1 + 1 \otimes e^{\frac{i}{\hbar}(x^0(\tau)-x^0(\tau_0)) \hat{H}_{\phi}}\right) \,\mathcal{O} \left(e^{-\frac{i}{\hbar} (\tau-\tau_0)\hat{H}_{\mathrm{det}}}\otimes 1 + 1 \otimes e^{-\frac{i}{\hbar}(x^0(\tau)-x^0(\tau_0)) \hat{H}_{\phi}}\right).
\end{align}
}
Note we use different parameters $\tau$ and $t$ for the time evolution operators as explained above, but we ultimately evolve up to the same time $\tau$ and $x^0(\tau)$.

In the interaction term, the self-adjoint operator $\hat{Q}^I(\tau)$ is some (freely time-evolved) observable of the detector coupled to the field at the detector's spacetime positions $x^\mu(\tau)$, and $\epsilon(\tau)$ parametrizes the interaction strength (i.e. the detector efficiency), possibly switched on/off in a finite time interval.
We consider a coupling to $\phi$ for simplicity, but couplings to any other observable of the field are possible e.g. to the conjugate field $\pi$. 
For example, in the case of the electromagnetic field realistic interactions would be with the momentum or the spin of the detector
\begin{align}
    H_{\mathrm{int}}=-\frac{e}{mc} \hat{p}_j\otimes \hat{A}^j(x(\tau))\,,\quad H_{\mathrm{int}} =\hat{s}_j \otimes \hat{B}^j(x(\tau))\,.
\end{align}
For simplicity we also assume the spectrum of $\hat{H}_{\mathrm{det}}$ is discrete with eigenstates $|E_n\rangle$ and eigenvalues $E_n$.

In the interaction picture, using the proper time $\tau$ of the detector as time parameter, the evolution operator is then
\begin{align}
    U_I(\tau_2,\tau_1) = \mathcal{T} \exp\left(-\frac{i}{\hbar} \int_{\tau_1}^{\tau_2} d\tau' \hat{H}_{\mathrm{int}}^I(\tau')\right)\quad\text{with\, $\hat{H}_{\mathrm{int}}^I(\tau')$\,  given above.}
\end{align}
Consider a pure initial state $|\Psi_i\rangle = |\psi_{\mathrm{det}}\rangle \otimes |\psi_{\phi}\rangle$ at time $\tau_i$.
To 1st order in perturbation theory, the final state at time $\tau_f$ is
\begin{align}
    |\Psi(\tau_f)\rangle_I &= U_I(\tau_f,\tau_i) |\Psi(\tau_i)\rangle \approx \left(1 -\frac{i}{\hbar} \int_{\tau_i}^{\tau_f} d\tau' \hat{H}_{\mathrm{int}}^I(\tau')\right) |\psi_{\mathrm{det}}\rangle \otimes |\psi_{\phi}\rangle \\
    &= |\psi_{\mathrm{det}}\rangle \otimes |\psi_{\phi}\rangle - \frac{i}{\hbar} \int_{\tau_i}^{\tau_f} d\tau' \epsilon(\tau') \,\,\,\hat{Q}^I(\tau') |\psi_{\mathrm{det}}\rangle \otimes \hat{\phi}^I(x^\mu(\tau')) |\psi_{\phi}\rangle.
\end{align}
Up to an irrelevant overall phase depending on the initial time $\tau_0$, the freely time-evolved field is\footnote{Here we denote $k_\mu x^\mu(\tau)= \omega_k\cdot(x^0(\tau)\!-\!\tau_0)- \vec{k}\cdot \vec{x}(\tau)$} 
\begin{align}
    \phi^I(x^\mu(\tau)) &= e^{\frac{i}{\hbar} \hat{H}_{\phi} (x^0(\tau)-x^0(\tau_0))} \hat{\phi}(\mathbf{x}(\tau)) e^{-\frac{i}{\hbar} \hat{H}_{\phi} (x^0(\tau)-x^0(\tau_0))} = \int \frac{d^3\mathbf{k}}{(2\pi)^3} \frac{1}{\sqrt{2\omega_k}} \left( a_k e^{-i k_\mu x^\mu(\tau)} + a_k^{\dagger} e^{i k_\mu x^\mu(\tau)} \right).
\end{align}
The transition amplitude to a final state $|\Psi_f\rangle = |\psi_{\mathrm{det},f}\rangle \otimes |\psi_{\phi,f}\rangle$ is thus
{\small
\begin{align}
    \langle\Psi_f | U_I(\tau_f,\tau_i) | \Psi_i\rangle &= - \frac{i}{\hbar} \int_{\tau_i}^{\tau_f} d\tau' \epsilon(\tau') \,\,\,\langle \psi_{\mathrm{det},f} | \hat{Q}^I(\tau') |\psi_{\mathrm{det}}\rangle \,\, \langle \psi_{\phi,f} | \hat{\phi}^I(x^\mu(\tau')) |\psi_{\phi}\rangle
    \\
    &=
    - \frac{i}{\hbar} \int_{\tau_i}^{\tau_f} d\tau' \epsilon(\tau') \,\,\langle \psi_{\mathrm{det},f} | e^{\frac{i}{\hbar}(\tau'-\tau_0)}\hat{Q} e^{-\frac{i}{\hbar}(\tau'-\tau_0)} |\psi_{\mathrm{det}}\rangle \\
    &\quad\qquad\times \int \frac{d^3\mathbf{k}}{(2\pi)^3} \frac{1}{\sqrt{2\omega_k}} \left(\langle \psi_{\phi,f} | a_k| \psi_{\phi,i}\rangle  e^{-i k_\mu x^\mu(\tau')} + \langle \psi_{\phi,f} | a_k^\dagger| \psi_{\phi,i}\rangle e^{i k_\mu x^\mu(\tau')} \right)\,.
\end{align}
}
If we consider energy eigenstates $|\psi_{\mathrm{det}}\rangle = |E_n \rangle$, $|\psi_{\mathrm{det},f}\rangle = |E_m \rangle$ as initial and final states of the detector, the matrix element of $\hat{Q}^I$ is
\begin{align}
    \langle E_m | \hat{Q}^I(\tau',\tau_0) | E_n \rangle &= e^{\frac{i}{\hbar} (E_m - E_n)(\tau'-\tau_0)} \langle E_m | \hat{Q}(0) | E_n \rangle.
\end{align}
If we further specialize to some detector wordline $\tau \to x^\mu(\tau)$ and field states $|\psi_{\phi,i\,(f)}\rangle$ we can compute the transition amplitude explicitly...
\medskip

\noindent
\textbf{Inertial motion.}
Up to a Lorentz transformation, which leaves the field decomposition \emph{formally} unchanged, the detector wordline is $x^\mu(\tau) = (\tau,0,0,0)$.
The amplitude is then
\begin{align}
    \langle\Psi_f | U_I(\tau_f,\tau_i) | \Psi_i\rangle 
    &=
    - \frac{i}{\hbar}\langle E_m | \hat{Q}(0) | E_n \rangle \left(\int \frac{d^3\mathbf{k}}{(2\pi)^3} \frac{\langle \psi_{\phi,f} | a_k| \psi_{\phi,i}\rangle}{\sqrt{2\omega_k}}  \int_{\tau_i}^{\tau_f}\!\! d\tau' \epsilon(\tau') e^{i \left(\frac{E_m-E_n}{\hbar} -\omega_k \right) (\tau'-\tau_0)} \right.\\
    &\qquad\qquad \qquad\qquad\left. +\int \frac{d^3\mathbf{k}}{(2\pi)^3} \frac{\langle \psi_{\phi,f} | a^\dagger_k| \psi_{\phi,i}\rangle}{\sqrt{2\omega_k}} \int_{\tau_i}^{\tau_f}\!\! d\tau' \epsilon(\tau') e^{i \left(\frac{E_m-E_n}{\hbar} +\omega_k \right) (\tau'-\tau_0)}\right)\,.
\end{align}
In the limit where $\tau_f-\tau_i \to \infty$ and the detector is switched on adiabatically $\epsilon(\tau)\to 1$, the time integrals yield delta functions enforcing energy conservation, and we thus get
{\small
\begin{align}
    \lim_{\substack{\tau_f-\tau_i \to \infty \\ \epsilon\to 1}} \langle\Psi_f | U_I(\tau_f,\tau_i) | \Psi_i\rangle =
    - \frac{i}{\hbar}\langle E_m | \hat{Q}(0) | E_n \rangle &\left(\int \frac{d^3\mathbf{k}}{(2\pi)^3}  \frac{\langle \psi_{\phi,f} | a_k| \psi_{\phi,i}\rangle}{\sqrt{2\omega_k}}\,\, (2\pi)\,\,\delta_D^{(1)}\!\left(\tfrac{E_m-E_n}{\hbar} -\omega_k\right)\right.\\
    &\left.\qquad + \int \frac{d^3\mathbf{k}}{(2\pi)^3} \frac{\langle \psi_{\phi,f} | a^\dagger_k| \psi_{\phi,i}\rangle}{\sqrt{2\omega_k}}\,\, (2\pi)\,\,\delta_D^{(1)}\!\left(\tfrac{E_m-E_n}{\hbar} +\omega_k\right)\right)\,.
\end{align}
}
In this limit only the first (resp. second) term contributes if $E_m > E_n$ (resp. $E_m < E_n$), corresponding to the detector absorbing (resp. emitting) a field quantum of fixed mode $k_\Delta$ such that $ |E_m - E_n| \equiv \hbar\,  \omega_{k_\Delta}$.
The amplitude $\propto\, \langle \psi_{\phi,f} | a_k^{(\dagger)}| \psi_{\phi,i}\rangle$ is modulated by the overlap between the initial and final field states after removing (resp. adding) the given quantum.
For example, consider relativistically normalized Fock states for the field 
\begin{align}
    |\{n_k\}_k\rangle := \prod_k \frac{\big(\sqrt{2\omega_k}\,\,a_k^\dagger\big)^{n_k}}{\sqrt{n_k!}} |0\rangle,\quad \text{so that}\quad \langle \mathbf{m} |\mathbf{n}\rangle = \prod_k \delta^{\mathrm{Kro}}_{m_k,n_k}\,\Big((2\omega_k)\, (2\pi)^3 \delta_D^{(3)}(k\!-\!k)\Big)^{n_k},.
\end{align}
If the final state has only one less quantum in a given mode $k_*$, i.e. $|\psi_{\phi,i}\rangle = |\{n_k\}_k\rangle$ and $|\psi_{\phi,f}\rangle = |\{n_k - \delta_{k,k_*}\}_k\rangle$, we have
\begin{align}
    \langle \psi_{\phi,f} | a_{k}| \psi_{\phi,i}\rangle = \sqrt{2\omega_{k_*}}\sqrt{n_{k_*}} \,\,(2\pi)^3 \delta_D^{(3)}(k-k_*) \langle \psi_{\phi,f} | \psi_{\phi,f}\rangle,\quad \langle \psi_{\phi,f} | a_{k}^\dagger| \psi_{\phi,i}\rangle=0.
\end{align}
Using $\delta_D^{(3)}(k-k_*)$ to kill the $d^3k$ integral, the amplitude becomes
\begin{align}
    \lim_{\substack{\tau_f-\tau_i \to \infty \\ \epsilon\to 1}} \langle\Psi_f | U_I(\tau_f,\tau_i) | \Psi_i\rangle &=
    - \frac{i}{\hbar}\langle E_m | \hat{Q}(0) | E_n \rangle  \sqrt{n_{k_*}} \langle \psi_{\phi,f} | \psi_{\phi,f}\rangle(2\pi)\,\,\delta_D^{(1)}\!\!\left(\tfrac{E_{m}-E_n}{\hbar}-\omega_{k_*}\right).
\end{align}
The transition amplitude is thus zero unless $k_*$ is such that $\hbar \omega_{k_*} = \sqrt{k_*^2+m^2} \overset{!}{=} E_m - E_n$, i.e. the detector absorbs a quantum of the field with just the right energy to jump from level $E_n$ to $E_m$. 
Analogous formulas hold if $E_m < E_n$.
\medskip

\noindent
\textbf{Time-energy uncertainty \& non-adiabatic switching.}
If the detector is switched on/off non-adiabatically in a finite time interval $\Delta\tau$, even for inertial motion there is a non-zero probability of excitation/de-excitation of the detector due to the time-energy uncertainty principle.
Roughly speaking, if the interaction is switched on-off in a finite time $\Delta \tau$, there is an uncertainty in the energy exchanged $\Delta E \gtrsim \hbar/\Delta \tau$, allowing the detector to be excited/de-excited even if the field stays in its vacuum state and the detector is inertial.
The interpretation is that such energy is not created/absorbed from nowhere, but rather is absorbed/taken from the switching process.

This is manifest in the above formulas since the time integrals do not yield perfect delta functions enforcing energy conservation even waiting an infinite time, but rather functions of $k$ peaked around the resonance frequencies $\omega_k = \pm (E_m - E_n)/\hbar$ with a width $\Delta \omega \sim 1/\Delta\tau$.
For example, considering a box switching function $\epsilon(\tau) = 1_{[\tau_i,\tau_f]}(\tau)$, the time integrals yield sinc functions 
\begin{align}
    \int_{-\infty}^{+\infty}\!\!\!\! d\tau' \epsilon(\tau') e^{i \left(\tfrac{E_m-E_n}{\hbar} \pm\omega_k \right) (\tau'-\tau_0)}
    \!= \!\!\int_{\tau_i}^{\tau_f} \!\!\!d\tau' e^{i \left(\tfrac{E_m-E_n}{\hbar} \pm\omega_k \right) (\tau'-\tau_0)}
    \!=\! (\tau_f\!-\!\tau_i)\,\frac{\sin\left( \nu_k\frac{(\tau_f-\tau_i)}{2}\right)}{ \nu_k\frac{(\tau_f-\tau_i)}{2}}\, e^{i\nu_k\frac{(\tau_f-\tau_0)+(\tau_i-\tau_0)}{2}},
\end{align}
with $\nu_k^{\pm} := \left(\frac{E_m-E_n}{\hbar} \pm \omega_k\right)$.
The sinc functions peaks for $\nu_k^\pm=0$, that is $k$ such that $\omega_k = \pm (E_m - E_n)/\hbar$, but it has finite width $\Delta \omega \sim 1/(\tau_f-\tau_i)$ that allows contributions from transitions $\langle \psi_{\phi,f} | a_k^{(\dagger)}| \psi_{\phi,i}\rangle$ away from the resonance frequencies.
\medskip



\noindent
\textbf{Uniformly accelerated motion \& finite-temperature effects.}\todotag{Check \& Finish}
The above analysis can be extended to arbitrary wordlines $x^\mu(\tau)$.
It is particularly interesting to consider a uniformly accelerated detector with proper acceleration $a$, following the Rindler trajectory (cf. Birrell \& Davies, pg. 52-58)
{\small
\begin{align}
    x^\mu(\tau) = \left(\frac{c^2}{a}\sinh\left(\frac{a \tau}{c}\right), \frac{c^2}{a}\cosh\left(\frac{a \tau}{c}\right),0,0\right).
\end{align}
}
In this case, even if the field is in its vacuum state $|\psi_{\phi,i}\rangle = |0\rangle$, the detector has a non-zero probability of being excited to higher energy levels $|\psi_{\mathrm{det},f}\rangle = |E_m\rangle$ due to its interaction with the vacuum fluctuations of the quantum field along its accelerated trajectory.
For example, considering the detector in its ground state $|E_0\rangle$, tracing over all 1-particle final states of the field $|\psi_{\phi,f}\rangle=\sqrt{2\omega_k}a_k^\dagger|0\rangle$ and using $\sum_k \to V\int \frac{d^3\mathbf{k}}{(2\pi)^3}$, and letting $\tau_f-\tau_i\to \infty$ with adiabatic switching $\epsilon(\tau)\to 1$, the transition probability is
{\small
\begin{align}
    P_{E_0\to E_m} &= \sum_{k} \left|\langle E_m | \otimes \langle \sqrt{2\omega_k} a_k^\dagger | U_I(\tau_f,\tau_i) | 0 \rangle \otimes | E_0 \rangle\right|^2 \\
    &= \frac{1}{\hbar^2} |\langle E_m | \hat{Q}(0) | E_0 \rangle|^2\, V\int \frac{d^3\mathbf{k}}{(2\pi)^3} \left| \int_{-\infty}^{+\infty} d\tau' e^{\frac{i}{\hbar}E_{m0} \tau'} e^{i k_\mu x^\mu(\tau')} \right|^2 \\
    &= \frac{1}{\hbar^2} |\langle E_m | \hat{Q}(0) | E_0 \rangle|^2 \,V \int \frac{d^3\mathbf{k}}{(2\pi)^3} \left| \int_{-\infty}^{+\infty} d\tau'e^{\frac{i}{\hbar}E_{m0} \tau'} e^{\frac{i}{a}\left(\omega_k \sinh\left(a \tau'\right) - k_x \cosh\left(a \tau'\right)\right)} \right|^2.
\end{align}
}
Considering a massless scalar field for simplicity $\omega_k=|k|$, the integral can be computed exactly.
The transition probability results to be proportional to a Bose-Einstein distribution
{\small
\begin{align}
    P_{E_0\to E_m} &\propto \frac{1}{e^{E_m/(k_B T_U)} - 1}\,,\quad k_BT_U = \frac{\hbar a}{2\pi c},
\end{align}
}
characteristic of a thermal bath of particles at the Unruh temperature $T_U = \frac{\hbar a}{2\pi k_B c}$.
This is the essence of the Unruh effect: an accelerated observer perceives the vacuum state of a quantum field as a thermal state with a temperature proportional to its acceleration.
In terms of the initial analogy: an accelerated cube of ice will melt even through vacuum.
%
\end{mytheorem}


\begin{mytheorem}[Reinterpreting Unruh effect: relating mode decompositions in two different frames\todotag{Polish, expand \& finish eventually}]
%
The Unruh effect can alternatively be understood in terms of two different mode decompositions of the quantum field in different frames related by non-inertial transformations, e.g. an inertial frame and a uniformly accelerated one, with corresponding non-trivial Bogoliubov transformations relating them.
Indeed the decomposition \eqref{eq:time_evolution_free_field} is invariant under global Lorentz transformations since the modes $k^\mu=(\omega_k,\mathbf{k})$ are 4-vectors, but not under general non-inertial transformations.
A uniformly accelerated observer will naturally use Rindler coordinates $(\eta,\xi,y,z)$, related to inertial ones $(t,x,y,z)$ by
\begin{align}
    t = \xi \sinh(\eta),\quad x = \xi \cosh(\eta),\quad y=y,\quad z=z, \quad \xi \in (0,+\infty),\quad \eta \in (-\infty,+\infty).
\end{align}

\begin{wrapfigure}[4]{r}{0.45\textwidth} % 16 = lines to wrap; adjust as needed
    \centering
    %\vspace{-0.9mm}            % optional: nudge up
    \includegraphics[width=0.3\textwidth]{images/unruh effect/modes_accelerated_frame.png}
    %\caption{}
    \label{fig:modes_accelerated_frame}
\end{wrapfigure}
Performing this change of coordinates, the accelerated observer will decompose the field in different modes and the relation between these two sets of modes will change over time, leading to mixing and excitations in the mode decomposition of the accelerated observer even if the field stays in vacuum for the inertial one.
Namely, a definite mode for the inertial observer is not monocromatic for the accelerated one, but rather a time-dependent superposition of modes.

Note this is somewhat different from the previous analysis.
Indeed above we considered \emph{only the wordline} of the detector and showed that it affects the interaction with the field, causing excitations even if the field is in its vacuum state for the inertial observer.
Here we are considering an entirely new frame, as if the accelerated observer had its own set of coordinates covering a portion of Minkowsky space, and thus its own mode decomposition of the field.
The Unruh effect can thus be reinterpreted as the fact that the vacuum state (resp. other states) for the inertial observer is not the vacuum state (resp. the same state) for the accelerated one, due to the non-trivial relation between the two mode decompositions.
Specifically, just like we found for the detector energy levels, one would find that the occupation numbers of the modes for the accelerated observer follow a thermal distribution at the Unruh temperature $T_U$ when the field is in its inertial vacuum state.
%
\end{mytheorem}
\end{document}
