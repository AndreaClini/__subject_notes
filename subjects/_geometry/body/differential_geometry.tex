...
For a metric connection $\nabla g\equiv 0$ possibly with non-zero torsion, the \textbf{Koszul formula} holds
\begin{align}
\begin{aligned}
    2\,g(\nabla_X Y, Z) ={}& X\!\left[g(Y,Z)\right] + Y\!\left[g(Z,X)\right] - Z\!\left[g(X,Y)\right] + g([X,Y],Z) - g([Y,Z],X) - g([X,Z],Y) \\
    & - g\!\left(T(X,Y),Z\right) - g\!\left(T(Y,Z),X\right) - g\!\left(T(X,Z),Y\right).
\end{aligned}
\end{align}
We then express the Christoffel symbols in terms of the metric $g_{ij} := g(e_i, e_j)$, the structure constants \(C^i_{\, jk}:=\theta^k([e_i,e_j])\) and the torsion components \(T^i_{\, jk}:=\theta^i(T(e_j,e_k))\) in a given tetrad as
\begin{align}
    \Gamma_{ij}^k=&\tfrac{1}{2} g^{k\ell} \Big( e_i\,g_{j \ell} + e_j\,g_{i \ell} - e_\ell\,g_{i j}\Big)
    + \tfrac{1}{2} g^{k\ell} \Big( g_{m \ell} C^m_{ij} - g_{m j} C^m_{i \ell} - g_{m i} C^m_{j \ell} \Big) \\[5pt]
    &\quad
...
If $C_{ij}^k\equiv 0$ we recover usual expression for the Christoffel symbols in a coordinate basis\footnote{In fact we have $C_{ij}^k\equiv 0$ iff there exist some coordinates s.t. $e_\mu=\partial_\mu$.}, split into symmetric $\propto \, \partial g$ and antisymmetric part $\propto \, T$ part.
Finally in an orthonormal tetrad $g_{ij} = \eta_{ij}$  and the 3 first terms $\sim \, \partial g$ vanish.
\end{mytheorem}
\begin{proof}
By metric compatibility $\nabla g=0$, that is $0=X(g(Y,Z)) - g(\nabla_X Y,Z) - g(Y, \nabla_X Z)$.
By definition of torsion $T(X,Y) = \nabla_X Y - \nabla_Y X - [X,Y]$ so that $\nabla_X Y = \nabla_Y X + [X,Y] + T(X,Y)$.
Using these two realtions we get the Koszul formula.
...
\begin{mytheorem}[Bianchi identities on the tangent bundle \& recovering coordinate formalism\todotag{Check and finish}]
...
\begin{enumerate}
    ...
    \item curvature 2-form on arbitrary fiber bundle OK
...
\begin{enumerate}
    ...
    \item Bianchi identities on the tangent bundle MISSING
