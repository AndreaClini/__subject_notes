...
Consider a field theory in curved spacetime with action, where $\{\phi_a\}_a$ denotes any multiplet of fields with arbitrary spin
\begin{align}
    S[\phi] = \int \mathrm{d}^d x \sqrt{|g|} \, \mathcal{L}(\phi_a, \partial_\mu \phi_a, g_{\mu\nu}).
\end{align}
The equations of motion for the fields are obtained varying with respect to $\phi_a$,
{\small
\begin{align}
...
The action for a vector field $A_\mu \,dx^\mu$ (electromagnetic potential) in curved spacetime is given by
\begin{align}
    S_\text{Maxwell} = -\frac{1}{4} \int_M \mathrm{d}^d x \sqrt{|g|} \, F_{\mu\nu} F^{\mu\nu}
    = -\frac{1}{4}\int_M\!\! F \wedge \star F = -\frac{1}{4}\,\langle dA, dA\rangle_{\mathrm{Hodge}}\,,
\end{align}
where $F=dA$ is the field strength 2-form, with coordinates\footnote{Recall the exterior derivative can be written exchanging partial with covariant derivatives.} $F_{\mu\nu} = \nabla_\mu A_\nu - \nabla_\nu A_\mu=\partial_\mu A_\nu - \partial_\nu A_\mu$.
Varying this action with respect to $A\mapsto A+\epsilon $ yields 
\begin{align}
    \delta S = -\frac{1}{2}\langle dA, d\epsilon \rangle_{\mathrm{Hodge}} = -\frac{1}{2} \langle \delta dA, \epsilon \rangle_{\mathrm{Hodge}} = -\frac{1}{2} \int_M \delta d A\wedge \star \epsilon,
\end{align}
where we integrated by parts using the exterior co-derivative $\delta$ i.e. the adjoint of $d$ wrt the Hodge inner product.
The Maxwell equations in curved spacetime are then
\begin{align}
    \delta d A = \delta F = 0, \quad \text{or in a chart} \quad \nabla_\mu F^{\mu\nu} = 0.
\end{align}
%
These are \emph{not} to be confused with the Bianchi identity\footnote{Note we can exchange partial with covariant derivatives since both expressions are tensorial and coincide in normal coordinates at a point, as we already discussed in general for the exterior derivative of any $p$-form.}
\begin{align}
    dF=0\quad \text{or in a chart} \quad  0=\partial_{[\alpha} F_{\beta\gamma]}\equiv \nabla_{[\alpha} F_{\beta\gamma]}.
\end{align}

If we couple the vector field to a conserved current $J^\mu$ (i.e. $\nabla_\mu J^\mu=0$), we can add an interaction term $S_\text{int} = \int \mathrm{d}^d x \sqrt{|g|} A_\mu J^\mu$ to the action, yielding the inhomogeneous Maxwell equations \todotag{Do better this part, writing lagrangian for scalar QDE}
\begin{align}
    \delta F = J\,,\quad \text{or in a chart} \quad \nabla_\mu F^{\mu\nu} = J^\nu.
\end{align}
%
\end{mytheorem}
