...
%
Consider the complex harmonic oscillator with Hamiltonian
\begin{equation}
    H = \tfrac{1}{2m}(p_x^2 + p_y^2) + \tfrac{m\omega^2}{2} (x^2 + y^2), \quad \{x, p_x\} = \{y, p_y\} = 1.
\end{equation}
\end{mytheorem}
...
\begin{align}\label{eq:IR_reg_replacements}
    ...
    (2\pi)^d \delta^{(d)}_D(\mathbf{k} - \mathbf{k}') \equiv \int_V d^dx e^{i\mathbf{x}\cdot(\mathbf{k} - \mathbf{k}')} \,\,\Longrightarrow \,\,V \delta_{k,k'}\,,\quad \frac{\delta}{\delta f_k}\,\,\Longrightarrow \,\, V \frac{\partial}{\partial f_k}\,.
\end{align}
...
\end{align}
%
The same is true for the Fourier transformed $\hat{\phi}_k(t)$ and $\hat{\pi}_k(t)$, since they are just linear combinations of the $\hat{\phi}(x)$ and $\hat{\pi}(x)$ operators, with multplication and functional derivatives wrt the conjugate of $f_k$ and $g_k$ respectively\footnote{Recall $f_k^*=f_{-k}$ and $g_k^*=g_{-k}$ by reality of the fields.}
\begin{align}
    \hat{\phi}_k &\to f_k\, \cdot, \quad
    \hat{\pi}_k \to -i \hbar (2\pi)^d \,\frac{\delta}{\delta f_{-k}}\,\quad \text{on $\hat{\phi}$ eigenstates }\, |f\rangle, \\
    \hat{\pi}_k &\to g_k\, \cdot, \quad
    \hat{\phi}_k \to i \hbar (2\pi)^d \,\frac{\delta}{\delta g_{-k}}\,\quad \text{on $\hat{\pi}$ eigenstates } \,|g\rangle.
\end{align}
%
Note the dimensions of functional derivatives in real and momentum space are
\begin{align}
    \left[\frac{\delta}{\delta f(x)}\right]= \left[\frac{1}{V}\frac{\partial}{\partial f(x)}\right]= L^{-d} [f(x)]^{-1}, \quad
    \left[\frac{\delta}{\delta f_k}\right]= \left[V \frac{\partial}{\partial f_k}\right]= L^{d}\, [f_k]^{-1}\,.
\end{align}
%
More generally, for any functional $\Psi[f] = \langle f | \Psi \rangle$ of the field consifgurations $f$, respectively of $g$, we have
{\small 
\begin{align}
    ...
    \langle g | \hat{\phi}(x) | \Psi \rangle = i \hbar \frac{\delta}{\delta g(x)} \Psi[g],\quad \langle g | \hat{\phi}_k | \Psi \rangle = i \hbar (2\pi)^d\frac{\delta}{\delta g_k^*} \Psi[g].
\end{align}
}
%
...
{\small
\begin{align}
    &\hat{H} = \int d^d x\,\tfrac{1}{2}\left( \hat{\pi}^2 +  (\nabla \hat{\phi})^2 + m^2 \hat{\phi}^2 \right)
    = \int \frac{d^d k}{(2\pi)^d}\,\tfrac{1}{2}\left(\hat{\pi}_k \hat{\pi}_{-k} + (|\mathbf{k}|^2 + m^2) \hat{\phi}_k \hat{\phi}_{-k}\right)
    \\
    \to \int &d^d x\, \tfrac{1}{2} \left( -\hbar^2 \frac{\delta^2}{\delta f(x)^2} + (\nabla_x f(x))^2 + m^2 f(x)^2 \right)
    \\ 
...
Crucially, if we use some infrared regularization, Fourier modes are countable discrete and we have the replacements \eqref{eq:IR_reg_replacements}.
%
\end{mytheorem} 
