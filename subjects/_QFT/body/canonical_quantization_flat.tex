...
It is particularly interesting to consider a uniformly accelerated detector with proper acceleration $a$, following the Rindler trajectory (see Birrell \& Davies, pg. 52-58)
{\small
\begin{align}
...
The transition probability results to be proportional to a Planckian distribution
{\small
\begin{align}
...
In terms of the initial analogies: an accelerated cube of ice will melt even if it is in vacuum.
%
\end{mytheorem}


\begin{mytheorem}[Another interpretation of Unruh effect: particles as mode decomposition in two different frames\todotag{Expand \& finish}]
%
The Unruh effect can also be understood in terms of two different mode decompositions of the quantum field, associated to two different frames of reference: an inertial frame and a uniformly accelerated frame.
...
A uniformly accelerated observer will naturally use Rindler coordinates $(\eta,\xi,y,z)$, related to inertial coordinates $(t,x,y,z)$ by
\begin{align}
    t = \xi \sinh(\eta),\quad x = \xi \cosh(\eta),\quad y=y,\quad z=z, \quad \xi \in (0,+\infty),\quad \eta \in (-\infty,+\infty).
\end{align}
Performing such change of coordinates the accelerated observer will decompose the field in different modes and the relation between these two sets of modes will change over time, leading to mixing and excitations in the mode decomposition of the accelerated observer even if the field stays in vacuum for the inertial one.\todotag{add image Kempf 8}
\end{mytheorem}