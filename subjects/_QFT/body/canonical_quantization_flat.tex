...
\begin{mytheorem}[Identifying ladder operators\questiontag{check}]
%
Ladder operators are a convenient way deal with harmonic oscillators: they diagonalize the Hamiltonian and yield a single \emph{complex} DoF $a$ to track instead of the two real canonical variables $q$ and $p$. 
The definition of ladder operators can be identified by direct inspection of the Hamiltonian after recaling canonical variables to adimensional ones as above.
...
Let us proceed more generally, simply defining a complex operator $a$ tracking both canonical variables as a linear combination for arbitrary constants $\alpha,\beta\in\R$
\begin{align}
    ...
    \end{cases}
\end{align}
We first choose $\beta=\beta(\alpha)$ to enforce convenient CCR for $a,a^\dagger$, and then fix $\alpha$ to make (somebody's) Hamiltonian as simple as possible.
Namely we choose
\begin{align}
...
\end{align}
In particular we see that for suitable $J$ and $K$ it might be convenient to define the ladder operators in some other way e.g. shifting them by suitable terms in case $J$ and $K$ do not depend on them.
\end{mytheorem}
...
\begin{align}
    q_z = \frac{1}{\sqrt{2}}(q_x + i q_y), \quad p_z = \frac{1}{\sqrt{2}}(p_x \mp i p_y), \quad \{q_z, p_z^{(\dagger)}\} = 1\,.
\end{align}
or as\footnotemark[\getrefnumber{fn:order_sign_CCR_conjugate}]
\begin{align}
    q_z = \frac{1}{\sqrt{2}}(q_x + \tfrac{i}{{m\omega}}\, p_y), \quad p_z = \frac{1}{\sqrt{2}}(p_x \pm i {m\omega}\,q_y), \quad \{q_z, p_z^{(\dagger)}\} = 1\,.
\end{align}
All choices lead to the same Hamiltonian expression
\begin{align}
    H = \tfrac{1}{m} p_z p_z^{\dagger} + m \omega^2 q_z q_z^{\dagger}\,.
\end{align}
In practice one often starts from a complex harmonic oscillator obeying the CCR with/out conjugation
\begin{align}
    \{q_z, p_z\} = 1, \quad \text{or}\quad \{q_z, p_z^{\dagger}\} = 1\,.
\end{align}
Then identify corresponding real variables\footnotemark[\getrefnumber{fn:order_sign_CCR_conjugate}] either expressing complex position (resp. momentum) as combination of real positions (resp. momenta) as
\begin{align}
...
We now consider a \emph{complex} position representation, we would like to identify\footnotemark[\getrefnumber{fn:order_sign_CCR_conjugate}] $q_z\sim z$ and respectively $p_z \sim \mp i\hbar \partial_z^{(*)}$.
...
%
Coherent states $|\alpha\rangle$ of the quantum harmonic oscillator are defined as the eigenstates of the annihilation operator $a$.
Note we are not assured they exist since $a$ is not a \emph{normal} operator...
\end{mytheorem}
...
\begin{itemize}
    ...
    In fact the $\phi_k$ are \emph{normal} operators, so they actually are observables with complex eigenvalue, even though it is not very intuitive. 
...
\begin{align}\label{eq:IR_reg_replacements}
    \int_V \frac{d^d k}{(2\pi)^d} &\,\,\Longrightarrow \,\,\frac{1}{V} \sum_k\,, \quad
    (2\pi)^d \delta^{(d)}_D(\mathbf{k} - \mathbf{k}') \equiv \int_V d^dx e^{i\mathbf{x}\cdot(\mathbf{k} - \mathbf{k}')} \,\,\Longrightarrow \,\,V \delta_{k,k'}\,,\quad \frac{\delta}{\delta f_k}\,\,\Longrightarrow \,\, \frac{V}{(2\pi)^d} \frac{\partial}{\partial f_k}\,.
\end{align}
...
The fields and Hamiltonian would then read\todotag{insert (q,p) expr}
{\small
\begin{align}
    \phi(t,\mathbf{x})&= \frac{1}{\sqrt{V}} \sum_k e^{i \mathbf{k}\cdot\mathbf{x}} \phi_\mathbf{k}(t)=\frac{1}{\sqrt{V}} \sum_k e^{i \mathbf{k}\cdot\mathbf{x}} \frac{1}{\sqrt{2\omega_k}} \left( a_k e^{i \mathbf{k}\cdot\mathbf{x}} + a_k^{\dagger} e^{- i \mathbf{k}\cdot\mathbf{x}} \right),\quad \phi_k(t) = \frac{1}{\sqrt{V}} \int d^3 x\, e^{-i \mathbf{k} \cdot \mathbf{x}} \phi(t, \mathbf{x}),
    \\
    \pi(t,\mathbf{x})&= \frac{1}{\sqrt{V}} \sum_k e^{i \mathbf{k}\cdot\mathbf{x}} \pi_\mathbf{k}(t)=\frac{1}{\sqrt{V}} \sum_k e^{i \mathbf{k}\cdot\mathbf{x}} \frac{1}{\sqrt{2\omega_k}} \left( -i \omega_k a_k e^{i \mathbf{k}\cdot\mathbf{x}} + i \omega_k a_k^{\dagger} e^{- i \mathbf{k}\cdot\mathbf{x}} \right),\quad \pi_k(t) = \frac{1}{\sqrt{V}} \int d^3 x\, e^{-i \mathbf{k} \cdot \mathbf{x}} \pi(t, \mathbf{x}).
    \\
    H&= \sum_k \tfrac{1}{2}\,\pi_k^\dagger\pi_k + \tfrac{1}{2}\omega_k^2 \,\phi_k^\dagger \phi_k =\sum_k \hbar \omega_k \left( a_k^{\dagger} a_k + \tfrac{1}{2} \right).
\end{align}
}
%
...
\begin{mytheorem}[Observable DoFs of the Klein-Gordon field in flat spacetime.\todotag{Fix}]
%
For a free theory in flat spacetime, we naively decompose the field operators at an arbitrary reference \emph{fixed} time $t$ in terms of creation and annihilation operators as
\begin{align}
...
\begin{aligned}
    ...
    \\
    &= \frac{1}{2\sqrt{\omega_k}}\Big[ \big(\tilde{q}_k+\tilde{q}_{-k}\big) + i \big(\tilde{p}_k - \tilde{p}_{-k}\big) \Big]
    = \tfrac{1}{2}\Big[ \big({q}_k+{q}_{-k}\big) + \tfrac{i}{\omega_k}\big({p}_k - {p}_{-k}\big) \Big]
    \\
    \pi_k(t_0) &= -i \sqrt{\frac{\omega_k}{2}} (a_k - a_{-k}^{\dagger}) \\
    &= \frac{\sqrt{\omega_k}}{2} \Big(\tilde{p}_k + \tilde{p}_{-k}\Big) - i \frac{\sqrt{\omega_k}}{2} \Big( \tilde{q}_k- \tilde{q}_{-k} \Big)
...
\end{align}
%
The same is true for the Fourier transformed $\hat{\phi}_k(t)$ and $\hat{\pi}_k(t)$, since they are just linear combinations of the $\hat{\phi}(x)$ and $\hat{\pi}(x)$ operators, with multiplication and \emph{conjugate} differentiation wrt $f_k$ and $g_k$\footnote{Recall $f_k^*=f_{-k}$ and $g_k^*=g_{-k}$ by reality of the fields.}
\begin{align}
...
\end{align}
In particular, in the $\hat{\phi}$ and $\hat{\pi}$ representation, in position and momentum space respectively, the Hamiltonian reads\footnote{Recall $\hat{\phi}^\dagger_k=-\hat{\phi}_k$, $\hat{\pi}^\dagger_k=-\hat{\pi}_k$ and $f_k^* = f_{-k}$, $g_k^* = g_{-k}$ by self-adjoint, and each element commutes with its adjoint.}
{\small
\begin{align}
