...
%
Consider a localized quantum system with a wordline $x(\tau)$ parametrized by its proper time $\tau$.
For simplicity we consider the detector in 1st quantization and the field in 2nd quantization.
The total space is $\mathcal{H}_{\phi} \otimes \mathcal{H}_{\mathrm{det}}$ with total hamiltonian
\begin{align}
    \hat{H}_{\mathrm{tot}}= 1 \otimes \hat{H}_{\phi} + \hat{H}_{\mathrm{det}} \otimes 1 + \hat{H}_{\mathrm{int}},\quad\text{with}\quad \hat{H}_{\mathrm{int}} = \epsilon(\tau) \,\,\hat{Q}(\tau) \otimes \phi(x(\tau)),
\end{align}
where $\hat{H}_{\phi}$ and $\hat{H}_{\mathrm{det}}$ are the free hamiltonians, the self-adjoint operator $\hat{Q}(\tau)$ is some observable of the detector coupled to the field at the detector's spacetime positions $x(\tau)$, and $\epsilon(\tau)$ parametrizes the interaction strength (i.e. the detector efficiency), possibly switched on/off in a finite time interval.
...
Use the proper time of the detector $\tau$ as time parameter.
We work in the interaction picture, with time evolution operator
\begin{align}
    U_I(\tau_2,\tau_1) = \mathcal{T} \exp\left(-\frac{i}{\hbar} \int_{\tau_1}^{\tau_2} d\tau' \hat{H}_{\mathrm{int}}^I(\tau',\tau_0)\right), \quad \hat{H}_{\mathrm{int}}^I(\tau,\tau_0) = e^{\frac{i}{\hbar} \hat{H}_0 (\tau-\tau_0)} \hat{H}_{\mathrm{int}}(\tau) e^{-\frac{i}{\hbar} \hat{H}_0 (\tau-\tau_0)}.
\end{align}
...
\begin{align}
    |\Psi(\tau_f)\rangle_I &= U_I(\tau_f,\tau_i) |\Psi(\tau_i)\rangle \\
    &\approx \left(1 -\frac{i}{\hbar} \int_{\tau_i}^{\tau_f} d\tau' \hat{H}_{\mathrm{int}}^I(\tau',\tau_0)\right) |\psi_{\mathrm{det}}\rangle \otimes |\psi_{\phi}\rangle \\
    &= |\psi_{\mathrm{det}}\rangle \otimes |\psi_{\phi}\rangle - \frac{i}{\hbar} \int_{\tau_i}^{\tau_f} d\tau' \epsilon(\tau') \,\,\,\hat{Q}^I(\tau',\tau_0) |\psi_{\mathrm{det}}\rangle \otimes \hat{\phi}^I(x(\tau'),\tau_0) |\psi_{\phi}\rangle.
\end{align}
...
\begin{align}
    \phi^I(x(\tau),\tau_0) &= e^{\frac{i}{\hbar} \hat{H}_{\phi} (x^0(\tau)-\tau_0)} \hat{\phi}(\vec{x}(\tau)) e^{-\frac{i}{\hbar} \hat{H}_{\phi} (x^0(\tau)-\tau_0)} = \int \frac{d^3\bar{k}}{(2\pi)^3} \frac{1}{\sqrt{2\omega_k}} \left( a_k e^{-i k_\mu x^\mu(\tau)} + a_k^{\dagger} e^{i k_\mu x^\mu(\tau)} \right).
\end{align}
The transition amplitude to a final state $|\Psi_f\rangle = |\psi_{\mathrm{det},f}\rangle \otimes |\psi_{\phi,f}\rangle$ is thus
{\small
\begin{align}
    \langle\Psi_f | U_I(\tau_f,\tau_i) | \Psi_i\rangle &= - \frac{i}{\hbar} \int_{\tau_i}^{\tau_f} d\tau' \epsilon(\tau') \,\,\,\langle \psi_{\mathrm{det},f} | \hat{Q}^I(\tau',\tau_0) |\psi_{\mathrm{det}}\rangle \,\, \langle \psi_{\phi,f} | \hat{\phi}^I(x(\tau'),\tau_0) |\psi_{\phi}\rangle
    \\
    &=
    ...
    &\quad\qquad\times \int \frac{d^3\bar{k}}{(2\pi)^3} \frac{1}{\sqrt{2\omega_k}} \left(\langle \psi_{\phi,f} | a_k| \psi_{\phi,i}\rangle  e^{-i k_\mu x^\mu(\tau')} + \langle \psi_{\phi,f} | a_k^\dagger| \psi_{\phi,i}\rangle e^{i k_\mu x^\mu(\tau')} \right)\,.
\end{align}
}
...
Up to a Lorentz transformation, we can take the detector wordline to be $x^\mu(\tau) = (\tau-\tau_0,0,0,0)$.
The amplitude is then
\begin{align}
    \langle\Psi_f | U_I(\tau_f,\tau_i) | \Psi_i\rangle 
    &=
    - \frac{i}{\hbar}\langle E_m | \hat{Q}(0) | E_n \rangle \left(\int \frac{d^3\bar{k}}{(2\pi)^3} \frac{1}{\sqrt{2\omega_k}} \langle \psi_{\phi,f} | a_k| \psi_{\phi,i}\rangle \int_{\tau_i}^{\tau_f}\!\! d\tau' \epsilon(\tau') e^{i \left(\frac{E_m-E_n}{\hbar} -\omega_k \right) (\tau'-\tau_0)} \right.\\
    &\qquad \qquad\qquad\left. +\int \frac{d^3\bar{k}}{(2\pi)^3} \frac{1}{\sqrt{2\omega_k}} \langle \psi_{\phi,f} | a^\dagger_k| \psi_{\phi,i}\rangle \int_{\tau_i}^{\tau_f}\!\! d\tau' \epsilon(\tau') e^{i \left(\frac{E_m-E_n}{\hbar} +\omega_k \right) (\tau'-\tau_0)}\right)\,.
\end{align}
...
\begin{align}
    ...
    - \frac{i}{\hbar}\langle E_m | \hat{Q}(0) | E_n \rangle &\left(\int \frac{d^3\bar{k}}{(2\pi)^3}  \frac{\langle \psi_{\phi,f} | a_k| \psi_{\phi,i}\rangle}{\sqrt{2\omega_k}}\,\, (2\pi)\,\,\delta_D\!\left(\tfrac{E_m-E_n}{\hbar} -\omega_k\right)\right.\\
    &\left.\qquad + \int \frac{d^3\bar{k}}{(2\pi)^3} \frac{\langle \psi_{\phi,f} | a^\dagger_k| \psi_{\phi,i}\rangle}{\sqrt{2\omega_k}}\,\, (2\pi)\,\,\delta_D\!\left(\tfrac{E_m-E_n}{\hbar} +\omega_k\right)\right)\,.
\end{align}
}
In this limit only the first (resp. second) term contributes if $E_m > E_n$ (resp. $E_m < E_n$), corresponding to the detector absorbing (resp. emitting) a field quantum of fixed mode $k_\Delta$ such that $ |E_m - E_n| \equiv \hbar\,  \omega_{k_\Delta} = \sqrt{k_\Delta^2+m^2}$.
...
For example, considering relativistically normalized Fock states  
\begin{align}
...
\end{align}
with final state with only one less quantum in mode $k_\Delta$, i.e. $|\psi_{\phi,i}\rangle = |\{n_k\}_k\rangle$ and $|\psi_{\phi,f}\rangle = |\{n_k - \delta_{k,k_\Delta}\}_k\rangle$, we have
\begin{align}
    \langle \psi_{\phi,f} | a_{k}| \psi_{\phi,i}\rangle = \frac{\sqrt{2\omega_{k_\Delta}}}{\sqrt{n_{k_\Delta}}} (2\pi)^3 \delta_D(k_\Delta-k_\Delta) \langle \psi_{\phi,f} | \psi_{\phi,f}\rangle\,.
\end{align}
Integrating over the solid angle and using the delta function to perform the radial integral, we finally get
\begin{align}
    \lim_{\substack{\tau_f-\tau_i \to \infty \\ \epsilon\to 1}} \langle\Psi_f | U_I(\tau_f,\tau_i) | \Psi_i\rangle &=
    - \frac{i}{\hbar}\langle E_m | \hat{Q}(0) | E_n \rangle \frac{(2\pi)^3 \delta_D(k_\Delta-k_\Delta)}{\sqrt{n_{k_\Delta}}} \langle \psi_{\phi,f} | \psi_{\phi,f}\rangle\\
    &\qquad\times\int \frac{d\Omega}{(2\pi)^3}\int dk\, k^2 \,\, (2\pi)\,\,\delta_D\!\left(E_{mn}/\hbar-\omega_k\right)\\
    &= -i \langle E_m | \hat{Q}(0) | E_n \rangle \frac{(2\pi)^3 \delta^{(3)}_D(k_\Delta-k_\Delta)}{\sqrt{n_{k_\Delta}}} \langle \psi_{\phi,f} | \psi_{\phi,f}\rangle \frac{E_{\Delta}\,\sqrt{E_{\Delta}^2-m^2}}{\pi}\,,
\end{align}
where $E_{\Delta} := |E_m - E_n|$ and $k_\Delta := \sqrt{E_{\Delta}^2 - m^2}$. Analogous formulas hold if $E_m < E_n$.
\medskip

\noindent
\textbf{Time-energy uncertainty \& non-adiabatic switching.}\todotag{Finish}
If the detector is switched on/off non-adiabatically in a finite time interval $\tau_f - \tau_i$, even in inertial motion there is a non-zero probability of excitation/de-excitation of the detector due to the time-energy uncertainty principle.
...
This is manifest in the above formulas since the time integrals do not yield perfect delta functions enforcing energy conservation, but rather functions peaked around the resonance frequencies $\omega_k = \pm (E_m - E_n)/\hbar$ with a width $\Delta \omega \sim 1/(\tau_f - \tau_i)$.
\medskip


\noindent
\textbf{Uniformly accelerated motion \& finite-temperature effects.}\todotag{Finish}
...
Calculting the transition amplitude and probability, one finds that the detector behaves as if it were immersed in a thermal bath of particles at the Unruh temperature
\begin{align}
