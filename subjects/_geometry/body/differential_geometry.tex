...
It is useful to count the actual physical degrees of freedom of a pseudo-Riemannian structure in \(n\) dimensions.
A symmetric covariant 2-tensor \(g_{\mu \nu}\) has \(\frac{n(n+1)}{2}\) independent components at each point.
However, diffeomorphisms can be used to eliminate \(n\) of them, which are spurious artifacts of coordinates.
The \textbf{number of physical degrees of freedom of a pseudo-Riemannian structure} is thus
\begin{equation}
    \frac{n(n+1)}{2} - n = \frac{n(n-1)}{2}.
\end{equation}
In \(n=4\) dimensions we have \(10-4=6\) physical degrees of freedom at each point.
Notably, in $n=2$ we only have one DoF, confirming that curvature in 2D is completely specified by a single scalar, the Gaussian curvature i.e. the sectional curvature of the only 2D plane in the tangent space.
