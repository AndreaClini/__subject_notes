...
\end{align}}
We remark this computation is extremely nontrivial for general Bogoliubov transformations, while much easier for diagonal ones.

The \textbf{scattering operator} is found by tweaking the operator relating the two vacua into a unitary one which is then disentangled using the algebra of ladder operators as discussed below, and reads
{\small\begin{align}
    \mathcal{S}:=\exp\left(\tfrac12a^\dagger_i(V\operatorname{arctgh}\Xi\, V^T)^*_{ij} a_j^\dagger - \tfrac12a_i(V\operatorname{arctgh}\Xi\, V^T)_{ij}a_j\right)=\exp\left(\tfrac12  \operatorname{arctgh}\Xi^* (b_i^\dagger)^2 - \tfrac12 \operatorname{arctgh}(\xi_j) (b_j)^2\right)\\
    =\exp\left(\tfrac12\tilde{a}^\dagger_i(U\operatorname{arctgh}\Xi\, U^T)^*_{ij} \tilde{a}_j^\dagger - \tfrac12\tilde{a}_i(U\operatorname{arctgh}\Xi\, U^T)_{ij}\tilde{a}_j\right)
    \exp\left(\tfrac12  \operatorname{arctgh}(\xi_j) (\tilde{b}_j^\dagger)^2 - \tfrac12 \operatorname{arctgh}(\xi_j) (\tilde{b}_j)^2\right)
\end{align}}
The equalities in the second line follow swapping the roles of the two theories with the replacements $\mathcal{S}\leftrightarrow \mathcal{S}^\dagger$, $\alpha \leftrightarrow \alpha^\dagger$ and $\beta \leftrightarrow -\beta^T$, in particular $\xi_j \leftrightarrow -\xi_j$ for diagonal Bogoliubov transformations.
It maps ladder operators and in turn any Fock stats with a given number of excitations into the corresponding ones with the same number of excitations in the other theory, 
...
{\color{red}
\begin{proof}
We proof the above identities in the diagonal basis.
...
\end{align}
The intuition and the rigorous procedure to disentangle the scattering operator is discussed below.
...
\end{align}}
Enforcing the initial conditions at $\lambda=0$, the unique solution $e^{-\lambda G} b e^{\lambda G} = b \cosh(\eta\lambda)-b^\dagger \sinh(\eta\lambda)$.
Evaluating at $\lambda=1$ we get the desired result
\begin{align}
