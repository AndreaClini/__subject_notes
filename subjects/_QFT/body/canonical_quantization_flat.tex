...
\begin{align}%\label{eq:1d_oscillator_ladder_operator_time_evolution}
    &i\hbar\,\partial_t a= [a, H_H(t,t_0)] = \hbar \omega a + \sqrt{\frac{\hbar}{2 m \omega}} J \quad
...
\begin{mytheorem}[Making semiclassical treatments rigorous: electron in classical EM field\questiontag{Polish}]
%
The above analysis of exciting harmonic oscillators allows to rigorously understand the treatment of a quantized electron in a classical electromagnetic field and other semiclassical treatments usually presented in basic QM courses.
...
If each mode of the EM field is in such a state, as expected due to arbitrary interactions, we understand why the above semiclassical treatment works so well.
...
\end{align}
where $H_{\mathrm{EM}} \sim \sum_{\mathbf{k},\lambda} \hbar \omega_k (a_{\mathbf{k},\lambda}^\dagger a_{\mathbf{k},\lambda} + \tfrac{1}{2})$ is the Hamiltonian of the free quantized EM field, $H_{\mathrm{e}^-}\sim p^2/2m$ is the Hamiltonian of the free electron and $H_{\mathrm{int}}\sim \vec{p}\otimes B$ describes a possible interaction.
...
However, if the EM field is in a coherent state $|\psi_{\mathrm{EM}}(t_0)\rangle = |\{\alpha_{\mathbf{k},\lambda}\}\rangle$, we can approximate the interaction Hamiltonian as
\begin{align}
    H_{\mathrm{int}} &= \vec{p} \otimes B \approx \vec{p} \otimes \langle \{\alpha_{\mathbf{k},\lambda}\}| \hat{B}(t,\mathbf{x}) | \{\alpha_{\mathbf{k},\lambda}\}\rangle = \vec{p} \otimes B^{cl}(t,\mathbf{x}) = B^{cl}(t,\mathbf{x})\cdot\,\, \vec{p}\otimes 1,
\end{align}
where $B^{cl}(t,\mathbf{x})$ is a classical EM field given by the expectation value of the field operator $\hat{B}$ on the coherent state.
...
\begin{mytheorem}[Coherent states of the KG field\todotag{Do it}]
    
\end{mytheorem}


\begin{mytheorem}[Exciting the KG field\todotag{Do it}]
    
\end{mytheorem}


\begin{mytheorem}[Semiclassical treatment made rigorous: an electron in a classical electromagnetic field\todotag{Expand \& finish}]
%
