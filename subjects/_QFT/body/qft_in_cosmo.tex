...
Assume the Stone--von Neumann theorem applies and decompose the quantum field in terms of two different sets of mode functions $\{u_n, u_n^*\}_n$ and $\{\tilde{u}_m, \tilde{u}_m^*\}_m$ related by a Bogoliubov transformation $\tilde{u}_m=\alpha_{mj} u_j + \beta_{mj} u_j^*$.
Equating the two expressions, we find ladder operators are related 
{\small
\begin{align}
    \hat{\phi}=\sum_j u_ja_j+u_j^*a_j^\dagger=\sum_m\tilde{u}_m\tilde{a}_m+\tilde{u}_m^* \tilde{a}_m^\dagger = \sum_j \Big(\sum_m \alpha_{mj} \tilde{a}_m + \beta_{mj}^* \tilde{a}_m^\dagger\Big) u_j + \Big(\sum_m \beta_{mj} \tilde{a}_m + \alpha_{mj}^* \tilde{a}_m^\dagger\Big) u_j^*
    \quad\Rightarrow\quad a_j= \sum_m \alpha_{mj} \tilde{a}_m + \beta_{mj}^* \tilde{a}_m^\dagger.
\end{align}}
That is concisely
\begin{align}
    \tilde{u}=\alpha u + \beta u^* \quad\Rightarrow\quad a= \alpha^T \tilde{a} + \beta^\dagger \tilde{a}^\dagger.
\end{align}
...
\end{align}
Now symmetrizing the PDE we get\todotag{Finish}
%
\end{mytheorem}
