...
\begin{align}
    \mathcal{L}= \sum_{n\in \Z^d}\frac{1}{2} m \dot{q}_n^2 - \frac{1}{2} m \omega_n^2 \sum_{j=1}^d (q_n - q_{n +\hat{e}_j})^2 - V(q_n).
\end{align}
The density of oscillators is $\rho = m / a^d$ and the speed of sound is $c_s = a \omega_n$, where $a$ is the lattice spacing, are kept constant as we take the continuum limit $a \to 0$.
...
\end{align}
where we redefined the field as $\phi(t,\mathbf{x}) := \rho^{1/2} q_n(t)= m^{1/2}a^{-d/2} q_n(t)$ and the potential density as $\mathcal{V}(\phi) := \lim_{a\to 0} a^{-d} V(\rho^{-1/2} \phi)$.
Choosing a quadratic potential $V(q_n) = \tfrac{1}{2} m^2 q_n^2$ yields the Klein-Gordon Lagrangian density for a free scalar field of mass $m$.
%
The field $\phi(t,\mathbf{x}) := \rho^{1/2} q_n(t)= m^{1/2}a^{-d/2} q_n(t)$ has mass dimension $[\phi] = \frac{d-1}{2}$ in $d+1$ spacetime dimensions.
Also note that we absorbed a factor $m^{1/2}$ in the rescaling of the field.
When we define \emph{adimensional} density variables for the field-constitutent harmonic oscillators below, the rescaling will thus be
\begin{equation}
    \tilde{q}_k = \sqrt{\frac{\omega_k}{\hbar}} q_k, \quad \tilde{p}_k = \frac{1}{\hbar}\sqrt{\frac{\hbar}{\omega_k}} p_k,
\end{equation}
rather than the usual $\tilde{q}_k = \sqrt{\frac{m\omega_k}{\hbar}} q_k$ and $\tilde{p}_k = \frac{1}{\hbar}\sqrt{\frac{\hbar}{m\omega_k}} p_k$, since the mass $m$ has already been absorbed and the \emph{dimensional} density variables $q_k,\,p_k$ will have corresponding dimensions.
%
\end{mytheorem}
...
On the other hand, a wave with fixed frequency will oscillate but maintain its shape--amplitude and frequency--overtime, freely travelling across the lake surface without deforming.
...
\begin{mytheorem}[Observable DoFs of the Klein-Gordon field in flat spacetime\todotag{Finish}]
%
...
\end{align}
In terms of creation and annihilation operators the canonical commutation relations become \footnote{Note that to get relativistically normalized commutation relations, we should use covariant creation and annihilation operators $a(\bar{k}) = \sqrt{2\omega_k} a_k$ and $a^\dagger(\bar{k}) = \sqrt{2\omega_k} a_k^\dagger$. This are indeed the operators creating relativistically normalized states in Minkowski spacetime, see later.}
\begin{align}
    [a_{\bar{k}}, a_{\bar{k}'}] = 0, \quad
    [a_{\bar{k}}^{\dagger}, a_{\bar{k}'}^{\dagger}] = 0, \quad
    [a_{\bar{k}}, a_{\bar{k}'}^{\dagger}] = (2\pi)^3 \delta^{(3)}(\bar{k} - \bar{k}').
\end{align}
%
The associated Hamiltonian, before normal ordering, is 
\begin{equation}
    \hat{H} = \int \frac{d^3 k}{(2\pi)^3} \, \hbar \omega_k \left( a_k^{\dagger} a_k + \tfrac{1}{2} (2\pi)^3 \delta^{(3)}(\underbrace{k-k}_{=0}) \right).
\end{equation}
%
The associated self-adjoint harmonic oscillators \emph{dimensional} $\{q_k, p_k\}_k$ and \emph{adimensional} $\{\tilde{q}_k, \tilde{p}_k\}_k$ $k$-space-density variables\footnote{In the sense of (a)-dimensional densities in Foruier space, that is $\propto\, k^{-3}$.} are identified as
\begin{align}
...
\end{align}
Note that we could have equivalently started from self-adjoint oscillators $\{q_k, p_k\}_k$, imposing field operators are related to these oscillators as above in order to obey the CCR, and then defined creation and annihilation operators as above.

In any case, this decomposition highlights that the true observable DoFs of the Klein-Gordon field are given by the self-adjoint harmonic oscillators $\{q_k, p_k\}_k$, or equivalently by the creation and annihilation operators $\{a_k, a_k^{\dagger}\}_k$, which indeed destroy or create quanta of the field with momentum $\bar{k}$ i.e. particles under suitable conditions.


Is the above decomposition unique?
...
This change the vacuum of the theory i.e. the state $|0\rangle$ annihilated by all $a_k$ operators.
This is not a problem in flat spacetime, where Poincaré invariance selects a preferred vacuum, also coinciding with the ground state of the Hamiltonian, but it becomes a serious issue in curved spacetime, where no preferred vacuum exists in general.
%
\end{mytheorem}


\begin{mytheorem}[Fluctuations of the Kelin-Gordon field in momentum \& position space\todotag{Huge fix needed}]
%
{\color{red}
Here there is a mistake $\phi_k$ is a complex oscillator. The states $n_k$ you are referring to are not eigenstates of $\phi_k$ but of the real and imaginary parts separately. You need to rewrite everything in terms of real oscillators $q_k, p_k$ or creation-annihilation operators $a_k, a_k^\dagger$.
...
\begin{align}
    |g\rangle &= \int_{W_{\phi}} Df\,\, |f\rangle\, \langle f | g \rangle, \quad \text{with}\quad
...
%
The value $\Psi[f]$ is the probability amplitude density of finding the field with configuration $\{f(x)\}_x$ when the system is in the state $|\Psi\rangle$.
That is, the probability density of finding the field with configuration between $\{f(x)\}_x$ and $\{f(x)+df(x)\}_x$ is
\begin{align}
    P_{\Psi}(\{f(x)\}_x) \,Df &= |\langle f | \Psi \rangle|^2 \,Df = |\Psi[f]|^2 \,Df.
\end{align}
Analogous considerations hold for $\Psi[g]$ and $P_{\Psi}(\{g(x)\}_x) \,Dg$ in the $\hat{\pi}$ representation.
%

Finally, just like in ordinary QM, the field operators act on functionals of the field configurations $f$ (or $g$) as functional multiplication and functional differentiation operators on their respective eigenstates, 
\begin{align}
...
\begin{align}
    \langle f | \hat{\phi}(x) | \Psi \rangle &= f(x) \Psi[f], \quad
    \langle f | \hat{\pi}(x) | \Psi \rangle = -i \hbar \frac{\delta}{\delta f(x)} \Psi[f], \\
    \langle g | \hat{\pi}(x) | \Psi \rangle &= g(x) \Psi[g], \quad
    \langle g | \hat{\phi}(x) | \Psi \rangle = i \hbar \frac{\delta}{\delta g(x)} \Psi[g].
\end{align}
%
Implementing the $\hat{\phi}$ and $\hat{\pi}$ operators in any of the two ways indeed reproduces the canonical commutation relations (easy check) and thus provides two (unitaryly equivalent) representations of the operator algebra on the Fock space $\mathcal{F}$.
%
...
For example, if the system is in one of the field configuration eigenstates $|f_0\rangle$, its wave functional is simply
\begin{align}
