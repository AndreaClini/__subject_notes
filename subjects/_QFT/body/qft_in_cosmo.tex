...
\end{align}
Similar `rotations' can be applied to the real basis of $\mathcal{S}_\R$ preserving the Darboux form of the symplectic structure, which shows the above bases are far from unique and in fact there exists an infinite family of them related by Bogoliubov transformations.
%
\end{mytheorem}
...
\end{align}
Since we used the same coordinates $x^\mu$, both hamiltonians must generate the same time evolution wrt the parameter $x^0=\tau$, and it might seem puzzling we have two different hamiltonians.
The puzzle is solved recalling that, both in the classical and quantum case, the time evolution of a field (resp.operator) operator is given by the Poisson bracket (resp. commutator) with the hamiltonian expressed in terms of the chosen canonical variables/DoFs plus any additional explicit time-dependence of the operator itself not captured by the time evolution of the canonical variables, that is
\begin{align}
    \frac{d\hat{O}}{dt} = \frac{\partial \hat{O}}{\partial t} + \{\hat{O}, \hat{H}\}.
\end{align}
For example, we can see the time evolution of the field $\chi(t)=a(t)\phi(t)$ as due to the sole evolution of $\chi$ or the evolution of $\phi$ plus the additional explicit time-dependence of the scale factor
\begin{align}
  \chi(t) &= e^{\tfrac{i}{h}H_\chi t} \chi(0) e^{-\tfrac{i}{h}H_\chi t} =a(t)\phi(t)= a(t) e^{\tfrac{i}{h}H_\phi t} \phi(0) e^{-\tfrac{i}{h}H_\phi t}.
\end{align}
In any case
\begin{align}
\frac{d\chi}{dt} &= \frac{\partial \chi}{\partial t} + \{\chi, H_\phi\} = \mathcal{H} a \phi + a \dot{\phi} = \pi_\chi = 0+ \pi_\chi = \frac{\partial \chi}{\partial t} + \{\chi, H_\chi\}\,.
\end{align}
