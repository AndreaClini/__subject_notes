...
\end{align}
More generally the `scattering operator' $\mathcal{S}=\exp\left(J_0 a_{\mathrm{out}}^\dagger - J_0^* a_{\mathrm{out}}\right)$ is the unitary operator relating the Fock spaces and ladder operators as
\begin{align}
    a_{\mathrm{out}} &= \mathcal{S} a_{\mathrm{in}} \mathcal{S}^\dagger, \quad |n\rangle_{\mathrm{out}} = \mathcal{S} |n\rangle_{\mathrm{in}}.
\end{align}
The above identity is most easily verified by defining $G=J_0 a_{\mathrm{out}}^\dagger - J_0^* a_{\mathrm{out}}$ and then solving the evolution equation
