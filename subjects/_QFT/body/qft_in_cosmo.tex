...
\begin{mytheorem}[On the equivalence of Bogoliubov bases\todotag{Finish \& polish}]
%
...
At the classical level, different bases ultimately yield the same space and mode functions are related by Bogoliubov transformations.
Do different Bogoliubov bases also yield equivalent quantizations?

If the number of modes if finite, the Stone--von Neumann theorem ensures they are indeed equivalent, in the sense that there exists a unitary transformation relating the two Fock spaces and mapping the ladder operators of one basis to the other,
\begin{align}
...
\begin{align}\label{eq:shale_stinespring_condition}
     \sum_{n,i} |\beta_{ni}|^2 < \infty.
\end{align}
In this case the two quantizations are not unitarily equivalent, the spaces $\mathcal{F}$ and $\tilde{\mathcal{F}}$ are unrelated and yield different physics.
This is a peculiarity of infinite-dimensional systems, and in particular of quantum field theories, since for finite-dimensional systems all Bogoliubov transformations are indeed always unitarily implementable and thus all quantizations are equivalent.\todotag{add short explanation}
