...
The Dirac equation for spin-$\tfrac{1}{2}$ fields $\psi$ in curved spacetime is given by
\begin{align}
    (i \gamma^\mu \nabla_\mu - m) \psi = 0,
\end{align}
where $\gamma^\mu=\gamma^\mu(x)$ are spacetime dependent gamma matrices satisfying the Clifford algebra $\{\gamma^\mu, \gamma^\nu\} = 2 g^{\mu\nu}(x)$ and $\nabla$ is the covariant derivative on the spinor bundle.
The Dirac equation in curved spacetime generalizes the flat spacetime Dirac equation by replacing the ordinary gamma matrices and partial derivatives with their curved spacetime counterparts.
...
This is more easily understood using an orthonormal tetrad $\{e_a(x)\}_a$ for the tangent bundle, so that $g_{ab} := g(e_a,e_b)\equiv \eta_{ab}$ is the Minkowski metric at each point $x\in M$ in such local basis.
In this tetrad we can simply define constant gamma matrices $\gamma_{\mathrm{flat}}^a$ satisfying the flat spacetime Clifford algebra $\{\gamma_{\mathrm{flat}}^a, \gamma_{\mathrm{flat}}^b\} = 2 \eta^{ab}$.
In a coordinate system $x^\mu$ where $e_a(x)=e_a^\mu(x) \partial_\mu$ the spacetime dependent gamma matrices are then simply given by
\begin{align}
    \gamma^\mu(x) = e_a^\mu(x) \gamma_{\mathrm{flat}}^a\,, \quad \text{since}\quad \{\gamma^\mu(x), \gamma^\nu(x)\} = 2 \eta^{ab} e_a^\mu(x) e_b^\nu(x) = 2 g^{\mu\nu}(x).
\end{align}

Finally recall that, given a metric connection $\nabla$ with connection 1-form $\omega_j^i(\bullet)$ in a tetrad frame $\{e_a\}_a$ of the tangent bundle, and given representation of $\gamma$-matrices $\gamma^a(x)=\gamma^a(x)^i_j$ obeying the Clifford algebra $\{\gamma^a(x),\gamma^b(x)\}= 2 g^{ab}(x)$ in such a tetrad, the covariant derivative acts on \emph{basis} spinors $s_j=(0\dots0\,\underbrace{1}_j\,0\dots0)$ as
\begin{align}
    \nabla_\bullet s_j = \frac{1}{2} \omega_{ab}(\bullet) \big(S^{a b}\big)^i_{j} \,s_j,\quad \text{where}\quad S^{ab}(x) := \frac{1}{4} [\gamma^a(x), \gamma^b(x)],
\end{align}
are the generators of the spin representation of the Lorentz group.
The action on a general section of the spinor bundle $\psi=s_j\, \psi^j$ is then obtained by the Leibniz rule
\begin{align}
    \nabla \psi = s_j \left( d\psi^j + \frac{1}{2} \omega_{ab} \big(S^{a b}\big)^j_{k} \, \psi^k \right).
\end{align}
%
\end{mytheorem}
}
