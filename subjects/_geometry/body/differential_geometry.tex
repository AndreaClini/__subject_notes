...
%
Given an affine connection $\nabla$ on a fibre bundle $E_F \to M$ with fiber $F$ and structure group $G$, and a local frame $\{f_a\}_a$, we define the $\mathfrak{g}$-valued connection 1-forms $\omega^{(F)}$ and $\R$-valued connection 1-forms $\omega^i_{\,j}$ as the set of 1-forms satisfying
\begin{equation}
