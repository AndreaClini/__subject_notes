...
\begin{align}\label{eq:inverse_transformation_bogoliubov_modes}
...
In particular, the vacuum of the first theory must be expressible as a suitable superposition of multi-particle states of the second theory $|0\rangle =f\big(\{\tilde{a}^\dagger_n\}_n\big)|\tilde{0}\rangle$ for a suitable function $f$ of creation operators.\footnote{Note $f$ will indeed be independent of annihilation operators $\tilde{a}$ since they annihilate the vacuum $|\tilde{0}\rangle$ and we may commute them to the right upon changing the form of the $f$.}
The $\tilde{a}_m^\dagger$ operators commute with each other, while acting with $\tilde{a}_m$ is equivalent to taking a derivative $[\tilde{a}_m,f\big(\{\tilde{u}^\dagger_n\}_n\big)] = \tfrac{\partial f}{\partial \tilde{a}_m^\dagger}$.
Imposing the vacuum condition in the first theory we thus get
{\small
\begin{align}
     0=a_j|0\rangle = \Big(\sum_m\alpha_{mj} \tilde{a}_m + \beta_{mj}^* \tilde{a}_m^\dagger\Big) |0\rangle
     = \Big(\sum_m\alpha_{mj} \tilde{a}_m + \beta_{mj}^* \tilde{a}_m^\dagger\Big) f\big(\{\tilde{a}^\dagger_n\}_n\big)|\tilde{0}\rangle = \sum_m \Big(\alpha_{mj} \frac{\partial f}{\partial \tilde{a}_m^\dagger} + \beta_{mj}^* \tilde{a}_m^\dagger f\Big)|\tilde{0}\rangle.
\end{align}}
Since this must hold for arbitrary $j$ we get a set of differential equations for $f$
\begin{align}
    \sum_m \Big(\alpha_{mj} \frac{\partial f}{\partial \tilde{a}_m^\dagger} + \beta_{mj}^* \tilde{a}_m^\dagger f\Big) = 0 \quad \forall j \quad \text{i.e.}\quad \nabla_{\tilde{a}^\dagger}f = -(\alpha^T)^{-1}\beta^\dagger \tilde{a}^\dagger f
    \,\,\Rightarrow\,\, f\big(\tilde{a})\, \propto\,  \exp\Big(-\tfrac{1}{2} \tilde{a}^\dagger_m \big[(\alpha^T)^{-1}\beta^\dagger\big]_{mn} \tilde{a}^\dagger_n\Big).
\end{align}
Beware the matrix $(\alpha^T)^{-1}\beta^\dagger=\beta^*\alpha^{-1}$ is indeed symmetric since \eqref{eq:bogol_transform_mode_function_matrix_form_quantum} enforces $\beta^T\alpha^*-\alpha^\dagger\beta=0$.
Normalizing the vacuum states we find\todotag{fnish}
{\small
\begin{align}
    |0\rangle = \frac{1}{|\det(\alpha)|^{\tfrac12}} e^{-\frac{1}{2} \tilde{a}^\dagger (\alpha^T)^{-1}\beta^\dagger \tilde{a}^\dagger} |\tilde{0}\rangle.
\end{align}}
More generally, using the Baker--Campbell--Hausdorff formula we can express any state of the first theory as a suitable superposition of states of the second theory, namely
{\small
\begin{align}
    ...
\end{align}}
