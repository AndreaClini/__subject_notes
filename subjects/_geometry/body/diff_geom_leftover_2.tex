...
\begin{mytheorem}[Euler-Lagrange equations in curved spacetime for arbitrary-spin fields \todotag{check}]
%
...
\end{align}
The lagrangian density $\mathcal{L}$ is a \emph{scalar} function of the following tensors\footnote{To be precise, for half-integer spin the first two are sections of a suitable fiber bundle.}: the fields $\phi^\star_{a,\bullet}$, possibly with spin, their covariant derivatives $\nabla_\mu \phi^\star_{a,\bullet}$ and the metric $g_{\mu\nu}$.
...
However, the resulting expression would be unnecessarily cumbersome.\footnote{Of course, when $\phi_a$ are scalar fields then $\partial_\mu\phi_a=\nabla_\mu\phi_a$.} Indeed, the lagrangian must be a scalar--i.e. a contraction of tensors--whereas partial derivatives of tensors are not even tensors, so that in order to obtain a correct expression we would need to introduce Christoffel symbols `implictly' everywhere needed anyway to make covariant derivatives appear.
...
Since variations of tensors\footnote{More generally, variations of \emph{sections} of any fiber bundle $E_F\to M$ wrt some internal parameter $\epsilon$.} $\frac{d T}{d\epsilon}$ are tensors of he same kind and the left-hand side is a scalar, we conclude that $\frac{\partial \mathcal{L}}{\partial \phi^\bullet_{a,\bullet}}$ is a $(\bullet,\star)$-tensor with opposite raised (resp. lowered) indices as $\phi^\star_{a,\bullet}$, $\frac{\partial \mathcal{L}}{\partial (\nabla_\mu \phi^\star_{a,\bullet})}$ is a tensor with one upper index more, and $\frac{\partial \mathcal{L}}{\partial g_{\mu\nu}}$ is a (2,0)-tensor.
On the other hand, if instead we considered a parametrization $\epsilon_4 \mapsto \partial_\mu \phi(\epsilon_4)$ its variation would not be a tensor, simply beacuse $\partial_\mu \phi^\star_{a,\bullet}$ is not a tensor from the start.
The corresponding variation of the lagrangian
{\small
\begin{align}
    \underbrace{\frac{\mathrm{d}\mathcal{L}}{\mathrm{d}\epsilon_4}}_{\mathrm{scalar}}=
    \frac{\partial \mathcal{L}}{\partial (\partial_\mu \phi^\star_{a,\bullet})} \frac{\mathrm{d}(\partial_\mu \phi^\star_{a,\bullet})}{\mathrm{d}\epsilon_4}
\end{align}
}
...
The \textbf{equations of motion} for the fields are \textbf{obtained varying with respect to $\phi^\star_{a,\bullet}$} \todotag{Fix all derivatives $\partial_\mu \to \nabla_\mu$} 
{\small
\begin{align}
...
Ineed, the derivative $\frac{\partial \mathcal{L}}{\partial (\nabla_\mu \phi^\star_{a,\bullet})}$ returns a tensor whose indices are all contracted except for one giving the vectr $X^\mu:=\frac{\partial \mathcal{L}}{\partial (\nabla_\mu \phi^\star_{a,\bullet})}\phi^\star_{a,\bullet}$. 
%

In the second passage we used that \textbf{covariant derivatives and variations commute} $\delta (\nabla_\mu \phi^\star_{a,\bullet}) = \nabla_\mu \delta \phi^\star_{a,\bullet}$.
This follows from commutation of \emph{ordinary} partial derivatives with variations $\delta$, which are just derivatives wrt an internal parameter, and the definition of covariant derivative for general sections of fiber bundles.
For example, for a section of $T^\star_\bullet M$ we have
\begin{align}
    \delta (\nabla_\mu \phi^\star_{a,\bullet})
    &\simeq \delta \left( \partial_\mu \phi^\star_{a,\bullet} -\Gamma_{\mu \#}^\star \phi^{\#}_{a,\bullet} + \Gamma_{\mu\bullet}^{\#}\phi^\star_{a,\#}\right)
    \\
    &= \partial_\mu \delta \phi^\star_{a,\bullet} - \Gamma_{\mu \#}^\star \delta \phi^{\#}_{a,\bullet} + \Gamma_{\mu\bullet}^{\#}\delta \phi^\star_{a,\#}
    = \nabla_\mu \delta \phi^\star_{a,\bullet}.
\end{align}
For general fiber bundles a similar reasoning applies, using the definition of covariant derivative in terms of a suitable representation of the connection one-form. For example for spinor fields one uses the spin connection as we defined above.
...
\end{align}
We remark the \textbf{above formulas hold for arbitrary-spin fields} $\phi^{\star}_{a,\bullet}$, including scalars, vectors, tensors and spinors, \textbf{arranged in any multiplet} $\{\phi_a\}_a$, provided the Lagrangian density $\mathcal{L}$ is a scalar, as it should.
\end{mytheorem}
